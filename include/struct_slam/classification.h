...
//self msg
#include <struct_slam/opt_point.h>
// #include <struct_slam/CompressedSensorData.h>
#include <struct_slam/ClassificationData.h>
//追加（デバッグ用）
...
#include <struct_slam/classificationConfig.h>
//クラスの定義
class classificationClass{
    private:
        ...
        struct_slam::opt_point smdCamera;
        //struct_slam::opt_point smdCamera, smdLRF;

        //送信データ
		ros::NodeHandle nhPub;
        ros::Publisher pub;
        struct_slam::ClassificationData cd;
        //クラスタリング処理
        // std::vector<int> mapIndex;//
    	std::vector<int> winIndex;//基準点（コア点）から参照値
        // struct_slam::CompressedSensorData compCamData;
        //--カメラパラメータ
        ...
        dynamic_reconfigure::Server<struct_slam::classificationConfig> server;
        dynamic_reconfigure::Server<struct_slam::classificationConfig>::CallbackType f;
    public:
        //in constracter.cpp
        ...
        void cameraMap_callback(const struct_slam::opt_point::ConstPtr& msg);
        //void subscribeSensorDataLRF();//データ受信
        //void laserMap_callback(const struct_slam::opt_point::ConstPtr& msg);
        //--rqt_reconfigureからの読み込み
        void configCallback(struct_slam::classificationConfig &config, uint32_t level);
        //--クラスタリング
