...
#include <struct_slam/imageMatchingConfig.h>
//debug
...
class imageMatchingClass{
    private:
        ...
        struct_slam::MaskImageData midPre,midCur;//midCur（画像データコピー用）
        bool imgCurOnce,imgPreOnce;
        //送信データ
		ros::NodeHandle nhPub;
        ros::Publisher pubMatch;
        struct_slam::ImageMatchingData matchData;//matchData(トピック送信用）
        //マップパラメータ
        float mapW;//width[m]
        float mapH;//height[m]
        ...
        //特徴点マッチング
        std::vector< cv::Point2f > featurePointsPre;//前のRGB画像
        std::vector< cv::Point2f > featurePointsCur;//今のRGB画像
        std::vector< cv::Point2f > featurePointsTemp;//コピーRGB画像
        ...
        dynamic_reconfigure::Server<struct_slam::imageMatchingConfig> server;
        dynamic_reconfigure::Server<struct_slam::imageMatchingConfig>::CallbackType f;
    ...
    public:
        ...
        //セット：内部パラメータの書き込み
        void setLaunchParam();
        void configCallback(struct_slam::imageMatchingConfig &config, uint32_t level);
        //ゲット：内部パラメータの読み込み
        int& getParam();//(未使用)
        //
        //in methods.cpp
        ...
        void maskImage_callback(const struct_slam::MaskImageData::ConstPtr& msg);//データ受信(mask画像)
        //--グレースケール化
        void cvtGray();
        //--データ確認
