...
//3次元空間のある点が与えられたとき，同じカメラで撮影された歪みのない画像または順方向の歪み係数を持つ画像における対応するピクセル座標を計算する．
...
/* 歪みや逆歪み係数のない画像のピクセル座標と深度が与えられた場合，同じカメラに対する3次元空間上の対応点を計算する */ 
static void rs2_deproject_pixel_to_point(float point[3], const struct rs2_intrinsics * intrin, const float pixel[2], float depth)
{
    assert(intrin->model != RS2_DISTORTION_MODIFIED_BROWN_CONRADY); // Cannot deproject from a forward-distorted image// 前方に歪んだイメージからのデプロージョンができない
    //assert(intrin->model != RS2_DISTORTION_BROWN_CONRADY); // Cannot deproject to an brown conrady model// 茶色のコンラディモデルにはデプロイできない
    ...
    {
        // need to loop until convergence // 収束するまでループする必要がある
        // 10 iterations determined empirically// 経験的に決定された10回のイテレーション
        for (int i = 0; i < 10; i++)
        {
            float r2 = x * x + y * y;
...
/* あるセンサーを基準とした3次元座標を、別の視点を基準とした3次元座標に変換する。*/
    ...
    const float from_pixel[2])
{
    ...
    //投影されたピクセルが入力ピクセルに最も近い深度ピクセルをラインに沿って探索する
    float min_dist = -1;
