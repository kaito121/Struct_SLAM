...
int kal_est=0;//カルマンフィルタの推定回数を記録する
int kal_new=0;//カルマンフィルタの更新回数を記録する
...
double LMWL[200][2],LMZL[200];//特徴点世界座標判断用配列
double LML[40];
int LMWLID[200],markerF[40];//マーカーの観測確認用
...
ofstream C_point("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/C_point.txt");
ofstream C_pointx("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/C_pointx.txt");
ofstream C_pointz("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/C_pointz.txt");

ofstream kal_kaisu("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/kal_kaisu.txt");//カルマンフィルタの推定回数と更新回数を記録する
...
//
  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      if(depth[markerIds.at(i)]>0&&depth[markerIds.at(i)]<10000){
        ...
        markerF[markerIds.at(i)]=1;//マーカーがある時
      }
  ...
  //初回検出プログラム-----------------------------------------------------------------------------------------------------
    //検出時にマーカが観測可能なときのみ検出実行(観測できない時は検出せず再度検出を行う)
  if (reset == true) {
    if(markerIds.size() > 0){
      ...
      //cv::goodFeaturesToTrack(今画像, 今の特徴点, 特徴点の個数, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
		  cv::goodFeaturesToTrack(image_curr, points_curr, 20, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);// 特徴点検出(グレースケール画像から特徴点検出)
      ...
      camera_point_c.resize(points_curr.size());//要素数初期設定(座標変換用)
        ...
        if(points_curr[i].x>=0&&points_curr[i].y>=0){
          ...
          if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
            ...
            //std::cout << "特徴点のカメラ座標:camera_point_p[depth_point_curr_ok="<<depth_point_curr_ok<<"]={"<< camera_point_p[depth_point_curr_ok].x <<","<<camera_point_p[depth_point_curr_ok].y<<","<<camera_point_p[depth_point_curr_ok].z<<"}"<< std::endl;
      ...
      W_point<<"世界座標更新ALLRealsec="<<ALLrealsec<<"\n";
      //カメラ座標→世界座標変換-------------(20211023もっとも近いマーカーに対応付けをする)
      for(int i=0;i<depth_point_curr_ok;i++){
          double miniLM=1000;
          int miniID=0;
        //観測された全マーカーと比較し最も距離が近いマーカーに対応させる
        for(int j=0;j<markerIds.size();j++){
          LML[markerIds.at(j)]=sqrt((MarkerW[markerIds.at(j)].at<float>(0)-camera_point_c[i].x)*(MarkerW[markerIds.at(j)].at<float>(0)-camera_point_c[i].x)-(MarkerW[markerIds.at(j)].at<float>(2)-camera_point_c[i].z)*(MarkerW[markerIds.at(j)].at<float>(2)-camera_point_c[i].z));
          if(miniLM<=LML[markerIds.at(j)]){
            miniLM=LML[markerIds.at(j)];
            miniID=markerIds.at(j);//最も近いマーカーIDを保存
          }
        }
	      std::cout <<"test4"<< std::endl;
	      std::cout <<"MarkerW["<<markerIds.at(miniID)<<"](0,0)="<<MarkerW[markerIds.at(miniID)].at<float>(0)<< std::endl;
	      std::cout <<"MarkerW["<<markerIds.at(miniID)<<"](1,0)="<<MarkerW[markerIds.at(miniID)].at<float>(1)<< std::endl;
	      std::cout <<"MarkerW["<<markerIds.at(miniID)<<"](2,0)="<<MarkerW[markerIds.at(miniID)].at<float>(2)<< std::endl;
	      std::cout <<"MC_point["<<markerIds.at(miniID)<<"][0]="<<MC_point[markerIds.at(miniID)][0]<< std::endl;
	      std::cout <<"MC_point["<<markerIds.at(miniID)<<"][1]="<<MC_point[markerIds.at(miniID)][1]<< std::endl;
	      std::cout <<"MC_point["<<markerIds.at(miniID)<<"][2]="<<MC_point[markerIds.at(miniID)][2]<< std::endl;
	      std::cout <<"camera_point_c["<<i<<"].x="<<camera_point_c[i].x<< std::endl;
	      std::cout <<"camera_point_c["<<i<<"].y="<<camera_point_c[i].y<< std::endl;
	      std::cout <<"camera_point_c["<<i<<"].z="<<camera_point_c[i].z<< std::endl;

        //world_point_c[i].x=MarkerW[markerIds.at(miniID)].at<float>(0)-MC_point[markerIds.at(miniID)][0]+camera_point_c[i].x;
        //world_point_c[i].y=MarkerW[markerIds.at(miniID)].at<float>(1)-MC_point[markerIds.at(miniID)][1]+camera_point_c[i].y;
        //world_point_c[i].z=MarkerW[markerIds.at(miniID)].at<float>(2)-MC_point[markerIds.at(miniID)][2]+camera_point_c[i].z;

        world_point_c[i].x=MarkerW[markerIds.at(miniID)].at<float>(0)+(-camera_point_c[i].x+MC_point[markerIds.at(miniID)][0])*cos(-Est_RobotTH)-(camera_point_c[i].z-MC_point[markerIds.at(miniID)][2])*sin(-Est_RobotTH);
        world_point_c[i].y=MarkerW[markerIds.at(miniID)].at<float>(1)+(-camera_point_c[i].y+MC_point[markerIds.at(miniID)][1]);
        world_point_c[i].z=MarkerW[markerIds.at(miniID)].at<float>(2)+(camera_point_c[i].x-MC_point[markerIds.at(miniID)][0])*sin(-Est_RobotTH)+(camera_point_c[i].z-MC_point[markerIds.at(miniID)][2])*cos(-Est_RobotTH);

        std::cout <<"world_point_c["<<i<<"]=("<<world_point_c[i].x<<","<<world_point_c[i].y<<","<<world_point_c[i].z<<")" <<std::endl;
        W_point<<"world_point_c["<<i<<"]=("<<world_point_c[i].x<<","<<world_point_c[i].y<<","<<world_point_c[i].z<<")" <<"\n";


        CameraLMP[0]=sqrt((camera_point_c[i].x*camera_point_c[i].x)+(camera_point_c[i].z*camera_point_c[i].z));
        CameraLMP[1]=atan2(camera_point_c[i].x,camera_point_c[i].z);
        Zu_P[i] = (cv::Mat_<double>(2,1) <<
          CameraLMP[0],
          CameraLMP[1]);

        //マーカーの世界座標と求めた特徴点の世界座標との距離を求め保存する(この距離を使って求めた特徴点の世界座標が正しいかの判断を行う)
        LMWL[i][0]=sqrt((world_point_c[i].x-MarkerW[markerIds.at(miniID)].at<float>(0))*(world_point_c[i].x-MarkerW[markerIds.at(miniID)].at<float>(0))+(world_point_c[i].z-MarkerW[markerIds.at(miniID)].at<float>(2))*(world_point_c[i].z-MarkerW[markerIds.at(miniID)].at<float>(2)));
        LMWL[i][1]=0;//判断配列ここの要素がしきい値以上になったらその特徴点を更新に使用しない
        LMWLID[i]=miniID;//対応するマーカーのIDを保存
      }
      reset = false;//if文切り替え
	    std::cout <<"初回検出プログラム終了"<< std::endl;
    }
    else{
	    std::cout <<"追跡特徴点なし"<< std::endl;
	    C_point<<"追跡特徴点なし\n";
    }
  }
  //オプティカルフロー-------------------------------------------------------------------------------------
    ...
    cv::calcOpticalFlowPyrLK(image_prev, image_curr, points_prev, points_curr, status, err);//オプティカルフロー
	  ...
	  for (i = k = n =0; i < status.size(); i++){
        ...
        world_point_c[k] = world_point_c[i];
        LMWL[k][0] = LMWL[i][0];
        LMWL[k][1] = LMWL[i][1];
        LMWLID[k] = LMWLID[i];
    ...
    for (j= n = 0; j < points_curr.size(); j++){
      ...
      if(abs(points_prev[j].x-points_curr[j].x)<50||abs(points_prev[j].y-points_curr[j].y)<50){
          ...
          world_point_c[n] = world_point_c[j];
          LMWL[n][0] = LMWL[j][0];
          LMWL[n][1] = LMWL[j][1];
          LMWLID[n] = LMWLID[j];
    ...
    world_point_c.resize(n);
      ...
      if(points_curr[i].x>=0&&points_curr[i].y>=0){
        ...
        cv::circle(img_dst, points_prev[i], 3, Scalar(255,255,255), -1, cv::LINE_AA);//一つ前の画像の座標
	      cv::circle(img_dst, points_curr[i], 4, Scalar(0, 255, 0), -1, cv::LINE_AA);//今の座標情報
        ...
        if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
          ...
          world_point_c[depth_point_curr_ok]=world_point_c[i];
          points_prev[depth_point_curr_ok] = points_prev[i];
          points_curr[depth_point_curr_ok] = points_curr[i];
          LMWL[depth_point_curr_ok][0] = LMWL[i][0];
          LMWL[depth_point_curr_ok][1] = LMWL[i][1];
          LMWLID[depth_point_curr_ok] = LMWLID[i];
          depth_point_curr_ok=depth_point_curr_ok+1;//Depth取得可能の個数をカウント
        }
      }
    }
    opt_pointx<<camera_point_c[10].x<<"\n";
    opt_pointy<<camera_point_c[10].y<<"\n";
    opt_pointz<<camera_point_c[10].z<<"\n";

    cv::circle(img_dst, points_prev[0], 6, Scalar(0,0,255), -1, cv::LINE_AA);//一つ前の画像の座標
	  cv::circle(img_dst, points_curr[0], 6, Scalar(255, 0, 0), -1, cv::LINE_AA);//今の座標情報
    ...
    //カメラ座標→世界座標変換-------------(20211021とりあえずIDが一番小さいマーカーの座標を使用)
    for(int i=0;i<depth_point_curr_ok;i++){
      //world_point_c[i].x=MarkerW[markerIds.at(0)].at<float>(0)-MC_point[markerIds.at(0)][0]+camera_point_c[i].x;
      //world_point_c[i].y=MarkerW[markerIds.at(0)].at<float>(1)-MC_point[markerIds.at(0)][1]+camera_point_c[i].y;
      //world_point_c[i].z=MarkerW[markerIds.at(0)].at<float>(2)-MC_point[markerIds.at(0)][2]+camera_point_c[i].z;
	    //std::cout <<"world_point_c["<<i<<"].x="<<world_point_c[i].x<< std::endl;
	    //std::cout <<"world_point_c["<<i<<"].y="<<world_point_c[i].y<< std::endl;
	    //std::cout <<"world_point_c["<<i<<"].z="<<world_point_c[i].z<< std::endl;
      C_point<<"camera_point_c["<<i<<"]=("<<camera_point_c[i].x<<","<<camera_point_c[i].y<<","<<camera_point_c[i].z<<")" <<"\n";
      std::cout <<"camera_point_c["<<i<<"]=("<<camera_point_c[i].x<<","<<camera_point_c[i].y<<","<<camera_point_c[i].z<<")" <<"\n";
      ...
      Zu_P[i] = (cv::Mat_<double>(2,1) <<
        CameraLMP[0],
        CameraLMP[1]);
      W_point<<"world_point_c["<<i<<"]=("<<world_point_c[i].x<<","<<world_point_c[i].y<<","<<world_point_c[i].z<<")" <<",camera_point_c["<<i<<"]=("<<camera_point_c[i].x<<","<<camera_point_c[i].y<<","<<camera_point_c[i].z<<")" <<"\n";

      //ここを一番近いマーカーが認識できなくなったら止めたい
      if(markerF[LMWLID[i]]==1){
        //観測したランドマークと特徴点間の距離を保存する
        LMZL[i]=sqrt((camera_point_c[i].x-MC_point[markerIds.at(0)][0])*(camera_point_c[i].x-MC_point[markerIds.at(0)][0])+(camera_point_c[i].z-MC_point[markerIds.at(0)][2])*(camera_point_c[i].z-MC_point[markerIds.at(0)][2]));

        //世界座標距離と観測距離の差がしきい値以上ならフラグを立てる→フラグが5回立ったら再度特徴点の世界座標を求める(マーカー観測時のみ)
        if(abs(LMWL[i][0]-LMZL[i])>0.05){
          LMWL[i][1]=LMWL[i][1]+1;
        }
        std::cout <<"LMWL["<<i<<"][1]=("<<LMWL[i][1]<<")\n";
        std::cout <<"abs(LMWL["<<i<<"][0]-LMZL["<<i<<"])=>abs("<<LMWL[i][0]<<"-"<<LMZL[i]<<")="<<abs(LMWL[i][0]-LMZL[i])<<"\n";

        //フラグが5回立ちかつマーカーが観測可能時,特徴点の世界座標の更新を行う
        if(LMWL[i][1]>=5){
          //world_point_c[i].x=MarkerW[markerIds.at(LMWLID[i])].at<float>(0)-MC_point[markerIds.at(LMWLID[i])][0]+camera_point_c[i].x;
          //world_point_c[i].y=MarkerW[markerIds.at(LMWLID[i])].at<float>(1)-MC_point[markerIds.at(LMWLID[i])][1]+camera_point_c[i].y;
          //world_point_c[i].z=MarkerW[markerIds.at(LMWLID[i])].at<float>(2)-MC_point[markerIds.at(LMWLID[i])][2]+camera_point_c[i].z;

          world_point_c[i].x=MarkerW[markerIds.at(LMWLID[i])].at<float>(0)+(-camera_point_c[i].x+MC_point[markerIds.at(LMWLID[i])][0])*cos(-Est_RobotTH)-(camera_point_c[i].z-MC_point[markerIds.at(LMWLID[i])][2])*sin(-Est_RobotTH);
          world_point_c[i].y=MarkerW[markerIds.at(LMWLID[i])].at<float>(1)+(-camera_point_c[i].y+MC_point[markerIds.at(LMWLID[i])][1]);
          world_point_c[i].z=MarkerW[markerIds.at(LMWLID[i])].at<float>(2)+(camera_point_c[i].x-MC_point[markerIds.at(LMWLID[i])][0])*sin(-Est_RobotTH)+(camera_point_c[i].z-MC_point[markerIds.at(LMWLID[i])][2])*cos(-Est_RobotTH);

          LMWL[i][0]=sqrt((world_point_c[i].x-MarkerW[markerIds.at(LMWLID[i])].at<float>(0))*(world_point_c[i].x-MarkerW[markerIds.at(LMWLID[i])].at<float>(0))+(world_point_c[i].z-MarkerW[markerIds.at(LMWLID[i])].at<float>(2))*(world_point_c[i].z-MarkerW[markerIds.at(LMWLID[i])].at<float>(2)));
          LMWL[i][1]=0;//判断配列ここの要素がしきい値以上になったらその特徴点を更新に使用しない
        }
      }
    }
      zn_pointl<<Zu_P[0].at<double>(0)<<"\n";
      zn_pointth<<Zu_P[0].at<double>(1)<<"\n";
      C_pointx<<camera_point_c[0].x<<"\n";
      C_pointz<<camera_point_c[0].z<<"\n";

    //znL<<"ZNL[0]="<<ZNL[0]<<",ZNL[1]="<<ZNL[1]<<",ZNL[2]="<<ZNL[2]<<",ZNL[3]="<<ZNL[3]<<",ZNL[4]="<<ZNL[4]<<",ZNL[5]="<<ZNL[5]<<",ZNL[6]="<<ZNL[6]<<",ZNL[7]="<<ZNL[7]<<"\n";
    ...
    if(camera_point_c.size()<10||k<10){//特徴点が100個以下になったら再び特徴点を検出する
      reset = true;
    ...
    //if(Des_RobotTH>=-3.141592653/2){//研究室
      ...
      //robot_velocity.linear.x  = 0.1; // 並進速度の初期化
      //robot_velocity.angular.z = 0.0; // 回転速度の初期化
      //robot_velocity.angular.z = -0.2; // 回転速度の初期化
  ...
  if(robot_velocity.angular.z==0){
    ...
    //At00=(1/(robot_velocity.angular.z+10e-5))*(sin(Act_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)-sin(Act_RobotTH));
    //At01=-(Des_RobotV/((robot_velocity.angular.z+10e-5)*(robot_velocity.angular.z+10e-5)))*(sin(Act_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)-sin(Act_RobotTH))
    //    +((Des_RobotV/(robot_velocity.angular.z+10e-5))*realsec*cos(Act_RobotTH+(robot_velocity.angular.z+10e-5)*realsec));
    //At10=(1/(robot_velocity.angular.z+10e-5))*(-cos(Act_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)+cos(Act_RobotTH));
    //At11=-(Des_RobotV/((robot_velocity.angular.z+10e-5)*(robot_velocity.angular.z+10e-5)))*(-cos(Act_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)+cos(Act_RobotTH))
    //    +((Des_RobotV/(robot_velocity.angular.z+10e-5))*realsec*sin(Act_RobotTH+(robot_velocity.angular.z+10e-5)*realsec));
    //At20=0;
    //At21=realsec;
  }
  else{
  ...
  if(robot_velocity.angular.z==0){
    Ft= (cv::Mat_<double>(3,3) << 
      ...
      0, 0, 1);
    //Ft= (cv::Mat_<double>(3,3) << 
    //  1, 0, (Act_RobotV/(robot_velocity.angular.z+10e-5))*(cos(Est_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)-cos(Est_RobotTH)),
    //  0, 1, (Act_RobotV/(robot_velocity.angular.z+10e-5))*(sin(Est_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)-sin(Est_RobotTH)),
    //  0, 0, 1);
    }
  else{
    Ft= (cv::Mat_<double>(3,3) << 
  ...
  if(robot_velocity.angular.z==0){
    ...
    //Est_RobotX=Est_RobotX+((Des_RobotV/(robot_velocity.angular.z+10e-5))*(sin(Est_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)-sin(Est_RobotTH)));
    //Est_RobotY=Est_RobotY+((Des_RobotV/(robot_velocity.angular.z+10e-5))*(-cos(Est_RobotTH+(robot_velocity.angular.z+10e-5)*realsec)+cos(Est_RobotTH)));
    //Est_RobotTH=Est_RobotTH+((robot_velocity.angular.z+10e-5)*realsec);
  }
  else{
  ...
  for(int i=0;i<markerIds.size();i++){
    ...
    kal_kaisu<<kal_new++<<"\n";
    //kal_kaisu<<"カルマン更新回数(マーカー)="<<kal_new++<<"\n";
    est_robotxx<<Est_RobotX<<"\n";
    est_robotyy<<Est_RobotY<<"\n";
    est_robotthth<<Est_RobotTH<<"\n";
  }
  ...
  //更新ステップは特徴点の数だけ更新を行う
  /*for(int i=0;i<depth_point_curr_ok;i++){
    if(LMWL[i][1]<5){//距離のしきい値が5回超えたら更新に使用しない
    std::cout <<"更新ステップ(特徴点)"<< std::endl;
    //観測方程式(信念分布の中心位置から見たLMまでの距離と角度)(理想推定)
    hu = (cv::Mat_<double>(2,1) <<
      sqrt((Est_RobotX-world_point_c[i].z)*(Est_RobotX-world_point_c[i].z) + (Est_RobotY-world_point_c[i].x)*(Est_RobotY-world_point_c[i].x)),
      atan2(world_point_c[i].x-Est_RobotY,world_point_c[i].z-Est_RobotX) - Est_RobotTH);
    hu_pointl<<"Hu_P["<<i<<"].at<double>(0)="<<hu.at<double>(0)<<"\n";
    hu_pointth<<"Hu_P["<<i<<"].at<double>(1)="<<hu.at<double>(1)<<"\n";

    double lu=sqrt((Est_RobotX-world_point_c[i].z)*(Est_RobotX-world_point_c[i].z)+(Est_RobotY-world_point_c[i].x)*(Est_RobotY-world_point_c[i].x));

    //観測方程式のヤコビ行列Ht信念分布の中心と世界座標ランドマーク位置(world_point_c[i].x,world_point_c[i].z)(理想推定)
    Ht= (cv::Mat_<double>(2,3) << //共分散Ht
      (Est_RobotX-world_point_c[i].z)/lu,       (Est_RobotY-world_point_c[i].x)/lu,       0,
      (world_point_c[i].x-Est_RobotY)/(lu*lu),  (Est_RobotX-world_point_c[i].z)/(lu*lu), -1);
    ...
    kal_kaisu<<kal_new++<<"\n";
    //kal_kaisu<<"カルマン更新回数(特徴点)="<<kal_new++<<"\n";
    est_robotxx<<Est_RobotX<<"\n";
    est_robotyy<<Est_RobotY<<"\n";
    est_robotthth<<Est_RobotTH<<"\n";
    }
  ...
  //マーカーTF-------------------------------------------------------------------------------------------
    ...
    maker_pose.position.x = 3.0;//Rvizと画像は座標系が異なるので注意
    maker_pose.position.y = 0;
    maker_pose.position.z = 0.460;
...
//tf(観測特徴点)-------------------------------------------------------------------------------------------------観測マーカー
    std::string target_maker_frame1 = "world_point_c";//cameraとマーカー間のリンク
    geometry_msgs::Pose maker_pose1;

    std::cout << "tf特徴点の世界座標:world_point_c[i]={x="<< world_point_c[0].x <<",y="<<world_point_c[0].y<<",z="<<world_point_c[0].z<<"}"<< std::endl;

    maker_pose1.position.x = world_point_c[0].z;//Rvizと画像は座標系が異なるので注意
    maker_pose1.position.y = world_point_c[0].x;
    maker_pose1.position.z = world_point_c[0].y;
  ...
  if(kaisu>1){
    ...
    //tf(観測特徴点)-------------------------------------------------------------------------------------------------観測マーカー
    std::string target_maker_frame2 = "camera_point_c";//cameraとマーカー間のリンク
    geometry_msgs::Pose maker_pose2;

    std::cout << "tf特徴点の世界座標:camera_point_c[i]={x="<< camera_point_c[0].x <<",y="<<camera_point_c[0].y<<",z="<<camera_point_c[0].z<<"}"<< std::endl;

    maker_pose2.position.x = camera_point_c[0].z;//Rvizと画像は座標系が異なるので注意
    maker_pose2.position.y = camera_point_c[0].x;
    maker_pose2.position.z = camera_point_c[0].y;
    ...
    br_maker2.sendTransform(tf::StampedTransform(maker_transform2, ros::Time::now(), MaptoAct_Robot_frame, target_maker_frame2));

    if (markerIds.size() > 0) {
    //tf(観測特徴点)-------------------------------------------------------------------------------------------------観測マーカー
    std::string target_maker_frame3 = "MC_point";//cameraとマーカー間のリンク
    geometry_msgs::Pose maker_pose3;

    std::cout << "tfマーカーの世界座標:MC_point[markerIds.at(0)]={x="<< MC_point[markerIds.at(0)][0] <<",y="<<MC_point[markerIds.at(0)][1]<<",z="<<MC_point[markerIds.at(0)][2]<<"}"<< std::endl;

    maker_pose3.position.x = MC_point[markerIds.at(0)][2];//Rvizと画像は座標系が異なるので注意
    maker_pose3.position.y = MC_point[markerIds.at(0)][0];
    maker_pose3.position.z = MC_point[markerIds.at(0)][1];
    ...
    br_maker3.sendTransform(tf::StampedTransform(maker_transform3, ros::Time::now(), MaptoAct_Robot_frame, target_maker_frame3));
    }
  }
