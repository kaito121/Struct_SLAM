...
double cropx=15*4;//予測範囲の範囲(template_size*n)
double cropy=15*4;//予測範囲の範囲(template_size*n)
    ...
    if (reset == false) {//初回以降動作
      ...
      for(int i=0;i<depth_point_prev_ok;i++){
        ...
        if(min_val1[i]<max_val1[i]*0.01){//最小値がしきい値以下なら表示
          ...
          if(cropx<=Est_pixel[i].x&&Est_pixel[i].x<=640-cropx&&cropy<=Est_pixel[i].y&&Est_pixel[i].y<=480-cropy){
            ...
            cv::rectangle(img_dst, cv::Rect(Est_pixel[i].x-cropx+min_pt1[i].x, Est_pixel[i].y-cropy+min_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(0, 255, 0), 3);//白枠
            cv::circle(img_dst, cv::Point(Est_pixel[i].x-cropx+min_pt1[i].x+(img_template1.cols/2), Est_pixel[i].y-cropy+min_pt1[i].y+(img_template1.rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
            Matching_pixel_curr[matchC_curr].x=Est_pixel[i].x-cropx+min_pt1[i].x+(img_template1.cols/2);//マッチング中心座標(画像全体座標)
            Matching_pixel_curr[matchC_curr].y=Est_pixel[i].y-cropy+min_pt1[i].y+(img_template1.rows/2);//マッチング中心座標(画像全体座標)
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<EST_MP_ok;i++){
          ...
          std::cout <<"MP_prev_Templ[i].cols="<<MP_prev_Templ[i].cols<<"MP_prev_Templ[i].rows="<<MP_prev_Templ[i].rows<< std::endl;//最初のフレーム
          ...
          if(min_val1[i]<max_val1[i]*0.1){//最小値がしきい値以下なら表示
            ...
            if(cropx<=Est_MP_pixel[i].x&&Est_MP_pixel[i].x<=640-cropx&&cropy<=Est_MP_pixel[i].y&&Est_MP_pixel[i].y<=480-cropy){
              std::cout << "マッチング:全て画面内の時["<<i<<"]"<< std::endl;
              cv::rectangle(img_dst, cv::Rect(Est_MP_pixel[i].x-cropx+min_pt1[i].x, Est_MP_pixel[i].y-cropy+min_pt1[i].y, MP_prev_Templ[i].cols, MP_prev_Templ[i].rows), cv::Scalar(0, 255, 0), 3);//白枠
              cv::circle(img_dst, cv::Point(Est_MP_pixel[i].x-cropx+min_pt1[i].x+(MP_prev_Templ[i].cols/2), Est_MP_pixel[i].y-cropy+min_pt1[i].y+(MP_prev_Templ[i].rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
              Matching_pixel_prev[matchC_prev].x=Est_MP_pixel[i].x-cropx+min_pt1[i].x+(MP_prev_Templ[i].cols/2);//マッチング中心座標(画像全体座標)
              Matching_pixel_prev[matchC_prev].y=Est_MP_pixel[i].y-cropy+min_pt1[i].y+(MP_prev_Templ[i].rows/2);//マッチング中心座標(画像全体座標)
