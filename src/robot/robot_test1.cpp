...
void callback2(const geometry_msgs::Twist::ConstPtr& vel) {
  ...
  if(kaisu2!=0){
    time_t diffsec2 = difftime(startTime2.tv_sec,endTime2.tv_sec);    // 秒数の差分を計算
    suseconds_t diffsub2 = startTime2.tv_usec - endTime2.tv_usec;      // マイクロ秒部分の差分を計算
    realsec2 = diffsec2+diffsub2*1e-6;                          // 実時間を計算
  ...
  des_robot<< "ALLrealsec2=" <<ALLrealsec2<<" ,realsec2=" <<realsec2<<" ,Des_RobotX=" <<Des_RobotX<<",Des_RobotY=" <<Des_RobotY<<",Des_RobotTH=" <<Des_RobotTH<<"\n";
