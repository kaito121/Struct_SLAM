...
double Est_LM_camera[40][3];//マーカーLMの推定カメラ座標
...
ofstream Est_camera("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Est_camera.txt");
  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      if(depth[markerIds.at(i)]>0&&depth[markerIds.at(i)]<10000){
        x = (pixel[markerIds.at(i)][0] - 320) / 615.337;//ここで正規化座標もしてる
        y = (pixel[markerIds.at(i)][1] - 240) / 615.458;
      ...
      //自己位置からマーカーのカメラ座標を推定する
      Est_LM_camera[markerIds.at(i)][0]=-MarkerW[markerIds.at(i)].at<float>(0)*cos(Est_RobotTH)+MarkerW[markerIds.at(i)].at<float>(2)*sin(Est_RobotTH)-(-Est_RobotY*cos(Est_RobotTH)+Est_RobotX*sin(Est_RobotTH));
      Est_LM_camera[markerIds.at(i)][1]=-MarkerW[markerIds.at(i)].at<float>(1);
      Est_LM_camera[markerIds.at(i)][2]= MarkerW[markerIds.at(i)].at<float>(2)*cos(Est_RobotTH)+MarkerW[markerIds.at(i)].at<float>(0)*sin(Est_RobotTH)-(Est_RobotY*sin(Est_RobotTH)+Est_RobotX*cos(Est_RobotTH));

      //Est_LM_camera[markerIds.at(i)][0]=-(MarkerW[markerIds.at(i)].at<float>(0)*cos(Est_RobotTH)+MarkerW[markerIds.at(i)].at<float>(2)*sin(Est_RobotTH)+Est_RobotY);
      //Est_LM_camera[markerIds.at(i)][1]=-MarkerW[markerIds.at(i)].at<float>(1);
      //Est_LM_camera[markerIds.at(i)][2]= MarkerW[markerIds.at(i)].at<float>(2)*cos(Est_RobotTH)-MarkerW[markerIds.at(i)].at<float>(0)*sin(Est_RobotTH)+Est_RobotX;
      //Est_camera<<"MC_point["<<markerIds.at(i)<<"][0]="<<MC_point[markerIds.at(i)][0]<<", MC_point["<<markerIds.at(i)<<"][1]="<<MC_point[markerIds.at(i)][1]<<", MC_point["<<markerIds.at(i)<<"][2]="<<MC_point[markerIds.at(i)][2]<<"\n";
      //Est_camera<<"Est_LM_camera["<<markerIds.at(i)<<"][0]="<<Est_LM_camera[markerIds.at(i)][0]<<", Est_LM_camera["<<markerIds.at(i)<<"][1]="<<Est_LM_camera[markerIds.at(i)][1]<<", Est_LM_camera["<<markerIds.at(i)<<"][2]="<<Est_LM_camera[markerIds.at(i)][2]<<"\n";
      Est_camera<<"MC_point["<<markerIds.at(i)<<"][0]="<<MC_point[markerIds.at(i)][0]<<", MC_point["<<markerIds.at(i)<<"][1]="<<MC_point[markerIds.at(i)][1]<<", MC_point["<<markerIds.at(i)<<"][2]="<<MC_point[markerIds.at(i)][2]<<"             Est_LM_camera["<<markerIds.at(i)<<"][0]="<<Est_LM_camera[markerIds.at(i)][0]<<", Est_LM_camera["<<markerIds.at(i)<<"][1]="<<Est_LM_camera[markerIds.at(i)][1]<<", Est_LM_camera["<<markerIds.at(i)<<"][2]="<<Est_LM_camera[markerIds.at(i)][2]<<"\n";
      //Est_camera<<"Est_LM_camera["<<markerIds.at(i)<<"][0]="<<Est_LM_camera[markerIds.at(i)][0]<<", Est_LM_camera["<<markerIds.at(i)<<"][1]="<<Est_LM_camera[markerIds.at(i)][1]<<", Est_LM_camera["<<markerIds.at(i)<<"][2]="<<Est_LM_camera[markerIds.at(i)][2]<<"\n";
        ...
        if(template_size<points_curr[i].y&&points_curr[i].y<480-template_size){
          if(template_size<points_curr[i].x&&points_curr[i].x<640-template_size){
            ...
            if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
              ...
              camera_point_c[depth_point_curr_ok].y = depth_point_curr[i] * ((points_curr[i].y - 241.696) / 615.458)/1000+0.3;
...
*/
        ...
        //一つ前のマッチング要素がなくマッチング追跡不可能なとき
          ...
          //今のロボットの推定位置を利用して観測した特徴点の世界座標を求める
          MP_curr2_world.resize(1000);//初期設定
          EST<<"\n";
          EST<<"(追跡失敗時)Est_RobotX="<<Est_RobotX<<",Est_RobotY="<<Est_RobotY<<", Est_RobotTH="<<Est_RobotTH<<"\n";

          cv::Mat Kaiten= (cv::Mat_<double>(3,3) <<
            cos(Est_RobotTH),   0,  sin(Est_RobotTH),
            0,                  1,  0,
            -sin(Est_RobotTH),  0 , cos(Est_RobotTH));
          

          for (int i = 0; i < DMP_curr2_ok; i++) {
            cv::Mat Camera_point0=(cv::Mat_<double>(3,1) <<
              -MP_curr2_camera[i].x,
              -MP_curr2_camera[i].y,
              MP_curr2_camera[i].z);

            cv::Mat World_point0;
            World_point0=Kaiten.inv()*Camera_point0;

            //MP_curr2_world[i].x = World_point0.at<double>(0)+Est_RobotY;
            //MP_curr2_world[i].y = World_point0.at<double>(1);
            //MP_curr2_world[i].z = World_point0.at<double>(2)+Est_RobotX;


            MP_curr2_world[i].x = cos(Est_RobotTH)*(-MP_curr2_camera[i].x+Est_RobotY)-sin(Est_RobotTH)*(MP_curr2_camera[i].z+Est_RobotX);
            MP_curr2_world[i].y = -MP_curr2_camera[i].y;
            MP_curr2_world[i].z = cos(Est_RobotTH)*(MP_curr2_camera[i].z+Est_RobotX)+sin(Est_RobotTH)*(-MP_curr2_camera[i].x+Est_RobotY);
            std::cout <<"MP_curr2_world["<<i<<"]="<<MP_curr2_world[i]<< std::endl;
            EST<<"MP_curr2_camera["<<i<<"].x="<<MP_curr2_camera[i].x<<",MP_curr2_camera["<<i<<"].y="<<MP_curr2_camera[i].y<<", MP_curr2_camera["<<i<<"].z="<<MP_curr2_camera[i].z<<"\n";
            EST<<"MP_curr2_world["<<i<<"].x="<<MP_curr2_world[i].x<<",MP_curr2_world["<<i<<"].y="<<MP_curr2_world[i].y<<", MP_curr2_world["<<i<<"].z="<<MP_curr2_world[i].z<<"\n";
          }
    ...
    if(Act_RobotX>0||Act_RobotY>0||Act_RobotTH>0){//ラグの調整
      ...
      EST<<"Est_RobotX="<<Est_RobotX<<",Est_RobotY="<<Est_RobotY<<", Est_RobotTH="<<Est_RobotTH<<"\n";
...
int main(int argc,char **argv){
  ...
  //廊下回転動作実験
  MarkerW[3]= (cv::Mat_<float>(3, 1) <<1.165, 0.28, 3.1);//20211118_V1廊下(直進4.5m,速度0.3,回転90度,回転速度0.2,直進10m,速度0.3)
  MarkerW[4]= (cv::Mat_<float>(3, 1) <<-0.61, 0.28, 4.8);
  MarkerW[5]= (cv::Mat_<float>(3, 1) <<7.57, 0.28, 3.1);
