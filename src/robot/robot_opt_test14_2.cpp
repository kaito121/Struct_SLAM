...
*/
        ...
        if(DMP_prev_ok>0||markerIds.size()>0){//1つ前のマッチング要素が存在するもしくはマーカー観測可能時(マッチング追跡可能時)
          ...
          for (int i = 0; i < DMP_curr2_ok; i++) {//2回目動作分もカルマン更新に使用している気がするがこれで問題ないか調べること
            std::cout <<"MP_curr2_pixel["<<i<<"]="<<MP_curr2_pixel[i]<< std::endl;
        ...
        //新規特徴点検出可能だが一つ前のマッチング要素がなくマッチング追跡不可能なとき(マーカーも観測できない時)
        //この場合は自己位置から新規観測特徴点の世界座標を推定する→カルマン更新には使用しない(2回目動作扱い)
        else if(DMP_curr_ok>0&&DMP_prev_ok<=0&&markerIds.size()<=0){
          for (int i = 0; i < DMP_curr_ok; i++) {
            ...
            //自己位置から新規観測特徴点の世界座標を推定する
            MP_curr_world[i].x=-MP_curr_camera[i].x*cos(Act_RobotTH)-MP_curr_camera[i].z*sin(Act_RobotTH)+Est_RobotY;
            MP_curr_world[i].y=-MP_curr_camera[i].y;
            MP_curr_world[i].z=MP_curr_camera[i].z*cos(Act_RobotTH)-MP_curr_camera[i].x*sin(Act_RobotTH)+Est_RobotX;

            MP_curr2_world[DMP_curr2_ok] = MP_curr_world[i];
            DMP_curr2_ok=DMP_curr2_ok+1;//Depthが取得可能な全マッチテンプレート数
          }
          MP_curr2_pixel.resize(DMP_curr2_ok);//Depth取得可能数でリサイズ(マッチング中心画像座標)
          MP_curr2_camera.resize(DMP_curr2_ok);//Depth取得可能数でリサイズ(マッチング中心カメラ座標)
        ...
        //新規特徴点検出不可能かつ一つ前のマッチング要素がなくマッチング追跡不可能なとき(マーカーも観測できない時)
        else if(DMP_curr_ok<=0&&DMP_prev_ok<=0&&markerIds.size()<=0){
