...
int Last_markerID;//最後に観測したマーカーのIDを保存する
...
//
  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      Last_markerID=markerIds.at(i);//最後に観測したマーカーのIDを保存
    }
...
*/
        ...
        //世界座標推定(マーカー観測不能かつマッチング数0の時)------------------------------------------------------
        //最後に観測したマーカーの世界座標からカメラ座標を推定し、その値を使って特徴点とリンクさせる
        MP_curr_world.resize(1000);//初期設定
        if(markerIds.size() <= 0&&DMP_curr2_ok<=0){
          //自己位置からマーカーのカメラ座標を推定する
          Est_LM_camera[Last_markerID][0]=-MarkerW[Last_markerID].at<float>(0)*cos(Act_RobotTH)+MarkerW[Last_markerID].at<float>(2)*sin(Act_RobotTH)-(-Act_RobotY*cos(Act_RobotTH)+Act_RobotX*sin(Act_RobotTH));
          Est_LM_camera[Last_markerID][1]=-MarkerW[Last_markerID].at<float>(1);
          Est_LM_camera[Last_markerID][2]= MarkerW[Last_markerID].at<float>(2)*cos(Act_RobotTH)+MarkerW[Last_markerID].at<float>(0)*sin(Act_RobotTH)-(Act_RobotY*sin(Act_RobotTH)+Act_RobotX*cos(Act_RobotTH));

          for(int i = 0; i < DMP_curr_ok; i++){
            MP_curr_world[i].x=MarkerW[Last_markerID].at<float>(0)+(-MP_curr_camera[i].x+Est_LM_camera[Last_markerID][0])*cos(-Act_RobotTH)-(MP_curr_camera[i].z-Est_LM_camera[Last_markerID][2])*sin(-Act_RobotTH);
            MP_curr_world[i].y=MarkerW[Last_markerID].at<float>(1)+(-MP_curr_camera[i].y+Est_LM_camera[Last_markerID][1]);
            MP_curr_world[i].z=MarkerW[Last_markerID].at<float>(2)+(MP_curr_camera[i].x-Est_LM_camera[Last_markerID][0])*sin(-Act_RobotTH)+(MP_curr_camera[i].z-Est_LM_camera[Last_markerID][2])*cos(-Act_RobotTH);
          }
          MP_curr_world.resize(DMP_curr_ok);//Depth取得可能数でリサイズ(マッチング中心世界座標)
        }
