...
//  
  ...
  if(time0 != false){
      //オドメトリーの速度からオプティカルフローを求める
      //Xv=(Act_RobotV*sin(Act_RobotTH)/camera_point_c[0].z)-robot_odometry.twist.twist.angular.z+(points_curr[0].x - 324.473)*((Act_RobotV*cos(Act_RobotTH)/camera_point_c[0].z)-(robot_odometry.twist.twist.angular.z*(points_curr[0].x - 324.473)));
      //Yv=(points_curr[0].y - 241.696)*((Act_RobotV*cos(Act_RobotTH)/camera_point_c[0].z)-(robot_odometry.twist.twist.angular.z*(points_curr[0].x - 324.473)));
      //pixelX=(points_curr[0].x - 324.473)+Xv*realsec;
      //pixelY=(points_curr[0].y - 241.696)+Xv*realsec;

      /*for(int i=0;i<depth_point_curr_ok;i++){
        double Vx=Act_RobotV*cos(Act_RobotTH)*realsec;
        double Vy=Act_RobotV*sin(Act_RobotTH)*realsec;
        double PCX=points_curr[i].x - 324.473;
        double PCY=points_curr[i].y - 241.696;
        //オドメトリーの速度からオプティカルフローを求める
        Xv=((-Vy/camera_point_c[i].z)-robot_odometry.twist.twist.angular.z*realsec)-PCX*((-Vx/camera_point_c[i].z)+(robot_odometry.twist.twist.angular.z*realsec*PCX));
        Yv=-PCY*((-Vx/camera_point_c[i].z)+(robot_odometry.twist.twist.angular.z*realsec*PCX));
        pixelX=points_curr[i].x+(Xv*realsec);
        pixelY=points_curr[i].y+(Yv*realsec);
        cv::circle(img_dst, cv::Point(pixelX,pixelY), 6, Scalar(0,255,255), -1, cv::LINE_AA);//一つ前の画像の座標
        cv::line(img_dst,cv::Point(points_curr[i].x,points_curr[i].y),cv::Point(pixelX,pixelY),cv::Scalar(0,0,255), 1, cv::LINE_AA);//180度(180*3)
      }*/

      //ロボットの動きから特徴点の運動復元を行う(特徴点の状態方程式を使用)
      for(int i=0;i<depth_point_curr_ok;i++){
        double Vx=Act_RobotV*cos(Act_RobotTH);
        double Vy=Act_RobotV*sin(Act_RobotTH);
        double PCX=points_curr[i].x - 324.473;
        double PCY=points_curr[i].y - 241.696;

        Est_point[i].x=-camera_point_c[i].x+robot_odometry.twist.twist.angular.z*realsec*camera_point_c[i].z+Act_RobotV*sin(-Act_RobotTH)*realsec;
        Est_point[i].y=-camera_point_c[i].y;
        Est_point[i].z=camera_point_c[i].z-robot_odometry.twist.twist.angular.z*realsec*-camera_point_c[i].x+Act_RobotV*cos(-Act_RobotTH)*realsec;
        ...
        cv::line(img_dst,cv::Point(points_curr[i].x,points_curr[i].y),cv::Point(Est_pixel[i].x,Est_pixel[i].y),cv::Scalar(0,0,255), 1, cv::LINE_AA);//180度(180*3)

        std::cout <<"Est_point["<<i<<"]=\n"<<Est_point[i]<< std::endl;
        std::cout <<"Est_pixel["<<i<<"]=\n"<<Est_pixel[i]<< std::endl;

      } 
        cv::circle(img_dst, cv::Point(points_curr[0].x,points_curr[0].y), 6, Scalar(0,255,0), -1, cv::LINE_AA);//一つ前の画像の座標
