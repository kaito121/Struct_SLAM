...
ofstream MC_camerax("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/MC_camerax.txt");//自己位置推定とマーカーの世界座標の関係確認
ofstream MC_cameray("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/MC_cameray.txt");//自己位置推定とマーカーの世界座標の関係確認
...
ofstream MP_camerax("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/MP_camerax.txt");//自己位置推定と特徴点の世界座標の関係確認
ofstream MP_cameray("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/MP_cameray.txt");//自己位置推定と特徴点の世界座標の関係確認
    ...
    //pub.publish(robot_velocity);    // 速度指令メッセージをパブリッシュ（送信）
     ...
     /*//廊下動作-----------------------------------------------------------------------------------------------------------------
      ...
      if(X_25==false&&TH_90==false&&Y_05==false){
        ...
        robot_velocity.linear.x  = VX+(2e-7*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1-1e-5*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1+0.0004*ALLrealsecV1*ALLrealsecV1-0.0061*ALLrealsecV1+0.0396);
      ...
      if(X_25==true&&TH_90==false&&Y_05==false){
        ...
        robot_velocity.angular.z  =  (THZ-0.1)+(0.0176*ALLrealsecM1+0.11);//(0.5)廊下
      ...
      if(X_25==true&&TH_90==false&&Y_05==false){
        robot_velocity.linear.x  =  0.0;
        robot_velocity.angular.z  = THZ;//(0.5)廊下はこっち
      }
      if(X_25==true&&TH_90==true&&Y_05==false){
        robot_velocity.linear.x  = VX;//(0.1)
        robot_velocity.angular.z = 0.0; // 回転速度の初期化}//xが1以上になったら終了
  ...
  if(time0 != false){
    if (reset == false) {//初回以降動作
      ...
      for(int i=0;i<depth_point_prev_ok;i++){
        ...
        //Est_point[i].x=camera_point_p[i].x*cos(robot_odometry.twist.twist.angular.z)*realsec+camera_point_p[i].z*sin(robot_odometry.twist.twist.angular.z)*realsec-Act_RobotV*sin(-Act_RobotTH)*realsec;
        //Est_point[i].y=camera_point_p[i].y;
        //Est_point[i].z=camera_point_p[i].x*-sin(robot_odometry.twist.twist.angular.z)*realsec+camera_point_p[i].z*cos(robot_odometry.twist.twist.angular.z)*realsec-Act_RobotV*cos(-Act_RobotTH)*realsec;
      ...
      if(markerIds.size() > 0){
        ...
        for (int i = 0; i < DMP_curr_ok; i++) {
          ...
          //MP_curr_world[i].x=MarkerW[markerIds.at(minMC)].at<float>(0)+(MP_curr_camera[i].x-MC_point[markerIds.at(minMC)][0])*cos(-Act_RobotTH)-(MP_curr_camera[i].z-MC_point[markerIds.at(minMC)][2])*sin(-Act_RobotTH);
          //MP_curr_world[i].y=MarkerW[markerIds.at(minMC)].at<float>(1)+(MP_curr_camera[i].y-MC_point[markerIds.at(minMC)][1]);
          //MP_curr_world[i].z=MarkerW[markerIds.at(minMC)].at<float>(2)+(MP_curr_camera[i].x-MC_point[markerIds.at(minMC)][0])*sin(-Act_RobotTH)+(MP_curr_camera[i].z-MC_point[markerIds.at(minMC)][2])*cos(-Act_RobotTH);
          ...
          MP_kaisu<<ALLrealsec<<"\n";
          MP_worldy<<MP_curr_world[i].x<<"\n";
          MP_worldx<<MP_curr_world[i].z<<"\n";
          MP_cameray<<MP_curr_camera[i].x<<"\n";
          MP_camerax<<MP_curr_camera[i].z<<"\n";
          ...
          //世界座標推定に使用する観測データ（カメラ座標系は画像投影可能かどうかを見る）
          double MP_pixelX[20],MP_pixelY[20],MC_pixelX[20],MC_pixelY[20];
          double MPW_cameraX[20],MPW_cameraY[20],MPW_cameraZ[20],MPW_pixelX[20],MPW_pixelY[20];
          double MCW_cameraX[20],MCW_cameraY[20],MCW_cameraZ[20],MCW_pixelX[20],MCW_pixelY[20];
          MP_pixelX[i]=615.337*(MP_curr_camera[i].x/MP_curr_camera[i].z)+324.473;
          MP_pixelY[i]=615.458*(MP_curr_camera[i].y/MP_curr_camera[i].z)+241.696;
          MC_pixelX[markerIds.at(minMC)]=615.337*(MC_point[markerIds.at(minMC)][0]/MC_point[markerIds.at(minMC)][2])+324.473;
          MC_pixelY[markerIds.at(minMC)]=615.458*(MC_point[markerIds.at(minMC)][1]/MC_point[markerIds.at(minMC)][2])+241.696;

          MPW_cameraX[i]=MP_curr_world[i].x*cos(Est_RobotTH)+MP_curr_world[i].z*sin(Est_RobotTH)-(Est_RobotY*cos(Est_RobotTH)+Est_RobotX*sin(Est_RobotTH));
          MPW_cameraY[i]=MP_curr_world[i].y;
          MPW_cameraZ[i]=-MP_curr_world[i].x*sin(Est_RobotTH)+MP_curr_world[i].z*cos(Est_RobotTH)-(-Est_RobotY*sin(Est_RobotTH)+Est_RobotX*cos(Est_RobotTH));
          MPW_pixelX[i]=615.337*(MPW_cameraX[i]/MPW_cameraZ[i])+324.473;
          MPW_pixelY[i]=615.458*(MPW_cameraY[i]/MPW_cameraZ[i])+241.696;

          MCW_cameraX[markerIds.at(minMC)]=MarkerW[markerIds.at(minMC)].at<float>(1)*cos(Est_RobotTH)+MarkerW[markerIds.at(minMC)].at<float>(2)*sin(Est_RobotTH)-(Est_RobotY*cos(Est_RobotTH)+Est_RobotX*sin(Est_RobotTH));
          MCW_cameraY[markerIds.at(minMC)]=MarkerW[markerIds.at(minMC)].at<float>(0);
          MCW_cameraZ[markerIds.at(minMC)]=-MarkerW[markerIds.at(minMC)].at<float>(1)*sin(Est_RobotTH)+MarkerW[markerIds.at(minMC)].at<float>(2)*cos(Est_RobotTH)-(-Est_RobotY*sin(Est_RobotTH)+Est_RobotX*cos(Est_RobotTH));
          MCW_pixelX[markerIds.at(minMC)]=615.337*(MCW_cameraX[markerIds.at(minMC)]/MCW_cameraZ[markerIds.at(minMC)])+324.473;
          MCW_pixelY[markerIds.at(minMC)]=615.458*(MCW_cameraY[markerIds.at(minMC)]/MCW_cameraZ[markerIds.at(minMC)])+241.696;

          W_point<<"MP_pixel["<<i<<"]=("<<MP_pixelX[i]<<", "<<MP_pixelY[i]<<")  ";
          W_point<<"MPW_pixel["<<i<<"]=("<<MPW_pixelX[i]<<", "<<MPW_pixelY[i]<<")  ";
          W_point<<"MC_pixel["<<markerIds.at(minMC)<<"]= ("<<MC_pixelX[markerIds.at(minMC)]<<", "<<MC_pixelY[markerIds.at(minMC)]<<")  ";
          W_point<<"MCW_pixel["<<markerIds.at(minMC)<<"]= ("<<MCW_pixelX[markerIds.at(minMC)]<<", "<<MCW_pixelY[markerIds.at(minMC)]<<")\n";
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<DMP_prev_ok;i++){
          ...
          //Est_MP_point[i].x=MP_prev_camera[i].x*cos(robot_odometry.twist.twist.angular.z)*realsec+MP_prev_camera[i].z*sin(robot_odometry.twist.twist.angular.z)*realsec-Act_RobotV*sin(Act_RobotTH)*realsec;
          //Est_MP_point[i].y=MP_prev_camera[i].y;
          //Est_MP_point[i].z=MP_prev_camera[i].x*-sin(robot_odometry.twist.twist.angular.z)*realsec+MP_prev_camera[i].z*cos(robot_odometry.twist.twist.angular.z)*realsec-Act_RobotV*cos(Act_RobotTH)*realsec;
...
*/
        ...
        if(markerIds.size() <= 0){
          ...
          for (int i = 0; i < DMP_curr_ok; i++) {
            ...
            //MP_curr_world[i].x=MP_curr2_world[minMC].x+(MP_curr_camera[i].x-MP_curr2_camera[minMC].x)*cos(-Act_RobotTH)-(MP_curr_camera[i].z-MP_curr2_camera[minMC].z)*sin(-Act_RobotTH);
            //MP_curr_world[i].y=MP_curr2_world[minMC].y+(MP_curr_camera[i].y-MP_curr2_camera[minMC].y);
            //MP_curr_world[i].z=MP_curr2_world[minMC].z+(MP_curr_camera[i].x-MP_curr2_camera[minMC].x)*sin(-Act_RobotTH)+(MP_curr_camera[i].z-MP_curr2_camera[minMC].z)*cos(-Act_RobotTH);
            ...
            MP_worldy<<MP_curr_world[i].x<<"\n";
            MP_worldx<<MP_curr_world[i].z<<"\n";
            MP_cameray<<MP_curr2_camera[i].x<<"\n";
            MP_camerax<<MP_curr2_camera[i].z<<"\n";
            MC_worldy<<"\n";
            MC_worldx<<"\n";
            MC_cameray<<"\n";
            MC_camerax<<"\n";
            ...
            //世界座標推定に使用する観測データ（カメラ座標系は画像投影可能かどうかを見る）
            double MP_pixelX[20],MP_pixelY[20],MP2_pixelX[20],MP2_pixelY[20];
            double MPW_cameraX[20],MPW_cameraY[20],MPW_cameraZ[20],MPW_pixelX[20],MPW_pixelY[20];
            double MP2W_cameraX[20],MP2W_cameraY[20],MP2W_cameraZ[20],MP2W_pixelX[20],MP2W_pixelY[20];
            MP_pixelX[i]=615.337*(MP_curr_camera[i].x/MP_curr_camera[i].z)+324.473;
            MP_pixelY[i]=615.458*(MP_curr_camera[i].y/MP_curr_camera[i].z)+241.696;
            MP2_pixelX[minMC]=615.337*(MP_curr2_camera[minMC].x/MP_curr2_camera[minMC].z)+324.473;
            MP2_pixelY[minMC]=615.458*(MP_curr2_camera[minMC].y/MP_curr2_camera[minMC].z)+241.696;
            ...
            MP2W_cameraX[minMC]=MP_curr2_world[minMC].x*cos(Est_RobotTH)+MP_curr2_world[minMC].z*sin(Est_RobotTH)-(Est_RobotY*cos(Est_RobotTH)+Est_RobotX*sin(Est_RobotTH));
            MP2W_cameraY[minMC]=MP_curr2_world[minMC].y;
            MP2W_cameraZ[minMC]=-MP_curr2_world[minMC].x*sin(Est_RobotTH)+MP_curr2_world[minMC].z*cos(Est_RobotTH)-(-Est_RobotY*sin(Est_RobotTH)+Est_RobotX*cos(Est_RobotTH));
            MP2W_pixelX[minMC]=615.337*(MP2W_cameraX[minMC]/MP2W_cameraZ[minMC])+324.473;
            MP2W_pixelY[minMC]=615.458*(MP2W_cameraY[minMC]/MP2W_cameraZ[minMC])+241.696;

            W_point<<"MP_pixel["<<i<<"]=("<<MP_pixelX[i]<<", "<<MP_pixelY[i]<<")  ";
            W_point<<"MPW_pixel["<<i<<"]=("<<MPW_pixelX[i]<<", "<<MPW_pixelY[i]<<")  ";

            W_point<<"MP2_pixel["<<minMC<<"]= ("<<MP2_pixelX[minMC]<<", "<<MP2_pixelY[minMC]<<")  ";
            W_point<<"MP2W_pixel["<<minMC<<"]= ("<<MP2W_pixelX[minMC]<<", "<<MP2W_pixelY[minMC]<<")\n";
          ...
          W_point2 <<"いままでの推定結果を利用して世界座標を推定する\n";

          for (int i = 0; i < DMP_curr_ok; i++) {
            //MP_curr_world2[i].x=-MP_curr_camera[i].x+Act_RobotTH*(MP_curr_camera[i].z+Act_RobotX)-Act_RobotY;
            //MP_curr_world2[i].y=-MP_curr_camera[i].y;
            //MP_curr_world2[i].z=MP_curr_camera[i].z-Act_RobotTH*(-MP_curr_camera[i].x-Act_RobotY)+Act_RobotX;

            MP_curr_world2[i].x=-MP_curr_camera[i].x+Act_RobotTH*(MP_curr_camera[i].z+Act_RobotX)-Act_RobotY;
            MP_curr_world2[i].y=-MP_curr_camera[i].y;
            MP_curr_world2[i].z=MP_curr_camera[i].z-Act_RobotTH*(-MP_curr_camera[i].x-Act_RobotY)+Act_RobotX;
            std::cout <<"新規テンプレートの世界座標:MP_curr_world2["<<i<<"]="<<MP_curr_world2[i]<< std::endl;
            W_point2 <<"新規テンプレートの世界座標:MP_curr_world2["<<i<<"]="<<MP_curr_world2[i]<<"\n";

            //MP_curr_world[i].x=(MP_curr_world[i].x+MP_curr_world2[i].x)/2;
    ...
    if(Act_RobotX>0||Act_RobotY>0||Act_RobotTH>0){//ラグの調整
      ...
      for(int i=0;i<markerIds.size();i++){
        ...
        std::cout <<"誤差共分散Cov="<<Cov<< std::endl;
        ...
        std::cout <<"Zu[markerIds.at(i)]=\n"<<Zu[markerIds.at(i)]<< std::endl;
        std::cout <<"hu=\n"<<hu<< std::endl;
      ...
      for(int i=0;i<DMP_curr2_ok;i++){
        ...
        //kal_kaisu<<kal_new++<<"\n";
        //kal_kaisu<<"カルマン��新回数(特徴点)="<<kal_new++<<"\n";//カルマン更新動作確認用
        //est_robotxx<<Est_RobotX<<"\n";//カルマン更新動作確認用
        //est_robotyy<<Est_RobotY<<"\n";
        //est_robotthth<<Est_RobotTH<<"\n";

        kansoku_Zu1<<Zu_P[i].at<double>(0,0)<<"\n";
        kansoku_Zu2<<Zu_P[i].at<double>(1,0)<<"\n";
        kansoku_hu1<<hu.at<double>(0,0)<<"\n";
        kansoku_hu2<<hu.at<double>(1,0)<<"\n";
  ...
  //経路描写-------------------------------------------------------------
    ...
    if(Tracking==false){ 
      ...
      cv::swap(MP_curr_world, MP_prev_world);//マッチ中心カメラ座標
    ...
    //3回目以降動作時
      ...
      cv::swap(MP_curr2_world, MP_prev_world);//マッチ中心カメラ座標
...
int main(int argc,char **argv){
  ...
  //20211030研究室(直進2.7m,回転-3.141592653/2.4,速度0.1,角速度-(0.2+(0.0176*ALLrealsecM1+0.11)))
  //MarkerW[3]= (cv::Mat_<float>(3, 1) <<0.0, 0.28, 3.0);
  //MarkerW[4]= (cv::Mat_<float>(3, 1) <<-2.3, 0.28, 2.74);
  //MarkerW[8]= (cv::Mat_<float>(3, 1) <<0.62, 0.73, 2.60);
  //LX=2.7,VX=0.1,omegaZ=2.4,THZ=0.3;
  ...
  //MarkerW[3]= (cv::Mat_<float>(3, 1) <<1.165, 0.28, 3.1);//20211120廊下(直進2.0m,速度0.3,回転90度,回転速度0.2,直進10m,速度0.3)
  ...
  //20211120V2廊下(直進2.0m,速度0.25,回転π/2度,回転速度0.15+α,直進8m,速度0.25)廊下回転動作実験
