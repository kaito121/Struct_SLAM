...
int act_v=0,act_omega=0;//ロボット速度制御用
...
ofstream sec_M1("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date_odom/sec_M1.txt");
ofstream sec_V1("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date_odom/sec_V1.txt");
...
//ロボット指令-------------------------------------------------------------------
    ...
    /*//回転動作実験動作-----------------------------------------------------------------------------------------------------------------
      ...
      if(X_25==false&&TH_90==false&&Y_05==false){
        ...
        robot_velocity.angular.z  =  THZ+(0.0006*ALLrealsecM1*ALLrealsecM1-0.0285*ALLrealsecM1+0.2553);//角速度修正
        pub.publish(robot_velocity);    // 速度指令メッセージをパブリッシュ（送信）
        robot_velocity.linear.x  =  0.0;
        robot_velocity.angular.z  =  THZ;
     ...
     //廊下動作1-----------------------------------------------------------------------------------------------------------------
      ...
      if(X_25==false&&TH_90==false&&Y_05==false){
        ...
        if(Act_RobotV<=VX+0.1&&act_v<5){
          robot_velocity.linear.x  = VX+(5e-7*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1-1e-5*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1+0.0004*ALLrealsecV1*ALLrealsecV1-0.0061*ALLrealsecV1+0.0396);
        }
        else{
          robot_velocity.linear.x  = VX;
          act_v=act_v+1;
        }
        robot_velocity.angular.z=0.02+(0-robot_odometry.twist.twist.angular.z)*1;
        //if(robot_odometry.twist.twist.angular.z<0){
        //  robot_velocity.angular.z = 0.02;// 回転速度の初期化}//xが1以上になったら終了
        //} 
        //else{robot_velocity.angular.z = 0.0;}
        ...
        robot_velocity.linear.x  =  0.0;
        //robot_velocity.angular.z  =  THZ+(0.002*ALLrealsecM1*ALLrealsecM1-0.01*ALLrealsecM1+0.2553)+0.02;//角速度修正
        //robot_velocity.angular.z  =  THZ+(0.0006*ALLrealsecM1*ALLrealsecM1-0.0285*ALLrealsecM1+0.2553)+0.02;//角速度修正
        robot_velocity.angular.z  =  THZ+(0.002*ALLrealsecM1*ALLrealsecM1-0.02*ALLrealsecM1+0.2553)+0.02;//角速度修正
        pub.publish(robot_velocity);    // 速度指令メッセージをパブリッシュ（送信）
        robot_velocity.linear.x  =  0.0;
        robot_velocity.angular.z  =  THZ;
        ...
        //} 
        //else{robot_velocity.angular.z = 0.0;}
        robot_velocity.angular.z=0.02+(0-robot_odometry.twist.twist.angular.z)*1.5;
        pub.publish(robot_velocity);    // 速度指令メッセージをパブリッシュ（送信）
        if(Act_RobotV<=0.1){robot_velocity.linear.x=0;}
        else{robot_velocity.linear.x  = VX;} // 並進速度vの初期化
        robot_velocity.angular.z = 0.0; // 回転速度ωの初期化}//xが1以上になったら終了
      ...
      if(X_25==false&&TH_90==false&&Y_05==false){
        ...
        if(Act_RobotV<=VX+0.1&&act_v<5){//指令値以上に観測値が大きくなった場合は、修正してない指令値を使用する(4回指令値以上になった場合変更する)
          robot_velocity.linear.x  = VX+(5e-7*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1-1e-5*ALLrealsecV1*ALLrealsecV1*ALLrealsecV1+0.0004*ALLrealsecV1*ALLrealsecV1-0.0061*ALLrealsecV1+0.0396);
        }
        else{
          robot_velocity.linear.x  = VX;
        ...
        //右側のモータのトルク不足を考慮
          if(robot_odometry.twist.twist.angular.z<0){robot_velocity.angular.z = 0.02;} 
          else{robot_velocity.angular.z = 0.0;}

        pub.publish(robot_velocity);    // 速度指令メッセージをパブリッシュ（送信）
        if(Act_RobotV<=0.1){robot_velocity.linear.x=0;}//観測値が動き出してから指令したことにしている(動作遅れを考慮)
        else{robot_velocity.linear.x  = VX;} // 並進速度vの初期化
        robot_velocity.angular.z = 0.0; // 回転速度ωの初期化}//xが1以上になったら終了
        if(Des_RobotX>=LX){
          X_25=true;
        ...
        robot_velocity.linear.x  =  0.0;
        if(robot_odometry.twist.twist.angular.z<=THZ+0.02&&act_omega<5){//指令値以上に観測値が大きくなった場合は、修正してない指令値を使用する(4回指令値以上になった場合変更する)
          robot_velocity.angular.z  = THZ+(0.002*ALLrealsecM1*ALLrealsecM1-0.02*ALLrealsecM1+0.2553)+0.02;//角速度修正
        }
        else{
          robot_velocity.angular.z  = THZ+0.02;
          act_omega=act_omega+1;
        }

        pub.publish(robot_velocity);    // 速度指令メッセージをパブリッシュ（送信）
        robot_velocity.linear.x  =  0.0;
        if(robot_odometry.twist.twist.angular.z<=0.1){robot_velocity.angular.z=0;}//観測値が動き出してから指令したことにしている(動作遅れを考慮)
        else{robot_velocity.angular.z  = THZ;} // 並進速度vの初期化
