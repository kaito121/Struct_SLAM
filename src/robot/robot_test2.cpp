//確率ロボティクスのカルマンフィルタ自己位置プログラムのTurtlbot実装
//20211016 あとセンサーとロボットの誤差共分散とセンサーのランドマーク観測情報が必要
...
void odomcallback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  if(robot_odometry.twist.twist.angular.z==0){
    Act_RobotX=Act_RobotX+(Act_RobotV*cos(Act_RobotTH)*realsec2);
    Act_RobotY=Act_RobotY+(Act_RobotV*sin(Act_RobotTH)*realsec2);
    Act_RobotTH=Act_RobotTH+(robot_odometry.twist.twist.angular.z*realsec2);
  }
  else{
    Act_RobotX=Act_RobotX+((Act_RobotV/robot_odometry.twist.twist.angular.z)*(sin(Act_RobotTH+robot_odometry.twist.twist.angular.z*realsec2)-sin(Act_RobotTH)));
    Act_RobotY=Act_RobotY+((Act_RobotV/robot_odometry.twist.twist.angular.z)*(-cos(Act_RobotTH+robot_odometry.twist.twist.angular.z*realsec2)+cos(Act_RobotTH)));
    Act_RobotTH=Act_RobotTH+(robot_odometry.twist.twist.angular.z*realsec2);
  }
  ...
  if(robot_velocity.angular.z==0){
    Des_RobotX=Des_RobotX+(Des_RobotV*cos(Des_RobotTH)*realsec2);
    Des_RobotY=Des_RobotY+(Des_RobotV*sin(Des_RobotTH)*realsec2);
    Des_RobotTH=Des_RobotTH+(robot_velocity.angular.z*realsec2);
  }
  else{
    Des_RobotX=Des_RobotX+((Des_RobotV/robot_velocity.angular.z)*(sin(Des_RobotTH+robot_velocity.angular.z*realsec2)-sin(Des_RobotTH)));
    Des_RobotY=Des_RobotY+((Des_RobotV/robot_velocity.angular.z)*(-cos(Des_RobotTH+robot_velocity.angular.z*realsec2)+cos(Des_RobotTH)));
    Des_RobotTH=Des_RobotTH+(robot_velocity.angular.z*realsec2);
  }
  ...
  if(robot_velocity.angular.z==0){
    At00=cos(Act_RobotTH)*realsec2;
    At01=0;
    At10=sin(Act_RobotTH)*realsec2;
    At11=0;
    At20=0;
    At21=realsec2;
  }
  else{
    At00=(1/robot_velocity.angular.z)*(sin(Act_RobotTH+robot_velocity.angular.z*realsec2)-sin(Act_RobotTH));
    At01=-(Des_RobotV/(robot_velocity.angular.z*robot_velocity.angular.z))*(sin(Act_RobotTH+robot_velocity.angular.z*realsec2)-sin(Act_RobotTH))
        +((Des_RobotV/robot_velocity.angular.z)*realsec2*cos(Act_RobotTH+robot_velocity.angular.z*realsec2));
    At10=(1/robot_velocity.angular.z)*(-cos(Act_RobotTH+robot_velocity.angular.z*realsec2)+cos(Act_RobotTH));
    At11=-(Des_RobotV/(robot_velocity.angular.z*robot_velocity.angular.z))*(-cos(Act_RobotTH+robot_velocity.angular.z*realsec2)+cos(Act_RobotTH))
        +((Des_RobotV/robot_velocity.angular.z)*realsec2*sin(Act_RobotTH+robot_velocity.angular.z*realsec2));
    At20=0;
    At21=realsec2;
  }
  ...
  if(robot_velocity.angular.z==0){
    Ft= (cv::Mat_<double>(3,3) << 
      1, 0, -Act_RobotV*sin(Est_RobotTH)*realsec2,
      0, 1, Act_RobotV*cos(Est_RobotTH)*realsec2,
      0, 0, 1);
    }
  else{
    Ft= (cv::Mat_<double>(3,3) << 
      1, 0, (Act_RobotV/robot_velocity.angular.z)*(cos(Est_RobotTH+robot_velocity.angular.z*realsec2)-cos(Est_RobotTH)),
      0, 1, (Act_RobotV/robot_velocity.angular.z)*(sin(Est_RobotTH+robot_velocity.angular.z*realsec2)-sin(Est_RobotTH)),
      0, 0, 1);
  }
  ...
  if(robot_velocity.angular.z==0){
    Est_RobotX=Est_RobotX+(Des_RobotV*cos(Est_RobotTH)*realsec2);
    Est_RobotY=Est_RobotY+(Des_RobotV*sin(Est_RobotTH)*realsec2);
    Est_RobotTH=Est_RobotTH+(robot_velocity.angular.z*realsec2);
  }
  else{
    Est_RobotX=Est_RobotX+((Des_RobotV/robot_velocity.angular.z)*(sin(Est_RobotTH+robot_velocity.angular.z*realsec2)-sin(Est_RobotTH)));
    Est_RobotY=Est_RobotY+((Des_RobotV/robot_velocity.angular.z)*(-cos(Est_RobotTH+robot_velocity.angular.z*realsec2)+cos(Est_RobotTH)));
    Est_RobotTH=Est_RobotTH+(robot_velocity.angular.z*realsec2);
  }

    
  //if(robot_velocity.angular.z==0){
  //  Est_RobotX=Est_RobotX+(Est_RobotV*cos(Est_RobotTH)*realsec2);
  //  Est_RobotY=Est_RobotY+(Est_RobotV*sin(Est_RobotTH)*realsec2);
  //  Est_RobotTH=Est_RobotTH+(robot_velocity.angular.z*realsec2);
  //}
  //else{
  //  Est_RobotX=Est_RobotX+((Est_RobotV/robot_velocity.angular.z)*(sin(Est_RobotTH+robot_velocity.angular.z*realsec2)-sin(Est_RobotTH)));
  //  Est_RobotY=Est_RobotY+((Est_RobotV/robot_velocity.angular.z)*(-cos(Est_RobotTH+robot_velocity.angular.z*realsec2)+cos(Est_RobotTH)));
  //  Est_RobotTH=Est_RobotTH+(robot_velocity.angular.z*realsec2);
  //}
  ...
  hu = (cv::Mat_<double>(2,1) <<
    sqrt((Est_RobotX-LM[0])*(Est_RobotX-LM[0]) + (Est_RobotY-LM[1])*(Est_RobotY-LM[1])),
    atan2(LM[1]-Est_RobotY,LM[0]-Est_RobotX) - Est_RobotTH);

  double lu=sqrt((Est_RobotX-LM[0])*(Est_RobotX-LM[0])+(Est_RobotY-LM[1])*(Est_RobotY-LM[1]));

  //観測方程式のヤコビ行列Ht信念分布の中心と世界座標ランドマーク位置(LM[0],LM[1])(理想推定)
  Ht= (cv::Mat_<double>(2,3) << //共分散Ht
    (Est_RobotX-LM[0])/lu,       (Est_RobotY-LM[1])/lu,       0,
    (LM[1]-Est_RobotY)/(lu*lu),  (Est_RobotX-LM[0])/(lu*lu), -1);

  //センサーの誤差共分散(直進と角度の誤差共分散を代入)
  Qt= (cv::Mat_<double>(2,2) <<
    (lu*siguma)*(lu*siguma),  0,
    0,            siguma*siguma);
