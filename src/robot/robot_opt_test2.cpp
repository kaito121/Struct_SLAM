...
cv::Mat EST_temp[100];//特徴点周囲の切り取った画像
...
int template_size;
cv::Point min_pt1[10], max_pt1[10];//テンプレートマッチング用変数
double min_val1[10], max_val1[10];
...
//  
  ...
  if (reset == true) {
    //if(markerIds.size() > 0){
        ...
        if(template_size<points_curr[i].y&&points_curr[i].y<480-template_size){
          if(template_size<points_curr[i].x&&points_curr[i].x<640-template_size){
            ...
            if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
              ...
              FeaturePoint[depth_point_curr_ok] = image(roi2); // 切り出し画像
      ...
      Template_Points_Prev.resize(depth_point_curr_ok);//Depth取得可能数でリサイズ(運動復元画像座標)
  ...
  /*else{
    ...
    for(int i=0;i<depth_point_curr_ok;i++){
      ...
      //ZNL[i]=sqrt((camera_point_c[i].x-MC_point[markerIds.at(0)][0])*(camera_point_c[i].x-MC_point[markerIds.at(0)][0])+(camera_point_c[i].z-MC_point[markerIds.at(0)][2])*(camera_point_c[i].z-MC_point[markerIds.at(0)][2]));
    }
  ...
  if(time0 != false){
    ...
    if (reset == false) {
      ...
      for(int i=0;i<depth_point_curr_ok;i++){
        ...
        cv::line(img_dst,cv::Point(points_prev[i].x,points_prev[i].y),cv::Point(Est_pixel[i].x,Est_pixel[i].y),cv::Scalar(0,0,255), 1, cv::LINE_AA);//180度(180*3)

        std::cout <<"Est_point["<<i<<"]=\n"<<Est_point[i]<< std::endl;
        std::cout <<"Est_pixel["<<i<<"]=\n"<<Est_pixel[i]<< std::endl;
        ...
        if(coix-template_size*2>=0&&coiy-template_size*2>0&&coix+template_size*2<=640&&coiy+template_size*2<=480){
          ...
          EST_temp[i] = image(roiEST); // 切り出し画像
          cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲(赤枠)
          cv::imshow("EST_temp", EST_temp[i]);//黄色の特徴点を中心としたクロップ画像
        }
        //yがはみ出る(xははみ出ない)
        ...
        std::cout <<"FeaturePoint[i].cols="<<FeaturePoint[i].cols<<"FeaturePoint[i].rows="<<FeaturePoint[i].rows<< std::endl;//最初のフレーム
        std::cout <<"EST_temp[i].cols="<<EST_temp[i].cols<<"EST_temp[i].rows="<<EST_temp[i].rows<< std::endl;//最初のフレーム

        //予測範囲に対しテンプレートマッチングを行う
        std::cout << "テンプレートマッチングプログラム["<<i<<"]"<< std::endl;
        FeaturePoint[i].copyTo(img_template1); // 切り出し画像
        cv::imshow("win_FeaturePoint", FeaturePoint[i]);//黄色の特徴点を中心としたクロップ画像

        cv::Mat img_minmax1;
        // テンプレートマッチング
        cv::matchTemplate(EST_temp[i], img_template1, img_minmax1, cv::TM_SQDIFF);//差分二乗和
        ...
        if(min_val1[i]<max_val1[i]*0.05){//最小値がしきい値以下なら表示
          ...
          if(coix-template_size*2>=0&&coiy-template_size*2>0&&coix+template_size*2<=640&&coiy+template_size*2<=480){
            ...
            Template_Points_Prev[i].x=Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2);
            Template_Points_Prev[i].y=Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2);
      ...
      //マッチング中心座標の個数がしきい値以下なら再び特徴点検出
      if(templP<3){
        reset = true;
        std::cout <<" 特徴点再検出リセット"<<std::endl;
      }
      cv::circle(img_dst, cv::Point(points_prev[0].x,points_prev[0].y), 6, Scalar(0,255,0), -1, cv::LINE_AA);//一つ前の画像の座標
      cv::circle(img_dst, cv::Point(Est_pixel[0].x,Est_pixel[0].y), 6, Scalar(255,255,255), -1, cv::LINE_AA);//一つ前の画像の座標
    }
