...
struct timeval startTime0, endTime0;  // 構造体宣言
float realsec,realsec0;//サンプリング時間（C++)
float ALLrealsec,ALLrealsec0;//サンプリング時間（C++)
...
ofstream camera_l("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/camera_l3.txt");
ofstream camera_th("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/camera_th3.txt");
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg, const sensor_msgs::CameraInfo::ConstPtr& cam_info)
{
  ...
  gettimeofday(&startTime0, NULL);// 開始時刻取得
  if(kaisu0!=0){
    time_t diffsec0 = difftime(startTime0.tv_sec, endTime0.tv_sec);    // 秒数の差分を計算
    suseconds_t diffsub0 = startTime0.tv_usec - endTime0.tv_usec;      // マイクロ秒部分の差分を計算
    realsec0 = diffsec0+diffsub0*1e-6;// 実時間を計算
    ALLrealsec0=ALLrealsec0+realsec0;                          
    printf("処理の時間=%f\n", realsec0);
    printf("合計時間=%f\n", ALLrealsec0);
  }
...
//  
  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      camera_l<<CameraLM[4][0] <<"\n";
      camera_th<<CameraLM[4][1] <<"\n";
