...
vector<cv::Point2f> MP_curr2_pixel;//マッチテンプレート座標の画像座標系
vector<cv::Point3f> MP_curr2_camera;//マッチテンプレート座標のカメラ座標系
  ...
  if(time0 != false){
    if (reset == false) {//初回以降動作
      ...
      if(Tracking == true){
        ...
        //複数重なる場合は最後
        for (int i = 0; i < DMP_curr2_ok; i++) {
          double remave=0,minlenght=1000000;
          int minj;
          std::cout <<"\n"<< std::endl;
        ...
        /*for (int i = 0; i < DMP_curr_ok; i++) {
          ...
          for (int j = 0; j < DMP_curr2_ok; j++) {
            length=sqrt((MP_curr_pixel[i].x-MP_curr2_pixel[j].x)*(MP_curr_pixel[i].x-MP_curr2_pixel[j].x)
                        +(MP_curr_pixel[i].y-MP_curr2_pixel[j].y)*(MP_curr_pixel[i].y-MP_curr2_pixel[j].y));

            std::cout <<"length="<<length<< std::endl;
            std::cout <<"template_size="<<template_size<< std::endl;
            std::cout <<"更新前:MP_curr_pixel["<<i<<"]="<<MP_curr_pixel[i]<< std::endl;
            std::cout <<"更新前:MP_curr2_pixel["<<j<<"]="<<MP_curr2_pixel[j]<< std::endl;
            
            //更新動作(テンプレートのかぶりがあるとき)(距離がテンプレートサイズより近い(prevをcurrで上書きする)))
            if(length<template_size*2){
              std::cout <<"更新動作------------------------------------------------------------------------"<<std::endl;
              MP_curr2_Templ[j] = MP_curr_Templ[i];//Depth取得可能なマッチテンプレート
              MP_curr2_pixel[j] = MP_curr_pixel[i];//Depth取得可能なマッチング中心画像座標
              MP_curr2_camera[j] = MP_curr_camera[i];
              remave=remave+1;
              std::cout <<"更新後:MP_curr_pixel["<<i<<"]="<<MP_curr_pixel[i]<< std::endl;
              std::cout <<"更新後:MP_curr2_pixel["<<j<<"]="<<MP_curr2_pixel[j]<< std::endl;
            }
          }
          //追加動作(テンプレートのかぶりが無いとき)
          if(remave==0){
            MP_curr2_Templ[DMP_curr2_ok] = MP_curr_Templ[i];//Depth取得可能なマッチテンプレート
            MP_curr2_pixel[DMP_curr2_ok] = MP_curr_pixel[i];//Depth取得可能なマッチング中心画像座標
            MP_curr2_camera[DMP_curr2_ok] = MP_curr_camera[i];
            std::cout <<"追加動作:MP_curr2_pixel["<<DMP_curr2_ok<<"]="<<MP_curr2_pixel[DMP_curr2_ok]<< std::endl;
            //std::cout <<"追加動作:MP_curr2_camera["<<DMP_curr2_ok<<"]="<<MP_curr2_camera[DMP_curr2_ok]<< std::endl;
            DMP_curr2_ok=DMP_curr2_ok+1;//Depthが取得可能な全マッチテンプレート数
          }
        ...
        //cv::line(img_dst2,cv::Point(MP_curr2_pixel[1].x,MP_curr2_pixel[1].y),cv::Point(MP_prev_pixel[1].x,MP_prev_pixel[1].y),cv::Scalar(0,0,0), 2, cv::LINE_AA);
