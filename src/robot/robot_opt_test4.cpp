...
vector<cv::Point2f> MP_curr_pixel;//マッチテンプレート座標の画像座標系
vector<cv::Point3f> MP_curr_camera;//マッチテンプレート座標のカメラ座標系
...
//  
  ...
  //全ての予測範囲が画面外に行った場合、もしくはキープするテンプレートがなくなった場合は初回動作に戻る
	std::cout <<"マッチング保存情報"<< std::endl;



  //特徴点検出とテンプレート作成
  //if (reset == true) {
    //if(markerIds.size() > 0){//検出時にマーカが観測可能なときのみ検出実行(観測できない時は検出せず再度検出を行う)
        ...
        if(template_size<points_curr[i].y&&points_curr[i].y<480-template_size){
          if(template_size<points_curr[i].x&&points_curr[i].x<640-template_size){
            ...
            if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
              ...
              cv::rectangle(img_master_temp, roi2,cv::Scalar(255, 255, 255), 2);//マッチング予測範囲(赤枠)
  ...
  if(time0 != false){
    if (reset == false) {//初回以降動作
      ...
      for(int i=0;i<depth_point_prev_ok;i++){
        ...
        double coix=Est_pixel[i].x+template_size;//予測範囲の中心座標
        double coiy=Est_pixel[i].y+template_size;//予測範囲の中心座標
        //予測範囲が全て画面内の時
        if(coix-template_size*2>=0&&coiy-template_size*2>0&&coix+template_size*2<=640&&coiy+template_size*2<=480){
          std::cout << "予測範囲が全て画面内の時["<<i<<"]"<< std::endl;
          cv::Rect roiEST(cv::Point(Est_pixel[i].x-template_size,Est_pixel[i].y-template_size), cv::Size(template_size*4, template_size*4));//線の中点を中心とした線の画像を切り取る
        ...
        //yがはみ出る(xははみ出ない)
        else if(coix-template_size*2>=0&&coix+template_size*2<640){
          if(coiy-template_size*2<0&&coiy>=0&&coiy+template_size*2>480){
            std::cout << "yがはみ出る(xははみ出ない)["<<i<<"]"<<std::endl;
            cv::Rect roiEST(cv::Point(Est_pixel[i].x-template_size,0), cv::Size(template_size*4, coiy+template_size*2));//線の中点を中心とした線の画像を切り取る
            EST_scope[i] = image(roiEST); // 切り出し画像
           cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲
          }
          else if(coiy-template_size*2>0&&coiy<=480&&coiy+template_size*2>480){
            std::cout << "yがはみ出る(xははみ出ない)["<<i<<"]"<<std::endl;
            cv::Rect roiEST(cv::Point(Est_pixel[i].x-template_size,Est_pixel[i].y-template_size), cv::Size(template_size*4, 480-coiy+template_size*2));//線の中点を中心とした線の画像を切り取る
            EST_scope[i] = image(roiEST); // 切り出し画像
            cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲
          }
        }
        //xがはみ出る(yははみ出ない)
        else if(coiy-template_size*2>=0&&coiy+template_size*2<480){
          if(coix-template_size*2<0&&coix>=0&&coix+template_size*2>640){
            std::cout << "xがはみ出る(yははみ出ない)["<<i<<"]"<<std::endl;
            cv::Rect roiEST(cv::Point(0,Est_pixel[i].y-template_size), cv::Size(coix+template_size*2, template_size*4));//線の中点を中心とした線の画像を切り取る
            EST_scope[i] = image(roiEST); // 切り出し画像
            cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲
          }
          else if(coix-template_size*2>0&&coix<=640&&coix+template_size*2>640){
            std::cout << "xがはみ出る(yははみ出ない)["<<i<<"]"<<std::endl;
            cv::Rect roiEST(cv::Point(Est_pixel[i].x-template_size,Est_pixel[i].y-template_size), cv::Size(640-coix+template_size*2, template_size*4));//線の中点を中心とした線の画像を切り取る
            EST_scope[i] = image(roiEST); // 切り出し画像
            cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲
          }
        }
        //xもyもはみ出る
        else if(coix>0&&coiy>0&&coix<640&&coiy<480){
          if(coix-template_size*2<0&&coiy-template_size*2){
            std::cout << "xもyもはみ出る["<<i<<"]"<< std::endl;
            cv::Rect roiEST(cv::Point(0,0), cv::Size(coix+template_size*2, coiy+template_size*2));//線の中点を中心とした線の画像を切り取る
            EST_scope[i] = image(roiEST); // 切り出し画像
            cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲
          }
          else if(coix+template_size*2<640&&coiy+template_size*2<480){
            std::cout << "xもyもはみ出る["<<i<<"]"<< std::endl;
            cv::Rect roiEST(cv::Point(Est_pixel[i].x-template_size,Est_pixel[i].y-template_size), cv::Size(640-coix+template_size*2, 480-coiy+template_size*2));//線の中点を中心とした線の画像を切り取る
            EST_scope[i] = image(roiEST); // 切り出し画像
            cv::rectangle(img_dst, roiEST,cv::Scalar(0, 255, 255), 2);//マッチング予測範囲
          }
        }
        ...
        cv::Mat img_minmax1;
        // テンプレートマッチング
        cv::matchTemplate(EST_scope[i], img_template1, img_minmax1, cv::TM_SQDIFF);//差分二乗和
        ...
        if(min_val1[i]<max_val1[i]*0.01){//最小値がしきい値以下なら表示
          ...
          if(coix-template_size*2>=0&&coiy-template_size*2>0&&coix+template_size*2<=640&&coiy+template_size*2<=480){
            ...
            cv::rectangle(img_dst, cv::Rect(Est_pixel[i].x-template_size+min_pt1[i].x, Est_pixel[i].y-template_size+min_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(0, 255, 0), 3);//白枠
            cv::circle(img_dst, cv::Point(Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2), Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
            Matching_pixel_curr[matchC_curr].x=Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2);//マッチング中心座標(画像全体座標)
            Matching_pixel_curr[matchC_curr].y=Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2);//マッチング中心座標(画像全体座標)
          ...
          //yがはみ出る(xははみ出ない)
          if(coix-template_size*2>=0&&coix+template_size*2<640){
            if(coiy-template_size*2<0&&coiy>=0&&coiy+template_size*2>480){
              std::cout << "マッチング:yがはみ出る(xははみ出ない)["<<i<<"]"<<std::endl;
              cv::rectangle(img_dst, cv::Rect(Est_pixel[i].x-template_size+min_pt1[i].x, min_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(0, 255, 0), 3);//白枠
              cv::circle(img_dst, cv::Point(Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2), min_pt1[i].y+(img_template1.rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
              Matching_pixel_curr[matchC_curr].x=Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2);
              Matching_pixel_curr[matchC_curr].y=min_pt1[i].y+(img_template1.rows/2);
              Matching_Templ_curr[matchC_curr]=img_template1;//マッチングしたテンプレート画像
              std::cout <<"マッチングの中心座標[matchC_curr="<<matchC_curr<<"]="<<Matching_pixel_curr[matchC_curr]<< std::endl;
              matchC_curr=matchC_curr+1;//マッチングの中心座標個数
            }
              ...
              cv::circle(img_dst, cv::Point(Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2), Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
              Matching_pixel_curr[matchC_curr].x=Est_pixel[i].x-template_size+(min_pt1[i].x*2)+(img_template1.cols/2);
              Matching_pixel_curr[matchC_curr].y=Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2);
              Matching_Templ_curr[matchC_curr]=img_template1;//マッチングしたテンプレート画像
              std::cout <<"マッチングの中心座標[matchC_curr="<<matchC_curr<<"]="<<Matching_pixel_curr[matchC_curr]<< std::endl;
              matchC_curr=matchC_curr+1;//マッチングの中心座標個数
            }
          ...
          //xがはみ出る(yははみ出ない)
            ...
            if(coix-template_size*2<0&&coix>=0&&coix+template_size*2>640){
              std::cout << "マッチング:xがはみ出る(yははみ出ない)["<<i<<"]"<<std::endl;
              cv::rectangle(img_dst, cv::Rect(min_pt1[i].x, Est_pixel[i].y-template_size+min_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(0, 255, 0), 3);//白枠
              cv::circle(img_dst, cv::Point(min_pt1[i].x+(img_template1.cols/2), Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
              Matching_pixel_curr[matchC_curr].x=min_pt1[i].x+(img_template1.cols/2);
              Matching_pixel_curr[matchC_curr].y=Est_pixel[i].y-template_size+(min_pt1[i].y*2)+(img_template1.rows/2);
              Matching_Templ_curr[matchC_curr]=img_template1;//マッチングしたテンプレート画像
              std::cout <<"マッチングの中心座標[matchC_curr="<<matchC_curr<<"]="<<Matching_pixel_curr[matchC_curr]<< std::endl;
              matchC_curr=matchC_curr+1;//マッチングの中心座標個数
          ...
          //xもyもはみ出る
            ...
            if(coix-template_size*2<0&&coiy-template_size*2){
              std::cout << "マッチング:xもyもはみ出る["<<i<<"]"<< std::endl;
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<DMP_prev_ok;i++){
          ...
          //予測範囲が全て画面内の時
          if(MPcoix[i]-template_size*2>=0&&MPcoiy[i]-template_size*2>0&&MPcoix[i]+template_size*2<=640&&MPcoiy[i]+template_size*2<=480){
            std::cout << "予測範囲が全て画面内の時["<<i<<"]"<< std::endl;
            cv::Rect roiEST(cv::Point(Est_MP_pixel[i].x-template_size,Est_MP_pixel[i].y-template_size), cv::Size(template_size*4, template_size*4));//線の中点を中心とした線の画像を切り取る
            EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
            MPcoix[EST_MP_ok]=MPcoix[i];
            MPcoiy[EST_MP_ok]=MPcoiy[i];
          ...
          //yがはみ出る(xははみ出ない)
          else if(MPcoix[i]-template_size*2>=0&&MPcoix[i]+template_size*2<640){
            if(MPcoiy[i]-template_size*2<0&&MPcoiy[i]>=0&&MPcoiy[i]+template_size*2>480){
              std::cout << "yがはみ出る(xははみ出ない)["<<i<<"]"<<std::endl;
              cv::Rect roiEST(cv::Point(Est_MP_pixel[i].x-template_size,0), cv::Size(template_size*4, MPcoiy[i]+template_size*2));//線の中点を中心とした線の画像を切り取る
              EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
              MPcoix[EST_MP_ok]=MPcoix[i];
              MPcoiy[EST_MP_ok]=MPcoiy[i];
              Est_MP_pixel[EST_MP_ok] = Est_MP_pixel[i];
            ...
            else if(MPcoiy[i]-template_size*2>0&&MPcoiy[i]<=480&&MPcoiy[i]+template_size*2>480){
              std::cout << "yがはみ出る(xははみ出ない)["<<i<<"]"<<std::endl;
              cv::Rect roiEST(cv::Point(Est_MP_pixel[i].x-template_size,Est_MP_pixel[i].y-template_size), cv::Size(template_size*4, 480-MPcoiy[i]+template_size*2));//線の中点を中心とした線の画像を切り取る
              EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
              MPcoix[EST_MP_ok]=MPcoix[i];
              MPcoiy[EST_MP_ok]=MPcoiy[i];
              Est_MP_pixel[EST_MP_ok] = Est_MP_pixel[i];
          ...
          //xがはみ出る(yははみ出ない)
          else if(MPcoiy[i]-template_size*2>=0&&MPcoiy[i]+template_size*2<480){
            if(MPcoix[i]-template_size*2<0&&MPcoix[i]>=0&&MPcoix[i]+template_size*2>640){
              std::cout << "xがはみ出る(yははみ出ない)["<<i<<"]"<<std::endl;
              cv::Rect roiEST(cv::Point(0,Est_MP_pixel[i].y-template_size), cv::Size(MPcoix[i]+template_size*2, template_size*4));//線の中点を中心とした線の画像を切り取る
              EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
              MPcoix[EST_MP_ok]=MPcoix[i];
              MPcoiy[EST_MP_ok]=MPcoiy[i];
              Est_MP_pixel[EST_MP_ok] = Est_MP_pixel[i];
            ...
            else if(MPcoix[i]-template_size*2>0&&MPcoix[i]<=640&&MPcoix[i]+template_size*2>640){
              std::cout << "xがはみ出る(yははみ出ない)["<<i<<"]"<<std::endl;
              cv::Rect roiEST(cv::Point(Est_MP_pixel[i].x-template_size,Est_MP_pixel[i].y-template_size), cv::Size(640-MPcoix[i]+template_size*2, template_size*4));//線の中点を中心とした線の画像を切り取る
              EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
              MPcoix[EST_MP_ok]=MPcoix[i];
              MPcoiy[EST_MP_ok]=MPcoiy[i];
              Est_MP_pixel[EST_MP_ok] = Est_MP_pixel[i];
          ...
          //xもyもはみ出る
          else if(MPcoix[i]>0&&MPcoiy[i]>0&&MPcoix[i]<640&&MPcoiy[i]<480){
            if(MPcoix[i]-template_size*2<0&&MPcoiy[i]-template_size*2){
              std::cout << "xもyもはみ出る["<<i<<"]"<< std::endl;
              cv::Rect roiEST(cv::Point(0,0), cv::Size(MPcoix[i]+template_size*2, MPcoiy[i]+template_size*2));//線の中点を中心とした線の画像を切り取る
              EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
              MPcoix[EST_MP_ok]=MPcoix[i];
              MPcoiy[EST_MP_ok]=MPcoiy[i];
              Est_MP_pixel[EST_MP_ok] = Est_MP_pixel[i];
            ...
            else if(MPcoix[i]+template_size*2<640&&MPcoiy[i]+template_size*2<480){
              std::cout << "xもyもはみ出る["<<i<<"]"<< std::endl;
              cv::Rect roiEST(cv::Point(Est_MP_pixel[i].x-template_size,Est_MP_pixel[i].y-template_size), cv::Size(640-MPcoix[i]+template_size*2, 480-MPcoiy[i]+template_size*2));//線の中点を中心とした線の画像を切り取る
              EST_MP_scope[EST_MP_ok] = image(roiEST); // 切り出し画像
              MPcoix[EST_MP_ok]=MPcoix[i];
              MPcoiy[EST_MP_ok]=MPcoiy[i];
              Est_MP_pixel[EST_MP_ok] = Est_MP_pixel[i];
        ...
        for(int i=0;i<EST_MP_ok;i++){
          ...
          //cv::imshow("win_MP_prev_Templ", MP_prev_Templ[i]);//黄色の特徴点を中心としたクロップ画像

          cv::Mat img_minmax1;
          // テンプレートマッチング
          cv::matchTemplate(EST_MP_scope[i], MP_prev_Templ[i], img_minmax1, cv::TM_SQDIFF);//差分二乗和
          cv::minMaxLoc(img_minmax1, &min_val1[i], &max_val1[i], &min_pt1[i], &max_pt1[i]);
          std::cout << "min_val1(白)["<<i<<"]=" << min_val1[i] << std::endl;//一致度が上がると値が小さくなる
          std::cout << "max_val1(白)["<<i<<"]=" << max_val1[i] << std::endl;
          if(min_val1[i]<max_val1[i]*0.1){//最小値がしきい値以下なら表示
            //予測範囲が全て画面内の時
            if(MPcoix[i]-template_size*2>=0&&MPcoiy[i]-template_size*2>0&&MPcoix[i]+template_size*2<=640&&MPcoiy[i]+template_size*2<=480){
              std::cout << "マッチング:全て画面内の時["<<i<<"]"<< std::endl;
              cv::rectangle(img_dst, cv::Rect(Est_MP_pixel[i].x-template_size+min_pt1[i].x, Est_MP_pixel[i].y-template_size+min_pt1[i].y, MP_prev_Templ[i].cols, MP_prev_Templ[i].rows), cv::Scalar(255, 255, 0), 3);//白枠
              cv::circle(img_dst, cv::Point(Est_MP_pixel[i].x-template_size+(min_pt1[i].x*2)+(MP_prev_Templ[i].cols/2), Est_MP_pixel[i].y-template_size+(min_pt1[i].y*2)+(MP_prev_Templ[i].rows/2)), 5, cv::Scalar(255, 255, 0), -1);//テンプレートの中心座標
              std::cout << "マッチング座標min_pt1["<<i<<"]=" << min_pt1[i] << std::endl;
              Matching_pixel_prev[matchC_prev].x=Est_MP_pixel[i].x-template_size+(min_pt1[i].x*2)+(MP_prev_Templ[i].cols/2);//マッチング中心座標(画像全体座標)
              Matching_pixel_prev[matchC_prev].y=Est_MP_pixel[i].y-template_size+(min_pt1[i].y*2)+(MP_prev_Templ[i].rows/2);//マッチング中心座標(画像全体座標)
              Matching_Templ_prev[matchC_prev]=MP_prev_Templ[i];//マッチングしたテンプレート画像
            ...
            //yがはみ出る(xははみ出ない)
            if(MPcoix[i]-template_size*2>=0&&MPcoix[i]+template_size*2<640){
              if(MPcoiy[i]-template_size*2<0&&MPcoiy[i]>=0&&MPcoiy[i]+template_size*2>480){
                std::cout << "マッチング:yがはみ出る(xははみ出ない)["<<i<<"]"<<std::endl;
                cv::rectangle(img_dst, cv::Rect(Est_MP_pixel[i].x-template_size+min_pt1[i].x, min_pt1[i].y, MP_prev_Templ[i].cols, MP_prev_Templ[i].rows), cv::Scalar(255, 255, 0), 3);//白枠
                cv::circle(img_dst, cv::Point(Est_MP_pixel[i].x-template_size+(min_pt1[i].x*2)+(MP_prev_Templ[i].cols/2), min_pt1[i].y+(MP_prev_Templ[i].rows/2)), 5, cv::Scalar(255, 255, 0), -1);//テンプレートの中心座標
                std::cout << "マッチング座標min_pt1["<<i<<"]=" << min_pt1[i] << std::endl;
                Matching_pixel_prev[matchC_prev].x=Est_MP_pixel[i].x-template_size+(min_pt1[i].x*2)+(MP_prev_Templ[i].cols/2);
                Matching_pixel_prev[matchC_prev].y=min_pt1[i].y+(MP_prev_Templ[i].rows/2);
                Matching_Templ_prev[matchC_prev]=MP_prev_Templ[i];//マッチングしたテンプレート画像
                std::cout <<"マッチングの中心座標[matchC_prev="<<matchC_prev<<"]="<<Matching_pixel_prev[matchC_prev]<< std::endl;
                matchC_prev=matchC_prev+1;//マッチングの中心座標個数
              }
                ...
                std::cout << "マッチング座標min_pt1["<<i<<"]=" << min_pt1[i] << std::endl;
                Matching_pixel_prev[matchC_prev].x=Est_MP_pixel[i].x-template_size+(min_pt1[i].x*2)+(MP_prev_Templ[i].cols/2);
                Matching_pixel_prev[matchC_prev].y=Est_MP_pixel[i].y-template_size+(min_pt1[i].y*2)+(MP_prev_Templ[i].rows/2);
                Matching_Templ_prev[matchC_prev]=MP_prev_Templ[i];//マッチングしたテンプレート画像
                std::cout <<"マッチングの中心座標[matchC_prev="<<matchC_prev<<"]="<<Matching_pixel_prev[matchC_prev]<< std::endl;
                matchC_prev=matchC_prev+1;//マッチングの中心座標個数
              }
            ...
            //xがはみ出る(yははみ出ない)
              ...
              if(MPcoix[i]-template_size*2<0&&MPcoix[i]>=0&&MPcoix[i]+template_size*2>640){
                std::cout << "マッチング:xがはみ出る(yははみ出ない)["<<i<<"]"<<std::endl;
                cv::rectangle(img_dst, cv::Rect(min_pt1[i].x, Est_MP_pixel[i].y-template_size+min_pt1[i].y, MP_prev_Templ[i].cols, MP_prev_Templ[i].rows), cv::Scalar(255, 255, 0), 3);//白枠
                cv::circle(img_dst, cv::Point(min_pt1[i].x+(MP_prev_Templ[i].cols/2), Est_MP_pixel[i].y-template_size+(min_pt1[i].y*2)+(MP_prev_Templ[i].rows/2)), 5, cv::Scalar(255, 255, 0), -1);//テンプレートの中心座標
                std::cout << "マッチング座標min_pt1["<<i<<"]=" << min_pt1[i] << std::endl;
                Matching_pixel_prev[matchC_prev].x=min_pt1[i].x+(MP_prev_Templ[i].cols/2);
                Matching_pixel_prev[matchC_prev].y=Est_MP_pixel[i].y-template_size+(min_pt1[i].y*2)+(MP_prev_Templ[i].rows/2);
                Matching_Templ_prev[matchC_prev]=MP_prev_Templ[i];//マッチングしたテンプレート画像
                std::cout <<"マッチングの中心座標[matchC_prev="<<matchC_prev<<"]="<<Matching_pixel_prev[matchC_prev]<< std::endl;
                matchC_prev=matchC_prev+1;//マッチングの中心座標個数
            ...
            //xもyもはみ出る
              ...
              if(MPcoix[i]-template_size*2<0&&MPcoiy[i]-template_size*2){
                std::cout << "マッチング:xもyもはみ出る["<<i<<"]"<< std::endl;
                cv::rectangle(img_dst, cv::Rect(min_pt1[i].x, min_pt1[i].y, MP_prev_Templ[i].cols, MP_prev_Templ[i].rows), cv::Scalar(255, 255, 0), 3);//白枠
                cv::circle(img_dst, cv::Point(min_pt1[i].x+(MP_prev_Templ[i].cols/2), min_pt1[i].y+(MP_prev_Templ[i].rows/2)), 5, cv::Scalar(255, 255, 0), -1);//テンプレートの中心座標
                std::cout << "マッチング座標min_pt1["<<i<<"]=" << min_pt1[i] << std::endl;
                Matching_pixel_prev[matchC_prev].x=min_pt1[i].x+(MP_prev_Templ[i].cols/2);
                Matching_pixel_prev[matchC_prev].y=min_pt1[i].y+(MP_prev_Templ[i].rows/2);
                Matching_Templ_prev[matchC_prev]=MP_prev_Templ[i];//マッチングしたテンプレート画像
              ...
              std::cout << "マッチしない["<<i<<"](min_val1["<<i<<"]<max_val1["<<i<<"]*0.01)="<<min_val1[i]<<"<"<<max_val1[i]*0.01<< std::endl;
          }
        }//for(int i=0;i<DMP_prev_ok;i++)→end (範囲予測+テンプレートマッチング)
        ...
        //マッチングしたテンプレートをマッチテンプレートとしてキープする
        //マッチテンプレートのDepthが取得不能な点を削除
        DMP_prev_ok=DMP_curr_ok;//Depth取得可能マッチテンプレート数をキープ+(3回目以降はこれに一つ前のマッチテンプレート要素を追加する)
        for (int i = 0; i < matchC_prev; i++) {
          DMP_prev[i] = depthimage.at<float>(cv::Point(Matching_pixel_prev[i].x,Matching_pixel_prev[i].y));//DMP_prev=Depth_Matching_pixel_prev
          //Depthが取得できない特徴点を削除する+Depthの外れ値を除く
          if(DMP_prev[i]>0.001&&DMP_prev[i]<10000){
            MP_curr_Templ[DMP_prev_ok] = Matching_Templ_prev[i];//Depth取得可能なマッチテンプレート
            MP_curr_pixel[DMP_prev_ok] = Matching_pixel_prev[i];//Depth取得可能なマッチング中心画像座標
            MP_curr_camera[DMP_prev_ok].x = -DMP_prev[i] * ((Matching_pixel_prev[i].x - 324.473) / 615.337)/1000;//カメラ座標変換
            MP_curr_camera[DMP_prev_ok].y = -DMP_prev[i] * ((Matching_pixel_prev[i].y - 241.696) / 615.458)/1000;
            MP_curr_camera[DMP_prev_ok].z = DMP_prev[i]/1000;
            std::cout <<"MP_curr_pixel["<<DMP_prev_ok<<"]="<<MP_curr_pixel[DMP_prev_ok]<< std::endl;
            std::cout <<"MP_curr_camera["<<DMP_prev_ok<<"]="<<MP_curr_camera[DMP_prev_ok]<< std::endl;
            DMP_prev_ok=DMP_prev_ok+1;//Depthが取得可能な全マッチテンプレート数
            //DMP_prev_ok3=DMP_prev_ok3+1;//保持テンプレート数
          }
		    }
        std::cout <<"全テンプレート数:DMP_prev_ok="<<DMP_prev_ok<< std::endl;
        MP_curr_pixel.resize(DMP_prev_ok);//Depth取得可能数でリサイズ(マッチング中心画像座標)
        MP_curr_camera.resize(DMP_prev_ok);//Depth取得可能数でリサイズ(マッチング中心カメラ座標)
  ...
  //経路描写-------------------------------------------------------------
    ...
    if(Tracking==false){ DMP_prev_ok=DMP_curr_ok;}//Depth取得可能マッチテンプレート数をキープ+(3回目以降はこれに一つ前のテンプレート要素を追加する)
