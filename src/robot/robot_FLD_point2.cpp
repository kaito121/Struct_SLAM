...
cv::Mat frame,img_dst,img_dst2,img_1,img_graph;
...
//テンプレートマッチ用変数
vector<cv::Point3f> Est_tate_point;//特徴線定義(運動復元
vector<cv::Point2f> Est_tate_pixel;//特徴線定義(運動復元
...
cv::Mat EST_tate_scope[100];//特徴線周囲の切り取った画像(予測範囲画像)
...
vector<cv::Point3f> Est_MT_point;//特徴線定義(運動復元
vector<cv::Point2f> Est_MT_pixel;//特徴線定義(運動復元
...
ofstream Realsec("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/FLD_point2_sec.txt");
...
void callback(const nav_msgs::Odometry::ConstPtr& msg,const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
  ...
  cv::Mat img_gray,img_gray2,img_edge,img_dst,img_fld,img_line2,img_line4;
  ...
  //線検出-----------------------------------------------------------------
  std::vector<cv::Vec4f> lines_fld;//FLD変換
  ...
  for(int i =  0; i < lines_fld.size(); i++){
      ...
      if(dep1[i]>0 && dep2[i]>0){//dep1とdep2が0より大きい時に実行する。(距離データの損失を考慮)//ここで距離０を除いてる
          lines2[ksk][0]=lines_fld[i][0],lines2[ksk][1]=lines_fld[i][1];
          lines2[ksk][2]=lines_fld[i][2],lines2[ksk][3]=lines_fld[i][3];
          dep1[ksk]=dep1[i],dep2[ksk]=dep2[i];
  ...
  for(int i=0; i<ksk; i++){
      ...
      lines3[lines3_count][0]=lines2[i][0],lines3[lines3_count][1]=lines2[i][1];//ナナメの線以外を抽出する
      lines3[lines3_count][2]=lines2[i][2],lines3[lines3_count][3]=lines2[i][3];
      lines3_dep[lines3_count][0]=dep1[ksk],lines3_dep[lines3_count][1]=dep2[ksk];
    ...
    lines_NNM[lines_NNM_count][0]=lines2[i][0],lines_NNM[lines_NNM_count][1]=lines2[i][1];//ナナメの線のみを抽出する
    lines_NNM[lines_NNM_count][2]=lines2[i][2],lines_NNM[lines_NNM_count][3]=lines2[i][3];
  ...
  for (int j=0; j< lines3_count; ++j) {
    ...
    if(lines3X>lines3Y){
      ...
      yokolines3[yokoyouso][0]=lines3[j][0],yokolines3[yokoyouso][1]=lines3[j][1];//(x成分),(y成分)
      yokolines3[yokoyouso][2]=lines3[j][2],yokolines3[yokoyouso][3]=lines3[j][3];//(x成分),(y成分)
      yokolines3[yokoyouso][4]=lines3_theta[j];
      yokoZ1[yokoyouso]=lines3_dep[j][0],yokoZ2[yokoyouso]=lines3_dep[j][1];
    ...
    //縦線に分類
      ...
      if(lines3Y>lines3X){
        ...
        tatelines3[tateyouso][0]=lines3[j][0],tatelines3[tateyouso][1]=lines3[j][1];
        tatelines3[tateyouso][2]=lines3[j][2],tatelines3[tateyouso][3]=lines3[j][3];
        tatelines3[tateyouso][4]=lines3_theta[j];

        //tatetheta[tateyouso]=lines3_theta[j];
        tateZ1[tateyouso]=lines3_dep[j][0],tateZ2[tateyouso]=lines3_dep[j][1];
  ...
  for (int i=0; i<tateyouso; ++i) {
    for (int j=i+1;j<tateyouso; ++j) {
      if (tatetheta[i] > tatetheta[j]) {
        tmp =  tatetheta[i];
        tmp1x =  tatelines3[i][0],tmp1y =  tatelines3[i][1];
        tmp2x =  tatelines3[i][2],tmp2y =  tatelines3[i][3];
        tmp3 =  tatelines3[i][4];

        tatetheta[i] = tatetheta[j];
        tatelines3[i][0] = tatelines3[j][0],tatelines3[i][1] = tatelines3[j][1];
        tatelines3[i][2] = tatelines3[j][2],tatelines3[i][3] = tatelines3[j][3];
        tatelines3[i][4] = tatelines3[j][4];

        tatetheta[j] = tmp;
        tatelines3[j][0] = tmp1x,tatelines3[j][1] = tmp1y;
        tatelines3[j][2] = tmp2x,tatelines3[j][3] = tmp2y;
        tatelines3[j][4] = tmp3;
      }
    }
  }
  ...
  for (int i=0; i<yokoyouso; ++i) {
    for (int j=i+1;j<yokoyouso; ++j) {
      if (yokotheta[i] > yokotheta[j]) {
        tmp =  yokotheta[i];
        tmp1x =  yokolines3[i][0],tmp1y =  yokolines3[i][1];
        tmp2x =  yokolines3[i][2],tmp2y =  yokolines3[i][3];
        tmp3 =  yokolines3[i][4];

        yokotheta[i] = yokotheta[j];
        yokolines3[i][0] = yokolines3[j][0],yokolines3[i][1] = yokolines3[j][1];
        yokolines3[i][2] = yokolines3[j][2],yokolines3[i][3] = yokolines3[j][3];
        yokolines3[i][4] = yokolines3[j][4];

        yokotheta[j] = tmp;
        yokolines3[j][0] = tmp1x,yokolines3[j][1] = tmp1y;
        yokolines3[j][2] = tmp2x,yokolines3[j][3] = tmp2y;
        yokolines3[j][4] = tmp3;
      }
    }
  }
  ...
  if(tateyouso==0){
    ...
    for (int j=0; j< tateyouso; ++j) {
      if(tateyouso==1){//要素がひとつしかないとき比較ができない
        ...
        tateclust[0][clust[0]][0]=tatelines3[j][0],tateclust[0][clust[0]][1]=tatelines3[j][1];//(X成分),(y成分)
        tateclust[0][clust[0]][2]=tatelines3[j][2],tateclust[0][clust[0]][3]=tatelines3[j][3];//(X成分),(y成分)
        ...
        if(j==0){
          ...
          if(clusR>abs(tatetheta[j]*3-tatetheta[j+1]*3)){
            ...
            // クラスタリングされたヨコ線(クラスタ番号,クラスタ内部個数,成分番号)
            tateclust[clusCT][clust[clusCT]][0]=tatelines3[j][0],tateclust[clusCT][clust[clusCT]][1]=tatelines3[j][1];//(X成分),(y成分)
            tateclust[clusCT][clust[clusCT]][2]=tatelines3[j][2],tateclust[clusCT][clust[clusCT]][3]=tatelines3[j][3];//(X成分),(y成分)
    ...
    for (int i=0; i<clust[MAXT]; ++i) {
      if(sqrt((tateclust[MAXT][i][0]-tateclust[MAXT][i][2])*(tateclust[MAXT][i][0]-tateclust[MAXT][i][2])
        ...
        tateclust[MAXT][tateok][0]=tateclust[MAXT][i][0],tateclust[MAXT][tateok][1]=tateclust[MAXT][i][1];
        tateclust[MAXT][tateok][2]=tateclust[MAXT][i][2],tateclust[MAXT][tateok][3]=tateclust[MAXT][i][3];
    ...
    for (int i=0; i<tateok; ++i) {
      for (int j=i+1;j<tateok; ++j) {
        if (tateclust[MAXT][i][0]+tateclust[MAXT][i][2] > tateclust[MAXT][j][0]+tateclust[MAXT][j][2]) {
          tmp1x =  tateclust[MAXT][i][0],tmp1y =  tateclust[MAXT][i][1];
          tmp2x =  tateclust[MAXT][i][2],tmp2y =  tateclust[MAXT][i][3];
          tmp3 =  tateclust[MAXT][i][4];

          tateclust[MAXT][i][0] = tateclust[MAXT][j][0],tateclust[MAXT][i][1] = tateclust[MAXT][j][1];
          tateclust[MAXT][i][2] = tateclust[MAXT][j][2],tateclust[MAXT][i][3] = tateclust[MAXT][j][3];
          tateclust[MAXT][i][4] = tateclust[MAXT][j][4];

          tateclust[MAXT][j][0] = tmp1x,tateclust[MAXT][j][1] = tmp1y;
          tateclust[MAXT][j][2] = tmp2x,tateclust[MAXT][j][3] = tmp2y;
          tateclust[MAXT][j][4] = tmp3;
        }
      }
      //tateclust[MAXT][j][1]の位置を上にする
      if(tateclust[MAXT][i][1]>tateclust[MAXT][i][3]){
        tempy=tateclust[MAXT][i][1],tempx=tateclust[MAXT][i][0];
    ...
    //縦線の法線ベクトルを求める-----------------------------------------
    MatrixXd R(2,2);
    R(0,0)=cos(M_PI/2),R(0,1)=-sin(M_PI/2);
    R(1,0)=sin(M_PI/2),R(1,1)=cos(M_PI/2);
  ...
  if(yokoyouso==0){
    ...
    for (int j=0; j< yokoyouso; ++j) {
      if(yokoyouso==1){//要素がひとつしかない時は比較ができない(クラスタリングできない)
        ...
        // クラスタリングされたヨコ線(クラスタ番号,クラスタ内部個数,成分番号)
        yokoclusy[0][clusy[0]][0]=yokolines3[j][0],yokoclusy[0][clusy[0]][1]=yokolines3[j][1];//(x成分),(y成分)
        yokoclusy[0][clusy[0]][2]=yokolines3[j][2],yokoclusy[0][clusy[0]][3]=yokolines3[j][3];//(x成分),(y成分)
        ...
        if(j==0){
          ...
          if(clusR>abs(yokotheta[j]*3-yokotheta[j+1]*3)){
            ...
            // クラスタリングされたヨコ線(クラスタ番号,クラスタ内部個数,成分番号)
            yokoclusy[clusCY][clusy[clusCY]][0]=yokolines3[j][0],yokoclusy[clusCY][clusy[clusCY]][1]=yokolines3[j][1];//(x成分),(y成分)
            yokoclusy[clusCY][clusy[clusCY]][2]=yokolines3[j][2],yokoclusy[clusCY][clusy[clusCY]][3]=yokolines3[j][3];//(x成分),(y成分)
  ...
  //特徴点検出とテンプレート作成--------------------------------------------------------------------------
  //特徴点検出--------------------------------------------------------------------------------------------------------------
	cv::cvtColor(img_src, image_curr, cv::COLOR_BGR2GRAY);//グレースケール化
  depth_point_curr_ok=0; //depth取得可能な特徴点の数(初期化)
  //cv::goodFeaturesToTrack(今画像, 今の特徴点, 特徴点の個数, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
	cv::goodFeaturesToTrack(image_curr, points_curr, 15, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);// 特徴点検出(グレースケール画像から特徴点検出)
  ...
  img_src.copyTo(img_point_master_temp);//切り取り場所を表示
  ...
  for (int i = 0; i < points_curr.size(); i++) {
    if(template_size<points_curr[i].y&&points_curr[i].y<480-template_size){
      if(template_size<points_curr[i].x&&points_curr[i].x<640-template_size){
        depth_point_curr[i] = img_depth.at<float>(cv::Point(points_curr[i].x,points_curr[i].y));
        ...
        if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
          ...
          FeaturePoint_curr[depth_point_curr_ok] = img_src(roi2); // 切り出し画像
          cv::rectangle(img_point_master_temp, roi2,cv::Scalar(255, 255, 255), 2);//テンプレート位置
  ...
  if(time0 != false){
    if (reset == false) {//初回以降動作
      ...
      for(int i=0;i<depth_point_prev_ok;i++){
        ...
        if(cropx<=Est_pixel[i].x&&Est_pixel[i].x<=640-cropx&&cropy<=Est_pixel[i].y&&Est_pixel[i].y<=480-cropy){
          ...
          EST_scope[i] = img_src(roiEST); // 切り出し画像
      ...
      for (int i = 0; i < matchC_curr; i++) {
        DMP_curr[i] = img_depth.at<float>(cv::Point(Matching_pixel_curr[i].x,Matching_pixel_curr[i].y));//DMP_curr=Depth_Matching_pixel_curr
        //Depthが取得できない特徴点を削除する+Depthの外れ値を除く
      ...
      //ロボットの動きから特徴線の運動復元を行う(特徴線の状態方程式を使用)
      ...
      for(int i=0;i<DTPP_ok;i++){
        ...
        if(cropx<=Est_tate_pixel[i].x&&Est_tate_pixel[i].x<=640-cropx&&cropy<=Est_tate_pixel[i].y&&Est_tate_pixel[i].y<=480-cropy){
          ...
          //cv::imshow("EST_tate_scope", EST_tate_scope[i]);//黄色の特徴線を中心としたクロップ画像
        }
        //左側
        ...
        //cv::imshow("win_TPCP_Templ", TPCP_Templ[i]);//黄色の特徴線を中心としたクロップ画像
      ...
      for (int i = 0; i < matchT_curr; i++) {
        ...
        //Depthが取得できない特徴線を削除する+Depthの外れ値を除く
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<DMP_prev_ok;i++){
          ...
          if(cropx<=Est_MP_pixel[i].x&&Est_MP_pixel[i].x<=640-cropx&&cropy<=Est_MP_pixel[i].y&&Est_MP_pixel[i].y<=480-cropy){
            ...
            EST_MP_scope[EST_MP_ok] = img_src(roiEST); // 切り出し画像
        ...
        for(int i=0;i<EST_MP_ok;i++){
          ...
          if(0.83<max_val1[i]){//最小値がしきい値以下なら表示
            ...
            if(cropx<=Est_MP_pixel[i].x&&Est_MP_pixel[i].x<=640-cropx&&cropy<=Est_MP_pixel[i].y&&Est_MP_pixel[i].y<=480-cropy){
              ...
              Matching_Templ_prev[matchC_prev] = img_src(roi_match); // 切り出し画像
        ...
        for (int i = 0; i < matchC_prev; i++) {
          DMP_curr2[i] = img_depth.at<float>(cv::Point(Matching_pixel_prev[i].x,Matching_pixel_prev[i].y));//DMP_prev=Depth_Matching_pixel_prev
          //Depthが取得できない特徴点を削除する+Depthの外れ値を除く
        ...
        //ロボットの動きから一つ前のマッチング座標の運動復元を行う(特徴線の状態方程式を使用)---------------------------------------------------特徴線
        EST_MT_ok=0;
        for(int i=0;i<DMT_prev_ok;i++){
          ...
          if(cropx<=Est_MT_pixel[i].x&&Est_MT_pixel[i].x<=640-cropx&&cropy<=Est_MT_pixel[i].y&&Est_MT_pixel[i].y<=480-cropy){
           ...
           // cv::imshow("EST_MT_scope", EST_MT_scope[EST_MT_ok]);//黄色の特徴線を中心としたクロップ画像
        ...
        if(markerIds.size() <= 0){
          double minlengh=1000000;
          int minMC;
          //特徴点が観測可能な場合は特徴点の世界座標を使用し、特徴線の世界座標を推定する
          if(DMP_curr_ok > 0){
             std::cout <<"マーカー観測不能時(特徴点観測可能時)"<< std::endl;
            for (int i = 0; i < DMT_curr_ok; i++) {
              for(int j = 0; j < DMP_curr_ok; j++){
                length=sqrt((MT_curr_camera[i].x-MP_curr_camera[j].x)*(MT_curr_camera[i].x-MP_curr_camera[j].x)
                +(MT_curr_camera[i].y-MP_curr_camera[j].y)*(MT_curr_camera[i].y-MP_curr_camera[j].y)
                +(MT_curr_camera[i].z-MP_curr_camera[j].z)*(MT_curr_camera[i].z-MP_curr_camera[j].z));
                if(minlengh>length){
                  minMC=j;
                  minlengh=length;
                }
              }
              MT_curr_world[i].x=MP_curr2_world[minMC].x+(MT_curr_camera[i].x-MP_curr_camera[minMC].x)*cos(-Est_RobotTH)-(MT_curr_camera[i].z-MP_curr_camera[minMC].z)*sin(-Est_RobotTH);
              MT_curr_world[i].y=MP_curr2_world[minMC].y+(MT_curr_camera[i].y-MP_curr_camera[minMC].y);
              MT_curr_world[i].z=MP_curr2_world[minMC].z+(MT_curr_camera[i].x-MP_curr_camera[minMC].x)*sin(-Est_RobotTH)+(MT_curr_camera[i].z-MP_curr_camera[minMC].z)*cos(-Est_RobotTH);
            }
            MT_curr_world.resize(DMT_curr_ok);//Depth取得可能数でリサイズ(マッチング中心世界座標)
            //for (int i = 0; i < DMT_curr_ok; i++) {
            //  std::cout <<"新規テンプレートの世界座標:MT_curr_world["<<i<<"]="<<MT_curr_world[i]<< std::endl;
          ...
          //特徴点も観測できない場合は特徴線の2回目動作での世界座標を使用し、特徴線の世界座標を推定する
          else{
            std::cout <<"マーカー観測不能時(特徴点観測不能時)"<< std::endl;
            for (int i = 0; i < DMT_curr_ok; i++) {
    ...
    if(Tracking == true){
      ...
      std::cout << "tf特徴線の世界座標:MT_curr2_world[i]={x="<< MT_curr2_world[0].x <<",y="<<MT_curr2_world[0].y<<",z="<<MT_curr2_world[0].z<<"}"<< std::endl;
    ...
    if(Act_RobotX>0||Act_RobotY>0||Act_RobotTH>0){//ラグの調整
      ...
      if(robot_velocity.angular.z==0){
        At00=cos(Act_RobotTH)*realsec,At01=0;
        At10=sin(Act_RobotTH)*realsec,At11=0;
        At20=0,At21=realsec;
      }
      else{
  ...
  if(kaisu>1){
    ...
    Est_Robot_pose.position.x = 0, Est_Robot_pose.position.y = 0, Est_Robot_pose.position.z = 0;
    ...
    Est_Robot_pose.orientation.x=0, Est_Robot_pose.orientation.y=0, Est_Robot_pose.orientation.z=0,Est_Robot_pose.orientation.w=0;
  }
...
int main(int argc,char **argv){
  ...
  MarkerW[4]= (cv::Mat_<float>(3, 1) <<1.0, 0.28, 12.95);//実測値(X:11,Y:1.99)
