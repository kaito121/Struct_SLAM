    ...
    if (reset == false) {//初回以降動作
      ...
      if(markerIds.size() > 0){
        ...
        for (int i = 0; i < DMP_curr_ok; i++) {
          ...
          MP_curr_world[i].x=MarkerW[markerIds.at(minMC)].at<float>(0)-MC_point[markerIds.at(minMC)][0]+MP_curr_camera[i].x;
          MP_curr_world[i].y=MarkerW[markerIds.at(minMC)].at<float>(1)-MC_point[markerIds.at(minMC)][1]+MP_curr_camera[i].y;
          MP_curr_world[i].z=MarkerW[markerIds.at(minMC)].at<float>(2)-MC_point[markerIds.at(minMC)][2]+MP_curr_camera[i].z;
        }
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<EST_MP_ok;i++){
          ...
          if(0.8<max_val1[i]){//最小値がしきい値以下なら表示
            ...
            if(cropx<=Est_MP_pixel[i].x&&Est_MP_pixel[i].x<=640-cropx&&cropy<=Est_MP_pixel[i].y&&Est_MP_pixel[i].y<=480-cropy){
              ...
              if(i==0){
              cv::rectangle(img_dst, cv::Rect(Est_MP_pixel[i].x-cropx+max_pt1[i].x, Est_MP_pixel[i].y-cropy+max_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(255, 0, 255), 3);//白枠
              cv::circle(img_dst, cv::Point(Est_MP_pixel[i].x-cropx+max_pt1[i].x+(img_template1.cols/2), Est_MP_pixel[i].y-cropy+max_pt1[i].y+(img_template1.rows/2)), 5, cv::Scalar(255, 0, 255), -1);//テンプレートの中心座標
              }
        ...
        if(markerIds.size() <= 0){
          ...
          for (int i = 0; i < DMP_curr_ok; i++) {
            ...
            MP_curr_world[i].x=MP_curr2_world[minMC].x-MP_curr2_camera[minMC].x+MP_curr_camera[i].x;
            MP_curr_world[i].y=MP_curr2_world[minMC].y-MP_curr2_camera[minMC].y+MP_curr_camera[i].y;
            MP_curr_world[i].z=MP_curr2_world[minMC].z-MP_curr2_camera[minMC].z+MP_curr_camera[i].z;
          }
          MP_curr_world.resize(DMP_curr_ok);//Depth取得可能数でリサイズ(マッチング中心世界座標)
          for (int i = 0; i < DMP_curr_ok; i++) {
            std::cout <<"新規テンプレートの世界座標:MP_curr_world["<<i<<"]="<<MP_curr_world[i]<< std::endl;
