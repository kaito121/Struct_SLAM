  ...
  if(time0 != false){
    if (reset == false) {//初回以降動作
      ...
      for(int i=0;i<depth_point_prev_ok;i++){
        ...
        cv::Mat img_minmax1;
        // テンプレートマッチング
        cv::matchTemplate(EST_scope[i], img_template1, img_minmax1, cv::TM_CCORR_NORMED);//正規化相互相関
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<EST_MP_ok;i++){
          ...
          cv::Mat img_minmax1;
          // テンプレートマッチング
          cv::matchTemplate(EST_MP_scope[i], MP_prev_Templ[i], img_minmax1, cv::TM_CCORR_NORMED);//正規化相互相関
          cv::minMaxLoc(img_minmax1, &min_val1[i], &max_val1[i], &min_pt1[i], &max_pt1[i]);
          std::cout << "min_val1(白)["<<i<<"]=" << min_val1[i] << std::endl;//一致度が上がると値が小さくなる
          std::cout << "max_val1(白)["<<i<<"]=" << max_val1[i] << std::endl;
          ...
          if(0.97<max_val1[i]){//最小値がしきい値以下なら表示
            ...
            if(cropx<=Est_MP_pixel[i].x&&Est_MP_pixel[i].x<=640-cropx&&cropy<=Est_MP_pixel[i].y&&Est_MP_pixel[i].y<=480-cropy){
              std::cout << "マッチング:全て画面内の時["<<i<<"]"<< std::endl;
              cv::rectangle(img_dst, cv::Rect(Est_MP_pixel[i].x-cropx+max_pt1[i].x, Est_MP_pixel[i].y-cropy+max_pt1[i].y, MP_prev_Templ[i].cols, MP_prev_Templ[i].rows), cv::Scalar(0, 255, 0), 3);//白枠
              cv::circle(img_dst, cv::Point(Est_MP_pixel[i].x-cropx+max_pt1[i].x+(MP_prev_Templ[i].cols/2), Est_MP_pixel[i].y-cropy+max_pt1[i].y+(MP_prev_Templ[i].rows/2)), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
              Matching_pixel_prev[matchC_prev].x=Est_MP_pixel[i].x-cropx+max_pt1[i].x+(MP_prev_Templ[i].cols/2);//マッチング中心座標(画像全体座標)
              Matching_pixel_prev[matchC_prev].y=Est_MP_pixel[i].y-cropy+max_pt1[i].y+(MP_prev_Templ[i].rows/2);//マッチング中心座標(画像全体座標)
