...
ofstream Test_MT_C2_Cx0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C2_Cx0.txt");//観測残差の確認
ofstream Test_MT_C2_Cy0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C2_Cy0.txt");//観測残差の確認
ofstream Test_MT_C1_Cx0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C1_Cx0.txt");//観測残差の確認
ofstream Test_MT_C1_Cy0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C1_Cy0.txt");//観測残差の確認
ofstream Test_MT_C2_Wx0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C2_Wx0.txt");//観測残差の確認
ofstream Test_MT_C2_Wy0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C2_Wy0.txt");//観測残差の確認
ofstream Test_MT_C1_Wx0("/home/fuji/catkin_ws/src/Struct_SLAM/src/robot/date/Test_MT_C1_Wx0.txt");//観測残差の確認
      ...
      for(int i=0;i<depth_point_prev_ok;i++){
        ...
        if(0.83<max_val1[i]){//最小値がしきい値以下なら表示
          ...
          if(cropx<=Est_pixel[i].x&&Est_pixel[i].x<=640-cropx&&cropy<=Est_pixel[i].y&&Est_pixel[i].y<=480-cropy){
            ...
            Matching_Templ_curr[matchC_curr]=FeaturePoint_prev[i];//マッチングしたテンプレート画像
...
*/
      ...
      if(Tracking == true){
        ...
        if(markerIds.size() <= 0){
          ...
          else if(DMP_curr2_ok==0){
            for (int i = 0; i < DMP_curr_ok; i++) {
              for(int j = 0; j < EST_MP_ok; j++){
                length=sqrt((MP_curr_camera[i].x-MP_prev_camera[j].x)*(MP_curr_camera[i].x-MP_prev_camera[j].x)
                +(MP_curr_camera[i].y-MP_prev_camera[j].y)*(MP_curr_camera[i].y-MP_prev_camera[j].y)
                +(MP_curr_camera[i].z-MP_prev_camera[j].z)*(MP_curr_camera[i].z-MP_prev_camera[j].z));
                if(minlengh>length){
                  minMC=j;
                  minlengh=length;
                }
              }
              MP_curr_world[i].x=MP_prev_world[minMC].x+(MP_curr_camera[i].x-MP_prev_camera[minMC].x)*cos(-Est_RobotTH)-(MP_curr_camera[i].z-MP_prev_camera[minMC].z)*sin(-Est_RobotTH);
              MP_curr_world[i].y=MP_prev_world[minMC].y+(MP_curr_camera[i].y-MP_prev_camera[minMC].y);
              MP_curr_world[i].z=MP_prev_world[minMC].z+(MP_curr_camera[i].x-MP_prev_camera[minMC].x)*sin(-Est_RobotTH)+(MP_curr_camera[i].z-MP_prev_camera[minMC].z)*cos(-Est_RobotTH);

              Test_MT_C2_Cy0<<MP_prev_camera[minMC].x<<"\n";
              Test_MT_C2_Cx0<<MP_prev_camera[minMC].z<<"\n";
              Test_MT_C1_Cy0<<MP_curr_camera[i].x<<"\n";
              Test_MT_C1_Cx0<<MP_curr_camera[i].z<<"\n";
              Test_MT_C2_Wy0<<MP_prev_world[minMC].x<<"\n";
              Test_MT_C2_Wx0<<MP_prev_world[minMC].z<<"\n";
              Test_MT_C1_Wy0<<MP_curr_world[i].x<<"\n";
              Test_MT_C1_Wx0<<MP_curr_world[i].z<<"\n";
              Test_MT_kaisu0<<ALLrealsec<<"\n";

              std::cout <<"MP_prev_camera["<<minMC<<"]= "<<MP_prev_camera[minMC]<< std::endl;
              std::cout <<"MP_curr_camera["<<i<<"]= "<<MP_curr_camera[i]<< std::endl;
              std::cout <<"MP_prev_world["<<minMC<<"]= "<<MP_prev_world[minMC]<< std::endl;
              std::cout <<"MP_curr_world["<<i<<"]=  "<<MP_curr_world[i]<< std::endl;
            }
          }
          MP_curr_world.resize(DMP_curr_ok);//Depth取得可能数でリサイズ(マッチング中心世界座標)
    ...
    if(Act_RobotX>0||Act_RobotY>0||Act_RobotTH>0){//ラグの調整
      ...
      //Mt= (cv::Mat_<double>(2,2) << 
      //    ((1.000E-3*abs(Act_RobotV))+(0.000126947*abs(robot_odometry.twist.twist.angular.z)))/realsec, 0,
      //    0, ((3.30119E-6*abs(Act_RobotV))+(0.01745329*abs(robot_odometry.twist.twist.angular.z)))/realsec);
