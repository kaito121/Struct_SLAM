...
cv::Mat_<float> Wt[40] = cv::Mat_<float>::zeros(6, 6);
...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
  ...
  if (markerIds.size() > 0) {
    ...
    cv::Mat_<float>Ot0_1=cv::Mat_<float>(markerIds.size()*2, 4);
    cv::Mat_<float>xEst0=cv::Mat_<float>(3,markerIds.size()*2);//今観測したマーカー座標(初回外パラ推定用)
    cv::Mat_<float>xEst_prev0=cv::Mat_<float>(3,markerIds.size()*2);//一つ前に観測したマーカー座標（初回外パラ推定用)
    cv::Mat_<float>Gt[40]=cv::Mat_<float>(3, 6);
    ...
    if(kaisu!=0){
      if(KAL==false){//カルマン推定前はこっち
        for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          if(point[i][3]!=0){//最新要素が無いものを除外
            ...
            Pt0(kosuu,0)=point[i][0];//マーカーのカメラ座標(観測)
            Pt0(kosuu+1,0)=point[i][2];

            Ot0_1(kosuu,0)=point_prve[i][0],Ot0_1(kosuu,1)=point_prve[i][2],Ot0_1(kosuu,2)=1,Ot0_1(kosuu,3)=0;
            Ot0_1(kosuu+1,0)=point_prve[i][2],Ot0_1(kosuu+1,1)=-point_prve[i][0],Ot0_1(kosuu+1,2)=0,Ot0_1(kosuu+1,3)=1;

            xEst_prev0(0,kosuu/2)=point_prve[i][0];
            xEst_prev0(1,kosuu/2)=0;
            xEst_prev0(2,kosuu/2)=point_prve[i][2];

            xEst0(0,kosuu/2)=point[i][0];
            xEst0(1,kosuu/2)=0;
            xEst0(2,kosuu/2)=point[i][2];

            xEst0_prev_cl[i](0,0)=point_prve[i][0];//一つ前に観測したマーカーのカメラ座標（カルマン用)
            xEst0_prev_cl[i](1,0)=0;
        ...
        for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          if(point[i][3]!=0){//最新要素が無いものを除外
            ...
            if(point[i][3]==xEst_prev_clP[i]){//追跡可能かどうか
              ...
              Ot0_1(kosuu,0)=xEst_prev_cl[i](0,0),Ot0_1(kosuu,1)=xEst_prev_cl[i](2,0),Ot0_1(kosuu,2)=1,Ot0_1(kosuu,3)=0;
              Ot0_1(kosuu+1,0)=xEst_prev_cl[i](2,0),Ot0_1(kosuu+1,1)=-xEst_prev_cl[i](0,0),Ot0_1(kosuu+1,2)=0,Ot0_1(kosuu+1,3)=1;

              xEst_prev0(0,kosuu/2)=xEst_prev_cl[i](0,0);
              xEst_prev0(1,kosuu/2)=0;
              xEst_prev0(2,kosuu/2)=xEst_prev_cl[i](2,0);
      ...
      std::cout <<"Pt=\n"<<Pt<< std::endl;
      std::cout <<"Ot_1=\n"<<Ot_1<< std::endl;
      std::cout <<"xEst=\n"<<xEst<< std::endl;//これ使ってないかも
      std::cout <<"xEst_prev=\n"<<xEst_prev<< std::endl;//これも使ってないかも

      //すべてのマーカー座標を利用して最小二乗法外部パラメータ推定(初回)
      cv::Mat_<float> Ot_1T = cv::Mat_<float>(4, 4);
      Ot_1T=Ot_1.t()*Ot_1;
      Xp=Ot_1T.inv(cv::DECOMP_SVD)*Ot_1.t()*Pt;//Ot_1Tは正方行列なのでこれでいいと思う
      //Xp=Ot_1.inv(cv::DECOMP_SVD)*Pt;//擬似逆行列なのでこっちのほうが正しいかも？
      //std::cout <<"Ot_1T=\n"<<Ot_1T<< std::endl;
      ...
      //外部パラメータから角速度を求める
      float tan=Xp(1,0)/Xp(0,0);//tan=sin/cos
      ...
      Vt(0,0)=Xp(2,0)/realsec;//Vx
      Vt(1,0)=0;//Vy
      Vt(2,0)=Xp(3,0)/realsec;//Vz
      Vt(3,0)=0;
      Vt(4,0)=0;
      Vt(5,0)=0;
      std::cout <<"Vt=\n"<<Vt<< std::endl;//並進ベクトル
      //カルマン初期設定(カルマン初回のみ実行)
      if(KAL==false){              //ここ本来はいるけど今テストのために消している
         Wt[ALLMarker] = cv::Mat_<float>::zeros(6, 6);
         Mt_cl[ALLMarker] = cv::Mat_<float>::zeros(3, 3);//誤差共分散(予測式)
         //Pt_cl[ALLMarker] = cv::Mat_<float>::zeros(3, 3);//誤差共分散(更新式)
      ...
      float Xp,Yp;
      int Otcount=0;
      cv::Mat delta = cv::Mat_<float>::eye(2, 2);// 単位行列
      //cv::Mat_<float>Ot=cv::Mat_<float>(kosuu, 6);
      cv::Mat_<float>Ot[ALLMarker]=cv::Mat_<float>(2, 6);
      cv::Mat_<float>Ot2_1[ALLMarker]=cv::Mat_<float>(6, 6);
      cv::Mat_<float>Ot2_1_T[ALLMarker]=cv::Mat_<float>(6, 6);
      ...
      for(int i=0;i<ALLMarker;i++){
        if(xEst0_clP[i]==1){//要素がある時のみ実行
          ...
          Ot[i](0,0)=camera_info.K[0],Ot[i](0,1)=0,Ot[i](0,2)=-Xp,Ot[i](0,3)=-Xp*xEst0_prev_cl[i](1,0),Ot[i](0,4)=Xp*xEst0_prev_cl[i](0,0)+camera_info.K[0]*xEst0_prev_cl[i](2,0),Ot[i](0,5)=-camera_info.K[0]*xEst0_prev_cl[i](1,0);
          Ot[i](1,0)=0,Ot[i](1,1)=camera_info.K[0],Ot[i](1,2)=-Yp,Ot[i](1,3)=-Yp*xEst0_prev_cl[i](1,0)-camera_info.K[0]*xEst0_prev_cl[i](2,0),Ot[i](1,4)=-Yp*xEst0_prev_cl[i](0,0),Ot[i](1,5)=camera_info.K[0]*xEst0_prev_cl[i](0,0);

          //Ot(Otcount,0)=camera_info.K[0],Ot(Otcount,1)=0,Ot(Otcount,2)=-Xp,Ot(Otcount,3)=-Xp*xEst0_prev_cl[i](1,0),Ot(Otcount,4)=Xp*xEst0_prev_cl[i](0,0)+camera_info.K[0]*xEst0_prev_cl[i](2,0),Ot(Otcount,5)=-camera_info.K[0]*xEst0_prev_cl[i](1,0);
          //Ot(Otcount+1,0)=0,Ot(Otcount+1,1)=camera_info.K[0],Ot(Otcount+1,2)=-Yp,Ot(Otcount+1,3)=-Yp*xEst0_prev_cl[i](1,0)-camera_info.K[0]*xEst0_prev_cl[i](2,0),Ot(Otcount+1,4)=-Yp*xEst0_prev_cl[i](0,0),Ot(Otcount+1,5)=camera_info.K[0]*xEst0_prev_cl[i](0,0);
          ...
          Ot2_1[i]=Ot[i].t()*Ot[i];
          std::cout <<"Ot2_1[i="<<i<<"]=\n"<<Ot2_1[i]<< std::endl;
          Ot2_1_T[i]=Ot2_1[i].inv(cv::DECOMP_SVD); //ここの逆行列の計算ができない...
          //Ot2_1_T[i]=Ot2_1[i].inv(); //ここの逆行列の計算ができない...
          std::cout <<"Ot2_1_T[i="<<i<<"]=\n"<<Ot2_1_T[i]<< std::endl;
          //std::cout <<"Ot2_1_T[i="<<i<<"].inv(cv::DECOMP_SVD)=\n"<<Ot2_1_T[i].inv(cv::DECOMP_SVD)<< std::endl;

          Wt[i]=Ot2_1_T[i]*Ot[i].t()*delta*0.12*Ot[i]*Ot2_1_T[i].t();
          std::cout <<"Wt[i="<<i<<"]=\n"<<Wt[i]<< std::endl;
        }
      }
      //std::cout <<"Ot=\n"<<Ot<< std::endl;
      ...
      for(int i=0;i<ALLMarker;i++){
        if(xEst0_clP[i]==1){//要素がある時のみ実行
          Mt_cl[i]=Ft*Pt_cl[i]*Ft.t()+Gt[i]*Wt[i]*Gt[i].t();
          std::cout <<"Pt_cl[i="<<i<<"]=\n"<<Pt_cl[i]<< std::endl;
      ...
      for(int i=0;i<ALLMarker;i++){
        if(xEst0_clP[i]==1){//要素がある時のみ実行
          ...
          Kt_cl[i]=Mt_cl[i]*Ht_cl[i].t()*u_.inv();
