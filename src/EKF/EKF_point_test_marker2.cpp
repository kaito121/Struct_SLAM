...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
  ...
  if (markerIds.size() > 0) {
    ...
    if(KAL_Wait>1){//KAL_Waitが1より大きいとき実行
      if(KAL==false){//カルマン推定前はこっち
        for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          if(MC_point[i][3]!=0){//最新要素が無いものを除外
            ...
            //共分散の内部要素(観測値ー前の推定値)
            //delta0(kosuu,0)=(xEst0_cl[i](0,0)-xEst0_prev_cl[i](0,0))*0.000226238;
            //delta0(kosuu+1,0)=(xEst0_cl[i](1,0)-xEst0_prev_cl[i](1,0))*0.000809871;
            //delta0(kosuu+2,0)=(xEst0_cl[i](2,0)-xEst0_prev_cl[i](2,0))*0.006797738;
      ...
      if (swap_on==false) {
        ...
        //マーカーを使用した外部パラメータ推定
        ALL_At=cv::Mat_<float>(kosuu, 6);
        ALL_Et=cv::Mat_<float>(kosuu, 1);
        At.rowRange(0, kosuu).copyTo(ALL_At.rowRange(0, kosuu));
	      Et.rowRange(0, kosuu).copyTo(ALL_Et.rowRange(0, kosuu));
      }
      ...
      for(int i=0;i<ALLMarker;i++){
        if(xEst0_clP[i]==1){//要素がある時のみ実行
          ...
          //共分散の内部要素(今の予測推定値ー前の推定値)(予測推定値を使うことで線形化)
          delta0(kosuu,0)=(xEst_cl[i](0,0)-xEst0_prev_cl[i](0,0))*0.000226238;
          delta0(kosuu+1,0)=(xEst_cl[i](1,0)-xEst0_prev_cl[i](1,0))*0.000809871;
          delta0(kosuu+2,0)=(xEst_cl[i](2,0)-xEst0_prev_cl[i](2,0))*0.006797738;
          kosuu=kosuu+3;
        }
      }
      ...
      for(int i=0;i<kosuu;i++){//共分散の対角化
        ...
        if(i%3==0){eyetemp=eyetemp*delta(i,0)*delta(i,0);}
        else if(i%3==1){eyetemp=eyetemp*delta(i,0)*delta(i,0);}
        else if(i%3==2){eyetemp=eyetemp*delta(i,0)*delta(i,0);}
  ...
  if(kaisu!=0){
    //マーカーが観測されなかった時もしくは再観測初回（特徴点のみで外パラ推定)
    if(markerIds.size() <= 0||KAL_Wait<=1){
        //特徴点1を使用した外部パラメータ推定)
        //std::cout <<"特徴点1を使用した外部パラメータ推定"<<std::endl;
      ...
      std::cout <<"kaisu="<<kaisu<< std::endl;//並進ベクトル
