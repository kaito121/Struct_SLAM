...
cv::Mat_<float> ALL_Tt=cv::Mat_<float>(6, 1);//教科書運動パラメータ(マーカー+特徴点)
...
ofstream outputfileTF("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/EKF_TF/TFposition.txt");//出力ファイルパス
ofstream outputfileALL_Vt("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/EKF_TF/TF_ALL_Vt.txt");//出力ファイルパス
...
//
      ...
      camera_prmX = camera_prmX-PT_Tt(5,0)*camera_prmY-PT_Tt(4,0)*camera_prmZ+PT_Tt(2,0);
      camera_prmY = -PT_Tt(5,0)*camera_prmX+camera_prmY+PT_Tt(3,0)*camera_prmZ+PT_Tt(0,0);
      camera_prmZ = PT_Tt(4,0)*camera_prmX-PT_Tt(5,0)*camera_prmY+camera_prmZ+PT_Tt(1,0);
      //カメラ位置
      camera_base_pose.position.x = camera_prmZ;//赤(tf:X,画像:Z)
      camera_base_pose.position.y = camera_prmX;//緑(tf:Y,画像:X)
      camera_base_pose.position.z = 0;//青(tf:Z,画像:Y)
