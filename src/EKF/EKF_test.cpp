...
//カルマンフィルタ初期設定---------------------------------------------------------------------------------------------------------
 if(kaisu==0){
  ...
  y = cv::Mat_<float>::zeros(1, 2);//観測残渣y
  S = cv::Mat_<float>::zeros(2, 2);
  K = cv::Mat_<float>::zeros(3, 3);//カルマンゲイン

  Ft = cv::Mat_<float>::zeros(3, 3);//Ftヤコビアン行列
  Ht = cv::Mat_<float>::zeros(2, 3);//Ftヤコビアン行列
...
//状態方程式
uR = u[0] + normW1(mt)*0;//右のタイヤ(雑音計算）
uL = u[1] + normW1(mt)*0;//左のタイヤ(雑音計算)
...
z[0]=sqrt((LM[0]-xAct[0])*(LM[0]-xAct[0]) + (LM[1]-xAct[1])*(LM[1]-xAct[1])) + normV1(mt)*0;
z[1]=atan2(LM[1]-xAct[1],LM[0]-xAct[0]) - xAct[2] + normV2(mt)*0;
...
cv::line(img_dst,cv::Point(xACT.at<float>(0)*5,xACT.at<float>(1)*5),cv::Point(z[0]*cos(z[1]),z[0]*sin(z[1])),cv::Scalar(0,0,0), 1, cv::LINE_AA);
cv::line(img_dst,cv::Point(xEST.at<float>(0)*5,xEST.at<float>(1)*5),cv::Point(LM[0],LM[1]),cv::Scalar(10,0,255), 1, cv::LINE_AA);
...
//cv::line(img_dst,cv::Point(xEST.at<float>(0)*5,xEST.at<float>(1)*5),cv::Point(zEst[0]*cos(zEst[1]),zEst[0]*sin(zEst[1])),cv::Scalar(0,0,255), 1, cv::LINE_AA);
