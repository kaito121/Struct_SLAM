...
ofstream outputfileTF("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/no_EKF_TF/TFposition.txt");//出力ファイルパス
ofstream outputfileALL_Vt("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/no_EKF_TF/TF_ALL_Vt.txt");//出力ファイルパス
ofstream outputfile_worldC("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/no_EKF_TF/worldC.txt");//出力ファイルパス
ofstream outputfile_WorldC_position("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/no_EKF_TF/WorldC_position.txt");//出力ファイルパス
...
cv::Mat_<float>LMW[40]=cv::Mat_<float>(3, 1);//マーカーの世界座標
//cv::Mat_<float>worldC[40]=cv::Mat_<float>(3, 1);//カメラの世界座標(まずはマーカー分用意してそこから中央値を求めそれを世界座標にする)
//cv::Mat_<float>worldC_prve[40]=cv::Mat_<float>(3, 1);
//cv::Mat_<float>WorldC[40]=cv::Mat_<float>(3, 1);
//cv::Mat_<float>WorldC_position[40]=cv::Mat_<float>::zeros(3, 1);
//メモ:カメラの世界座標は平均、中央値、最頻値などいろいろな方法を探る必要がある
cv::Mat_<float>worldC=cv::Mat_<float>(3, 1);//カメラの世界座標(まずはマーカー分用意してそこから中央値を求めそれを世界座標にする)
cv::Mat_<float>worldC_prve=cv::Mat_<float>(3, 1);
cv::Mat_<float>WorldC=cv::Mat_<float>(3, 1);
cv::Mat_<float>WorldC_position=cv::Mat_<float>::zeros(3, 1);
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg, const sensor_msgs::CameraInfo::ConstPtr& cam_info)
{
  ...
  //マーカーの世界座標登録(向きは画像姿勢)(TFの姿勢ではない)
  LMW[3] = (cv::Mat_<float>(3,1) <<-0.43, 0.29, 1.05);
  LMW[6] = (cv::Mat_<float>(3,1) <<0.57, 0.29, 2.09);
  LMW[11] = (cv::Mat_<float>(3,1) <<-0.60, 0.28, 3.68);
  LMW[27] = (cv::Mat_<float>(3,1) <<0.41, 0.28, 5.30);
...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
  ...
  float mini=1000;//マーカーとの最小距離を見つけるための要素
  int miniID;
  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      if(depth[markerIds.at(i)]>0&&depth[markerIds.at(i)]<10000){
        ...
        //最もカメラとの距離が近いマーカーを代表マーカーとして使用する（今後は今後は最低距離1mとかにしたほうがもしかしたらよいかも？)
        if(mini>point[markerIds.at(i)][2]){
          mini=point[markerIds.at(i)][2];
          miniID=markerIds.at(i);
        }

        //std::cout << "マーカーのカメラ座標:point["<<markerIds.at(i)<<"]={"<< point[markerIds.at(i)][0] <<","<<point[markerIds.at(i)][1]<<","<<point[markerIds.at(i)][2]<<"}"<< std::endl;
        //std::cout << "マーカーの世界座標　:LMW["<<markerIds.at(i)<<"] = {"<< LMW[markerIds.at(i)](0,0) <<","<<LMW[markerIds.at(i)](1,0)<<","<<LMW[markerIds.at(i)](2,0)<<"}"<< std::endl;
        ////世界座標原点からカメラまでのベクトル
        //worldC[markerIds.at(i)](0,0)=LMW[markerIds.at(i)](0,0)-point[markerIds.at(i)][0];
        //worldC[markerIds.at(i)](1,0)=LMW[markerIds.at(i)](1,0)-point[markerIds.at(i)][1];
        //worldC[markerIds.at(i)](2,0)=LMW[markerIds.at(i)](2,0)-point[markerIds.at(i)][2];
        //std::cout << "カメラの世界座標:worldC["<<markerIds.at(i)<<"] = {"<< worldC[markerIds.at(i)](0,0) <<","<<worldC[markerIds.at(i)](1,0)<<","<<worldC[markerIds.at(i)](2,0)<<"}"<< std::endl;
        //outputfile_worldC<<"カメラの世界座標:worldC["<<markerIds.at(i)<<"] = {"<< worldC[markerIds.at(i)](0,0) <<","<<worldC[markerIds.at(i)](1,0)<<","<<worldC[markerIds.at(i)](2,0)<<"}"<<"\n";
        //if(kaisu!=0){
        //  std::cout << "カメラの世界座標:worldC_prve["<<markerIds.at(i)<<"] = {"<< worldC_prve[markerIds.at(i)](0,0) <<","<<worldC_prve[markerIds.at(i)](1,0)<<","<<worldC_prve[markerIds.at(i)](2,0)<<"}"<< std::endl;
        //  WorldC[markerIds.at(i)](0,0)=worldC[markerIds.at(i)](0,0)-worldC_prve[markerIds.at(i)](0,0);
        //  WorldC[markerIds.at(i)](1,0)=worldC[markerIds.at(i)](1,0)-worldC_prve[markerIds.at(i)](1,0);
        //  WorldC[markerIds.at(i)](2,0)=worldC[markerIds.at(i)](2,0)-worldC_prve[markerIds.at(i)](2,0);
        //  //outputfile_WorldC_position<<"WorldC["<<markerIds.at(i)<<"] = {"<< WorldC[markerIds.at(i)](0,0) <<","<<WorldC[markerIds.at(i)](1,0)<<","<<WorldC[markerIds.at(i)](2,0)<<"}"<<"\n";
        //  WorldC_position[markerIds.at(i)]=WorldC_position[markerIds.at(i)]+WorldC[markerIds.at(i)];
        //  std::cout << "カメラの世界座標:WorldC_position["<<markerIds.at(i)<<"] = {"<< WorldC_position[markerIds.at(i)](0,0) <<","<<WorldC_position[markerIds.at(i)](1,0)<<","<<WorldC_position[markerIds.at(i)](2,0)<<"}"<< std::endl;
        //  outputfile_WorldC_position<<"カメラの世界座標:WorldC_position["<<markerIds.at(i)<<"] = {"<< WorldC_position[markerIds.at(i)](0,0) <<","<<WorldC_position[markerIds.at(i)](1,0)<<","<<WorldC_position[markerIds.at(i)](2,0)<<"}"<<"\n";
        //}
      }
    }
    //最もカメラとの距離が近いマーカーを代表マーカーとして使用する（今後は今後は最低距離1mとかにしたほうがもしかしたらよいかも？)
    std::cout << "マーカーのカメラ座標:point["<<miniID<<"]={"<< point[miniID][0] <<","<<point[miniID][1]<<","<<point[miniID][2]<<"}"<< std::endl;
    std::cout << "マーカーの世界座標　:LMW["<<miniID<<"] = {"<< LMW[miniID](0,0) <<","<<LMW[miniID](1,0)<<","<<LMW[miniID](2,0)<<"}"<< std::endl;
    //世界座標原点からカメラまでのベクトル
    worldC(0,0)=LMW[miniID](0,0)-point[miniID][0];
    worldC(1,0)=LMW[miniID](1,0)-point[miniID][1];
    worldC(2,0)=LMW[miniID](2,0)-point[miniID][2];
    std::cout << "カメラの世界座標:worldC = {"<< worldC(0,0) <<","<<worldC(1,0)<<","<<worldC(2,0)<<"}"<< std::endl;
    //outputfile_worldC<<"マーカーの世界座標　:LMW["<<miniID<<"] = {"<< LMW[miniID](0,0) <<","<<LMW[miniID](1,0)<<","<<LMW[miniID](2,0)<<"}"<<"\n";
    //outputfile_worldC<<"マーカーのカメラ座標:point["<<miniID<<"]={"<< point[miniID][0] <<","<<point[miniID][1]<<","<<point[miniID][2]<<"}"<<"\n";
    outputfile_worldC<<"カメラの世界座標:worldC["<<miniID<<"] = {"<< worldC(0,0) <<","<<worldC(1,0)<<","<<worldC(2,0)<<"}"<<"\n";
    if(kaisu!=0){
      std::cout << "カメラの世界座標:worldC_prve = {"<< worldC_prve(0,0) <<","<<worldC_prve(1,0)<<","<<worldC_prve(2,0)<<"}"<< std::endl;
      WorldC=worldC-worldC_prve;
      //outputfile_WorldC_position<<"WorldC = {"<< WorldC(0,0) <<","<<WorldC(1,0)<<","<<WorldC(2,0)<<"}"<<"\n";
      WorldC_position=WorldC_position+WorldC;
      std::cout << "カメラの世界座標:WorldC_position = {"<< WorldC_position(0,0) <<","<<WorldC_position(1,0)<<","<<WorldC_position(2,0)<<"}"<< std::endl;
      outputfile_WorldC_position<<"カメラの世界座標:WorldC_position = {"<< WorldC_position(0,0) <<","<<WorldC_position(1,0)<<","<<WorldC_position(2,0)<<"}"<<"\n";
    }
    ...
    if(kaisu!=0){
      ...
      for(int i=0;i<ALLMarker;i++){
        if(xEst0_clP[i]==1){//要素がある時のみ実行
          ...
          xEst_prev_cl[i]=xEst0_cl[i];//現在の推定を保存
          xEst_prev_clP[i]=1;
      ...
      //outputfile11<<"xEst0_prev_cl[11]="<<xEst0_prev_cl[11](0,0)<<", xEst0_cl[11]="<<xEst0_cl[11](0,0)<<",外部パラメータ推定:Tt="<<Tt(0,0)<<",  "<<Tt(3,0)<<",外部パラメータ推定:ALL_Tt="<<ALL_Tt(0,0)<<",  "<<ALL_Tt(3,0)<<"\n";
      //outputfile11<<"                  "<<xEst0_prev_cl[11](1,0)<<"               "<<xEst0_cl[11](1,0)<<"                       "<<Tt(1,0)<<",  "<<Tt(4,0)<<"                           "<<ALL_Tt(1,0)<<",  "<<ALL_Tt(4,0)<<"\n";
      //outputfile11<<"                  "<<xEst0_prev_cl[11](2,0)<<"                   "<<xEst0_cl[11](2,0)<<"                          "<<Tt(2,0)<<",  "<<Tt(5,0)<<"                           "<<ALL_Tt(2,0)<<",  "<<ALL_Tt(5,0)<<"\n";
    ...
    //for(int i=0;i<markerIds.size();i++){
      ...
      std::cout << "tf特徴点のカメラ座標:point[3][0]={x="<< xEst0_prev_cl[3](2,0) <<",y="<<-xEst0_prev_cl[3](0,0)<<",z="<<-xEst0_prev_cl[3](1,0)<<"}"<< std::endl;
      ...
      std::cout << "tf特徴点のカメラ座標:point[6][0]={x="<< xEst0_prev_cl[6](2,0) <<",y="<<-xEst0_prev_cl[6](0,0)<<",z="<<-xEst0_prev_cl[6](1,0)<<"}"<< std::endl;
      ...
      std::cout << "tf特徴点のカメラ座標:point[11][0]={x="<< xEst0_prev_cl[11](2,0) <<",y="<<-xEst0_prev_cl[11](0,0)<<",z="<<-xEst0_prev_cl[11](1,0)<<"}"<< std::endl;
      ...
      std::cout << "tf特徴点のカメラ座標:point[27][0]={x="<< xEst0_prev_cl[27](2,0) <<",y="<<-xEst0_prev_cl[27](0,0)<<",z="<<-xEst0_prev_cl[27](1,0)<<"}"<< std::endl;
  ...
  //ここがカメラの姿勢部分
    ...
    camera_base_pose.position.x = camera_base_pose.position.x+WorldC(2,0);//赤(xは進行方向)
    camera_base_pose.position.y = camera_base_pose.position.y+WorldC(0,0);//緑
    camera_base_pose.position.z = camera_base_pose.position.z+WorldC(1,0);//青
    camera_base_pose.orientation.w = 0.1;
    //outputfileALL_Vt<<"X:WorldC_position(2,0)="<<WorldC_position(2,0)<<" Y:WorldC_position(0,0)="<<WorldC_position(0,0)<<" Z:WorldC_position(1,0)="<<WorldC_position(1,0)<<"\n";
  ...
  for(int i=0;i<markerIds.size();i++){
    ...
    //worldC_prve[markerIds.at(i)] = worldC[markerIds.at(i)].clone();//現在のカメラの世界座標を保存
  }
    worldC_prve = worldC.clone();//現在のカメラの世界座標を保存
