...
cv::Mat F,H,u1,u2,W,V,Q,R,xDes[3],xAct[3],xEst[3],P,z,I,y,K,S;
//cv::Mat uR,uL,vAct,vDes,omegaAct,omegaDes,LM1,LM2;

float u[2],LM[2],uR,uL,vAct,vDes,omegaAct,omegaDes;
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
    ...
    std::normal_distribution<> normW1(0.1, 0.5);// 平均0.04, 分散値0.04の正規分布
    std::normal_distribution<> normW2(0.1, 0.5);// 平均0.04, 分散値0.04の正規分布

    std::normal_distribution<> normV1(0.2, 0.5);// 平均0.09, 分散値0.04の正規分布
    std::normal_distribution<> normV2(0.2, 0.5);// 平均0.09, 分散値0.04の正規分布
...
//カルマンフィルタ初期設定---------------------------------------------------------------------------------------------------------
 if(kaisu==0){
  ...
  u1 = (cv::Mat_<float>(2, 1) <<0.2,0.1);//入力1
  u2 = (cv::Mat_<float>(2, 1) <<0.4,0.2);//入力2

  LM1 = (cv::Mat_<float>(2, 1) <<10,5);//ランドマークポジション
  LM2 = (cv::Mat_<float>(2, 1) <<10,5);//ランドマークポジション
  ...
  xDes[3] = cv::Mat_<float>::zeros(2, 1);//目標指令状態
  xAct[3] = cv::Mat_<float>::zeros(2, 1);//雑音の影響を考慮した実際の状態
  xEst[3] = cv::Mat_<float>::zeros(2, 1);//推定状態

  z[2] = cv::Mat_<float>::zeros(2, 1);//出力方程式
  ...
  //uR = cv::Mat_<float>::zeros(2, 1);
  //uL = cv::Mat_<float>::zeros(2, 1);

  //vAct = cv::Mat_<float>::zeros(2, 1);
  //vDes = cv::Mat_<float>::zeros(2, 1);

  //omegaAct = cv::Mat_<float>::zeros(2, 1);
  //omegaDes = cv::Mat_<float>::zeros(2, 1);
...
std::cout <<"11111111111111111"<< std::endl;
 cv::Mat normWT1 = (cv::Mat_<float>(2,1) << normW1(mt), normW1(mt)); 
 cv::Mat normWT2 = (cv::Mat_<float>(2,1) << normW2(mt), normW2(mt)); 

 cv::Mat normVT1 = (cv::Mat_<float>(2,1) << normV1(mt), normV1(mt)); 
 cv::Mat normVT2 = (cv::Mat_<float>(2,1) << normV2(mt), normV2(mt)); 
...
uR = u1 + normWT1;//右のタイヤ(雑音計算）
uL = u2 + normWT2;//左のタイヤ(雑音計算)

vAct = (uR+uL)/2;//雑音あり
vDes = (u1+u2)/2;//雑音なし

omegaAct = (uR-uL)/(2*d);//雑音あり
omegaDes = (u1-u2)/(2*d);//雑音なし

//雑音の影響を考慮した実際の状態
xAct[2] = xAct[2] + omegaAct * Ts;
xAct[0] = xAct[0] + vAct * Ts *cos(xAct[2]+omegaAct*Ts/2);
xAct[1] = xAct[1] + vAct * Ts *sin(xAct[2]+omegaAct*Ts/2);
...
//出力方程式
Z[0]=sqrt((LM1-xAct[1])*(LM1-xAct[1]) + (LM2-xAct[2])*(LM2-xAct[2])) + normVT1;
Z[1]=atan2(LM2-xAct[2],LM1-xAct[1]) - xAct[3] + normVT2;
