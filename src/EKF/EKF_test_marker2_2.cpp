...
int xEst0_prev_cl0[40*4][3],xEst0_cl0[40*4][3],xEst_prev_cl0[40*4][3],xEst_cl0[40*4][3];//マーカーのIDとコーナーIDを保存
float xEst_point[40][4][4];//カルマン推定結果の保存(ID対応化)
int xEst_point_i[40][4];//誤差共分散保存用対応i保存
cv::Mat_<float> xEst_point_Pt[40*4] = cv::Mat_<float>::zeros(3, 3);//誤差共分散の保存(ID対応化)
    ...
    if(kaisu!=0){
        ...
        for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          for(int j=0;j<4;j++){
            if(point[i][j][3]!=0){//最新要素が無いものを除外
              ...
              xEst0_prev_cl0[kosuu/2][0]=i;//一つ前に観測したマーカーのマーカーIDを保存
              xEst0_prev_cl0[kosuu/2][1]=j;//一つ前に観測したマーカーのコーナーIDを保存

              xEst0_cl[kosuu/2](0,0)=point[i][j][0];//今観測したマーカーのカメラ座標(カルマン用)
              xEst0_cl[kosuu/2](1,0)=0;
              xEst0_cl[kosuu/2](2,0)=point[i][j][2];
              xEst0_cl0[kosuu/2][0]=i;//今観測したマーカーのマーカーIDを保存
              xEst0_cl0[kosuu/2][1]=j;//今観測したマーカーのコーナーIDを保存
      ...
      std::cout << "カルマン推定後"<< std::endl;
        for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          for(int j=0;j<4;j++){
            if(point[i][j][3]!=0){//最新要素が無いものを除外
              if(point[i][j][3]==xEst_point[i][j][3]){//追跡可能かどうか
                std::cout << "point     ["<<i<<"]["<<j<<"]={"<< point[i][j][0] <<","<<point[i][j][1]<<","<<point[i][j][2]<<","<<point[i][j][3]<<"}"<< std::endl;
                std::cout << "xEst_point["<<i<<"]["<<j<<"]={"<< xEst_point[i][j][0] <<","<<xEst_point[i][j][1]<<","<<xEst_point[i][j][2]<<","<<xEst_point[i][j][3]<<"}"<< std::endl;
                Pt0(kosuu,0)=point[i][j][0];//マーカーのカメラ座標(観測)
                Pt0(kosuu+1,0)=point[i][j][2];

                Ot0_1(kosuu,0)=xEst_point[i][j][0],Ot0_1(kosuu,1)=xEst_point[i][j][2],Ot0_1(kosuu,2)=1,Ot0_1(kosuu,3)=0;
                Ot0_1(kosuu+1,0)=xEst_point[i][j][2],Ot0_1(kosuu+1,1)=-xEst_point[i][j][0],Ot0_1(kosuu+1,2)=0,Ot0_1(kosuu+1,3)=1;

                xEst_prev0(0,kosuu/2)=xEst_point[i][j][0];
                xEst_prev0(1,kosuu/2)=0;
                xEst_prev0(2,kosuu/2)=xEst_point[i][j][2];

                xEst0(0,kosuu/2)=point[i][j][0];
                xEst0(1,kosuu/2)=0;
                xEst0(2,kosuu/2)=point[i][j][2];

                xEst0_prev_cl[kosuu/2](0,0)=xEst_point[i][j][0];//一つ前に推定したマーカーのカメラ座標（カルマン用)
                xEst0_prev_cl[kosuu/2](1,0)=0;
                xEst0_prev_cl[kosuu/2](2,0)=xEst_point[i][j][2];
                xEst0_prev_cl0[kosuu/2][0]=i;//一つ前に推定したマーカーのマーカーIDを保存
                xEst0_prev_cl0[kosuu/2][1]=j;//一つ前に推定したマーカーのコーナーIDを保存

                Pt_cl[kosuu/2]=xEst_point_Pt[xEst_point_i[i][j]];//一つ前に推定した誤差共分散の代入
      ...
      std::cout << "xEst0_prev_cl0["<<i<<"][0]="<<xEst0_prev_cl0[i][0]<< std::endl;
      std::cout << "xEst0_prev_cl0["<<i<<"][1]="<<xEst0_prev_cl0[i][1]<< std::endl;
      std::cout << "xEst0_cl["<<i<<"]=\n"<<xEst0_cl[i]<< std::endl;
      std::cout << "xEst0_cl0["<<i<<"][0]="<<xEst0_cl0[i][0]<< std::endl;
      std::cout << "xEst0_cl0["<<i<<"][1]="<<xEst0_cl0[i][1]<< std::endl;
      }
      ...
      //if(kaisu<2){                //ここ本来はいるけど今テストのために消している
         ...
         Mt_cl[kosuu] = cv::Mat_<float>::zeros(3, 3);//誤差共分散(予測式)
         //Pt_cl[kosuu] = cv::Mat_<float>::zeros(3, 3);//誤差共分散(更新式)
         Yt_cl[kosuu] = cv::Mat_<float>::zeros(2, 1);//観測残差
         Ht_cl[kosuu] = cv::Mat_<float>::zeros(2, 3);//Htヤコビアン行列
         Ut_cl[kosuu] = cv::Mat_<float>::zeros(2, 1);//共分散
         Kt_cl[kosuu] = cv::Mat_<float>::zeros(3, 2);//カルマンゲイン
      ...
      for(int i=0;i<kosuu/2;i++){
        ...
        xEst_cl0[i][0]=xEst0_prev_cl0[i][0];//対応マーカーIDを保存
        xEst_cl0[i][1]=xEst0_prev_cl0[i][1];//対応コーナーIDを保存
        std::cout << "xEst_cl0["<<i<<"][0]="<<xEst_cl0[i][0]<< std::endl;
        std::cout << "xEst_cl0["<<i<<"][1]="<<xEst_cl0[i][1]<< std::endl;
      ...
      for(int i=0;i<kosuu/2;i++){
        ...
        xEst_prev_cl0[i][0]=xEst_cl0[i][0];//対応マーカーIDを保存
        xEst_prev_cl0[i][1]=xEst_cl0[i][1];//対応コーナーIDを保存
        std::cout << "xEst_prev_cl0["<<i<<"][0]="<<xEst_prev_cl0[i][0]<< std::endl;
        std::cout << "xEst_prev_cl0["<<i<<"][1]="<<xEst_prev_cl0[i][1]<< std::endl;
        xEst_point[xEst_cl0[i][0]][xEst_cl0[i][1]][0]=xEst_cl[i](0,0);//カルマン推定結果を保存(ID対応化)
        xEst_point[xEst_cl0[i][0]][xEst_cl0[i][1]][1]=xEst_cl[i](1,0);
        xEst_point[xEst_cl0[i][0]][xEst_cl0[i][1]][2]=xEst_cl[i](2,0);
        xEst_point[xEst_cl0[i][0]][xEst_cl0[i][1]][3]=1;
        xEst_point_i[xEst_cl0[i][0]][xEst_cl0[i][1]]=i;//誤差共分散保存用対応i保存
        xEst_point_Pt[xEst_point_i[xEst_cl0[i][0]][xEst_cl0[i][1]]]=Pt_cl[i];//誤差共分散の保存(ID対応化)
