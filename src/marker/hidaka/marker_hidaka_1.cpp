  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      for(int j=0;j<4;j++){
        ...
        depth[j] = depthimage.at<float>(pixel[markerIds.at(i)][j][0],pixel[markerIds.at(i)][j][1]);//ここでエラーが起こってる(原因不明時々segment fualtになる)
    ...
    if(kaisu!=0){
      for(int i=0;i<markerIds.size();i++){
        ...
        //for(int j=0;j<depth_ok[markerIds.at(i)];j++){//これを考慮する必要がある
          ...
          Ot_1(kosuu,0)=point[markerIds.at(i)][j][0],Ot_1(kosuu,1)=point[markerIds.at(i)][j][2],Ot_1(kosuu,2)=1,Ot_1(kosuu,3)=0;
          Ot_1(kosuu+1,0)=point[markerIds.at(i)][j][2],Ot_1(kosuu+1,1)=-point[markerIds.at(i)][j][0],Ot_1(kosuu+1,2)=0,Ot_1(kosuu+1,3)=1;
