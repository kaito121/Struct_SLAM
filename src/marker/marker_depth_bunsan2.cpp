...
//20210924 マーカーの全範囲のDepth情報を取得しその分散を出力するプログラム
...
ofstream outputfile1("/home/fuji/catkin_ws/src/Struct_SLAM/src/marker/Depth1.txt");
...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
  ...
  float depth[50][1000][500],SUMdepth[50],pixelkosuu[50],AVEdepth[50];
  ...
  if (markerIds.size() > 0) {
    ...
    SUMdepth[kaisu]=0,pixelkosuu[kaisu]=0;
    //マーカーの範囲選択(ID.27)
    for(int i=MarkerC[27][0][0];i<=MarkerC[27][1][0];i++){
     for(int j=MarkerC[27][0][1];j<=MarkerC[27][2][1];j++){
       depth[kaisu][i][j] = depthimage.at<float>(cv::Point(i,j));//範囲内の全Depthを取得
       SUMdepth[kaisu]=SUMdepth[kaisu]+depth[kaisu][i][j];
       pixelkosuu[kaisu]=pixelkosuu[kaisu]+1;
       outputfile1<<depth[kaisu][i][j]<<"\n";
      std::cout <<"depth["<<kaisu<<"]["<<i<<"]["<<j<<"]="<< depth[kaisu][i][j] << std::endl;
		  cv::circle(imageCopy, cv::Point(i,j), 3, Scalar(0,255,0),  -1, cv::LINE_AA);//緑点
      
     }
    }
    AVEdepth[kaisu]=SUMdepth[kaisu]/pixelkosuu[kaisu];
