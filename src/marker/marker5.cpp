...
//marker5:複数のマーカー対応化
//複数のマーカー対応かができたら次は拡張カルマンフィルタで外部パラメータの推定精度を向上させる
...
static int MID[50];//各IDでマーカーが出てきた回数(初回のマーカーIDの時は世界座標保存)
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg, const sensor_msgs::CameraInfo::ConstPtr& cam_info)
{
  ...
  float pixel[50][4][2],depth,point[50][4][3],x,y,r2,f,ux,uy;
  float MLength[50],MAngle[50];//マーカーまでの距離と角度
  //float pixel[100][2],depth,point[100][3];
  cv::Mat_<float> MarkerCameraALL=cv::Mat_<float>(4*(markerIds.size()), 4);//マーカーのカメラ座標の行列
  cv::Mat_<float> MarkerWorldALL=cv::Mat_<float>(4*(markerIds.size()), 3);//マーカーの世界座標の行列
  int kosuu=0;
  ...
  if (markerIds.size() > 0) {
    ...
    for(int i=0;i<markerIds.size();i++){
      ...
      for(int j=0;j<4;j++){
        ...
        depth = depthimage.at<float>(MarkerC[markerIds.at(i)][j][0],MarkerC[markerIds.at(i)][j][1]);
        ...
        MarkerCamera[markerIds.at(i)](j,0) = depth * x/1000;//メートル表示変換
        MarkerCamera[markerIds.at(i)](j,1) = depth * y/1000;
        MarkerCamera[markerIds.at(i)](j,2) = depth/1000;
        MarkerCamera[markerIds.at(i)](j,3)=1;

        MarkerCameraALL(kosuu,0)=MarkerCamera[markerIds.at(i)](j,0);//マーカーのカメラ座標(観測)
        MarkerCameraALL(kosuu,1)=MarkerCamera[markerIds.at(i)](j,1);
        MarkerCameraALL(kosuu,2)=MarkerCamera[markerIds.at(i)](j,2);
        MarkerCameraALL(kosuu,3)=MarkerCamera[markerIds.at(i)](j,3);

        if(MID[markerIds.at(i)]==0){//各IDで初めてマーカーが観測された時そのマーカーを世界座標とする
          MarkerWorld[markerIds.at(i)](j,0)=MarkerCamera[markerIds.at(i)](j,0);
          MarkerWorld[markerIds.at(i)](j,1)=MarkerCamera[markerIds.at(i)](j,1);
          MarkerWorld[markerIds.at(i)](j,2)=MarkerCamera[markerIds.at(i)](j,2);
        }
        
        if(kaisu2==0){//マーカーの世界座標配列化(初回はカメラ座標＝世界座標)
          MarkerWorldALL(kosuu,0)=MarkerWorld[markerIds.at(i)](j,0);
          MarkerWorldALL(kosuu,1)=MarkerWorld[markerIds.at(i)](j,1);
          MarkerWorldALL(kosuu,2)=MarkerWorld[markerIds.at(i)](j,2);
          //MarkerWorldALL(kosuu,0)=0.3;
          //MarkerWorldALL(kosuu,1)=0.3;
          //MarkerWorldALL(kosuu,2)=1.43;
        }
        kosuu++;
      }
      std::cout <<" MarkerCamera["<<markerIds.at(i)<<"]=\n"<< MarkerCamera[markerIds.at(i)] << std::endl;//マーカーのカメラ座標(観測)
      MID[markerIds.at(i)]=MID[markerIds.at(i)]+1;//各IDでマーカーが出てきた回数
    }
  
    std::cout <<" MarkerCameraALL=\n"<< MarkerCameraALL<< std::endl;//全マーカーのカメラ座標(観測)
    std::cout <<" MarkerWorldALL=\n"<< MarkerWorldALL<< std::endl;//全マーカーのカメラ座標(観測)
    ...
    //マーカーの世界座標配列化
    //MarkerCameraALL.rowRange(0, 2).copyTo(MarkerWorldALL.rowRange(0, 2));
