...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
  ...
  cv::Mat RGBimage,depthimage,image;//opencvの画像
	...
	for (int i = 0; i < points_curr.size(); i++) {
    	...
    	std::cout <<"画像座標(curr)["<<i<<"]="<<points_curr[i]<< std::endl;//特徴点の座標
		std::cout <<"画像座標(prev)["<<i<<"]="<<points_prev[i]<< std::endl;//特徴点の座標
  ...
  std::cout << "内部パラメータ: A=\n" << A << std::endl;
  cv::Mat_<double> ScreenP[points_curr.size()] = cv::Mat_<double>(3, 1);//点Pの画像座標系（u1,v1,1)※斉次化
  cv::Mat_<double> CameraP[points_curr.size()] = cv::Mat_<double>(3, 1);//点Pのカメラ座標系（xc,yc,zc)
