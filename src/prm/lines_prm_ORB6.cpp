...
std::string win_depth4 = "depth";//深度画像（修正前）
...
//
    ...
    //Screenp[NotZeroL] << round(points_curr[i].x+(RGBimage.size().width/2)), round(points_curr[i].y+(RGBimage.size().height/2)), 1;//斉次化(整数化四捨五入(round関数))+画像正規化座標変換
