  ...
  double optcurrx,optcurry,optprevx,optprevy;
    ...
    std::cout <<"cv::norm(points_prev["<<i<<"] - points_curr["<<i<<"])="<<pointnorm[i]<< std::endl;//特徴点の座標
		...
		if(i==10){cv::circle(img_dst, points_curr[i], 8, Scalar(100,255,200), -1, cv::LINE_AA);}
			...
			cv::circle(img_dst, points_curr[i], 6, c, -1, cv::LINE_AA);//今の座標情報
      ...
      Screenp[i] << round(points_curr[i].x), round(points_curr[i].y), 1;//斉次化(整数化四捨五入)
      std::cout << "斉次画像座標:Screenp["<<i<<"]=\n" << Screenp[i] << std::endl;

      Camerap[i]=Zs[i]*K.inv()*Screenp[i];//画像座標系→カメラ座標系座標変換
      std::cout << "特徴点のカメラ座標:Camerap["<<i<<"]=\n" << Camerap[i] << std::endl;
...
//-------------------------------------------------------------------------------------------------------------------    
    ...
    if(kaisu==0){WorldP=CameraP;}//開始時は世界座標とカメラ座標の原点は一致しているので世界座標から見た点Pの座標とカメラ座標から見た点Pの座標は一致
