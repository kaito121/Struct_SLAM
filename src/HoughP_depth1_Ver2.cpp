...
std::cout <<"lines1.size= "<<lines1.size()<< std::endl;
 ...
 /* 並び替えした数値を出力 */
  ...
  //for (int i=0; i<lines1.size(); ++i){ printf("lines[%d][1]=%f\n", i,lines1[i][1]); }
    ...
    if( B==lines1[i+1][1]){
        std::cout <<"lines1["<<i<<"]= 同じ数値 "<< std::endl;
        A[t][j]=lines1[i+1][1];//代入
        j=j+1;//配列カウント
        c[t]=c[t]+1;//要素数（同じ数値は何個あるか）
      }
    //前の番号と異なる数値
    else{
        std::cout <<"lines1["<<i<<"]= 異なる数値 "<< std::endl;
      ...
      cv::line(img_line,cv::Point(pt1x,pt1y),cv::Point(pt2x,pt2y),cv::Scalar(t*25,t*50,t), 2, cv::LINE_AA);
    } 
    //}
