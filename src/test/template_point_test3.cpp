...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
  ...
  if (reset == true) {
    ...
    int limited=0;//テンプレートマッチングの不具合を考慮(外枠8Pixelの特徴点を検出しない)
      ...
      cv::rectangle(img_dst, cv::Point(points_curr[i].x-8,points_curr[i].y+8), 
      cv::Point(points_curr[i].x+8,points_curr[i].y-8), cv::Scalar(255, 0, 0), 1, cv::LINE_AA);//四角形を描写(青)
    
    //特徴点を検出したら特徴点の周りの画像をクロップして保存→その後保存した画像でBoWを行う予定
    //テンプレートマッチングの不具合を考慮(外枠8Pixelの特徴点を検出しない)
    if(8<points_curr[i].y&&points_curr[i].y<480-8){
      if(8<points_curr[i].x&&points_curr[i].x<640-8){
      ...
      cv::Rect roi2(cv::Point(points_curr[i].x-8,points_curr[i].y-8), cv::Size(16, 16));//特徴点を中心とした16☓16pixelの画像を切り取る
      //FeaturePoint[i] = image(roi2); // 切り出し画像
      FeaturePoint[i] = image_curr(roi2); // 切り出し画像
      cv::imshow("win_FeaturePoint", FeaturePoint[i]);//黄色の特徴点を中心としたクロップ画像


      std::cout <<"特徴点画像クロッププログラムlimited["<<limited<<"]"<< std::endl;
      cv::rectangle(img_1, cv::Point(points_curr_limited[i].x-8,points_curr_limited[i].y+8), 
      cv::Point(points_curr_limited[i].x+8,points_curr_limited[i].y-8), cv::Scalar(255, 255, 255), 2, cv::LINE_AA);//四角形を描写(白)
  ...
  //二回目動作プログラム--------------------------------------------------------------------------------------------
    ...
    for (int i = 0; i < Template_Points_Prev.size(); i++) {
     ...
     //if(req == 1){
        ...
        cv::matchTemplate(image_curr, img_template1, img_minmax1, cv::TM_SQDIFF);//差分二乗和
        ...
        if(min_val1[i]<max_val1[i]*0.01){//最小値がしきい値以下なら表示
          cv::rectangle(img_dst, cv::Rect(min_pt1[i].x, min_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(255, 255, 255), 3);//白枠
          cv::circle(img_dst, cv::Point(min_pt1[i].x+img_template1.cols/2, min_pt1[i].y+img_template1.rows/2), 5, cv::Scalar(255, 255, 255), -1);//テンプレートの中心座標
