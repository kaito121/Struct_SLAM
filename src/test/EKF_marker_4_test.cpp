...
ofstream outputfile11("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/marker_test_ALL_xEst[4].txt");//出力ファイルパス
ofstream outputfileDpx("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/xEst0_cl[4](0,0).txt");//出力ファイルパス
ofstream outputfileDpy("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/xEst0_cl[4](1,0).txt");//出力ファイルパス
ofstream outputfileDpz("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/xEst0_cl[4](2,0).txt");//出力ファイルパス
ofstream outputfileDpEstx("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/xEst_cl[4](0,0).txt");//出力ファイルパス
ofstream outputfileDpEsty("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/xEst_cl[4](1,0).txt");//出力ファイルパス
ofstream outputfileDpEstz("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/xEst_cl[4](2,0).txt");//出力ファイルパス
ofstream outputfilePix("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/pixel_cl[4](0,0).txt");//出力ファイルパス
ofstream outputfilePiy("/home/fuji/catkin_ws/src/Struct_SLAM/src/EKF/pixel_cl[4](1,0).txt");//出力ファイルパス
...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
  ...
  if (markerIds.size() > 0) {
    ...
    //マーカーが追跡可能かどうかを見る(MC_pointとMC_point_prveで同じマーカーがあれば追跡可能)
    if(kaisu!=0){
    if(KAL_Wait>1){//KAL_Waitが1より大きいとき実行
      if(KAL==false){//カルマン推定前はこっち
          if(MC_point[4][3]!=0){//最新要素が無いものを除外
            if(MC_point[4][3]==MC_point_prve[4][3]){//追跡可能かどうか
            std::cout << "MC_point     [4]={"<< MC_point[4][0] <<","<<MC_point[4][1]<<","<<MC_point[4][2]<<","<<MC_point[4][3]<<"}"<< std::endl;
            std::cout << "MC_point_prev[4]={"<< MC_point_prve[4][0] <<","<<MC_point_prve[4][1]<<","<<MC_point_prve[4][2]<<","<<MC_point_prve[4][3]<<"}"<< std::endl;

            xEst0_prev_cl[4](0,0)=MC_point_prve[4][0];//一つ前に観測したマーカーのカメラ座標（カルマン用)
            xEst0_prev_cl[4](1,0)=MC_point_prve[4][1];
            xEst0_prev_cl[4](2,0)=MC_point_prve[4][2];
            xEst0_prev_clP[4]=1;//要素がある時は1(マッチング確認用)

            xEst0_cl[4](0,0)=MC_point[4][0];//今観測したマーカーのカメラ座標(カルマン用)
            xEst0_cl[4](1,0)=MC_point[4][1];
            xEst0_cl[4](2,0)=MC_point[4][2];
            xEst0_clP[4]=1;//要素があるときは1;(マッチング確認用)

            pixel_cl[4](0,0)=pixel[4][0] - camera_info.K[2];//今観測したマーカーの正規化画像座標(カルマン用)
            pixel_cl[4](1,0)=pixel[4][1] - camera_info.K[5];

            ////三次元空間上
            //At0(kosuu,0)=-1,  At0(kosuu,1)=0,   At0(kosuu,2)=0,   At0(kosuu,3)=0,                       At0(kosuu,4)=-xEst0_prev_cl[4](2,0),  At0(kosuu,5)=-xEst0_prev_cl[4](1,0);
            //At0(kosuu+1,0)=0, At0(kosuu+1,1)=-1,At0(kosuu+1,2)=0, At0(kosuu+1,3)=xEst0_prev_cl[4](2,0), At0(kosuu+1,4)=0,                     At0(kosuu+1,5)=-xEst0_prev_cl[4](0,0);
            //At0(kosuu+2,0)=0, At0(kosuu+2,1)=0, At0(kosuu+2,2)=-1,At0(kosuu+2,3)=-xEst0_prev_cl[4](1,0),At0(kosuu+2,4)=xEst0_prev_cl[4](0,0), At0(kosuu+2,5)=0;

            //xz平面上を移動する条件下の時
            At0(0,0)=-1,  At0(0,1)=0,   At0(0,2)=0,   At0(0,3)=0,   At0(0,4)=-xEst0_prev_cl[4](2,0),  At0(0,5)=0;
            At0(1,0)=0,   At0(1,1)=-1,  At0(1,2)=0,   At0(1,3)=0,   At0(1,4)=0,                       At0(1,5)=0;
            At0(2,0)=0,   At0(2,1)=0,   At0(2,2)=-1,  At0(2,3)=0,   At0(2,4)=xEst0_prev_cl[4](0,0),   At0(2,5)=0;

            Et0(0,0)=xEst0_cl[4](0,0)-xEst0_prev_cl[4](0,0);
            Et0(1,0)=xEst0_cl[4](1,0)-xEst0_prev_cl[4](1,0);
            Et0(2,0)=xEst0_cl[4](2,0)-xEst0_prev_cl[4](2,0);

            //誤差共分散を求めるときに使用する式AW
            AWt0(0,0)=-1, AWt0(0,1)=0,  AWt0(0,2)=0,  AWt0(0,3)=-xEst0_cl[4](2,0);
            AWt0(1,0)=0,  AWt0(1,1)=-1, AWt0(1,2)=0,  AWt0(1,3)=0;
            AWt0(2,0)=0,  AWt0(2,1)=0,  AWt0(2,2)=-1, AWt0(2,3)=xEst0_cl[4](0,0);

            //AWt0(0,0)=-1,   AWt0(0,1)=0,    AWt0(0,2)=0,    AWt0(0,3)=-xEst0_prev_cl[4](2,0);
            //AWt0(1,0)=0,  AWt0(1,1)=-1, AWt0(1,2)=0,  AWt0(1,3)=0;
            //AWt0(2,0)=0,  AWt0(2,1)=0,  AWt0(2,2)=-1, AWt0(2,3)=xEst0_prev_cl[4](0,0);

            
            //共分散の内部要素(観測値ー前の推定値)
            delta0(0,0)=(xEst0_cl[4](0,0)-xEst0_prev_cl[4](0,0))*(xEst0_cl[4](0,0)-xEst0_prev_cl[4](0,0));
            delta0(1,0)=(xEst0_cl[4](1,0)-xEst0_prev_cl[4](1,0))*(xEst0_cl[4](1,0)-xEst0_prev_cl[4](1,0));
            delta0(2,0)=(xEst0_cl[4](2,0)-xEst0_prev_cl[4](2,0))*(xEst0_cl[4](2,0)-xEst0_prev_cl[4](2,0));

            }
            //追跡できない時
            else{xEst0_prev_clP[4]=0,xEst0_clP[4]=0;}//要素がない時は0(マッチング確認用)
          }
          //最新要素が無い時
          else{xEst0_prev_clP[4]=0,xEst0_clP[4]=0;}//要素がない時は0(マッチング確認用)
     ...
     /*   for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          if(MC_point[i][3]!=0){//最新要素が無いものを除外
            ...
            std::cout << "MC_point     ["<<i<<"]={"<< MC_point[i][0] <<","<<MC_point[i][1]<<","<<MC_point[i][2]<<","<<MC_point[i][3]<<"}"<< std::endl;
            std::cout << "MC_point_prev["<<i<<"]={"<< MC_point_prve[i][0] <<","<<MC_point_prve[i][1]<<","<<MC_point_prve[i][2]<<","<<MC_point_prve[i][3]<<"}"<< std::endl;
            ...
            //誤差共分散を求めるときに使用する式AW
            AWt0(kosuu,0)=-1,   AWt0(kosuu,1)=0,    AWt0(kosuu,2)=0,    AWt0(kosuu,3)=-xEst0_cl[i](2,0);
            AWt0(kosuu+1,0)=0,  AWt0(kosuu+1,1)=-1, AWt0(kosuu+1,2)=0,  AWt0(kosuu+1,3)=0;
            AWt0(kosuu+2,0)=0,  AWt0(kosuu+2,1)=0,  AWt0(kosuu+2,2)=-1, AWt0(kosuu+2,3)=xEst0_cl[i](0,0);
      ...
      std::cout << "カルマン推定後"<< std::endl;
          if(MC_point[4][3]!=0){//最新要素が無いものを除外
            std::cout << "MC_point[4=4][3]="<<MC_point[4][3]<<",xEst_prev_clP[4=4]="<<xEst_prev_clP[4]<< std::endl;
            if(MC_point[4][3]==xEst_prev_clP[4]){//追跡可能かどうか
              std::cout << "point       [4]={"<< MC_point[4][0] <<","<<MC_point[4][1]<<","<<MC_point[4][2]<<","<<MC_point[4][3]<<"}"<< std::endl;
              std::cout << "xEst_prev_cl[4]={"<< xEst_prev_cl[4](0,0) <<","<<xEst_prev_cl[4](1,0)<<","<<xEst_prev_cl[4](2,0)<<","<<xEst_prev_clP[4]<<"}"<< std::endl;
              std::cout << "Pt_cl       [4]=\n"<<Pt_cl[4]<< std::endl;//１つ前の推定誤差共分散

              xEst0_prev_cl[4](0,0)=xEst_prev_cl[4](0,0);//一つ前に推定したマーカーのカメラ座標（カルマン用)
              xEst0_prev_cl[4](1,0)=xEst_prev_cl[4](1,0);
              xEst0_prev_cl[4](2,0)=xEst_prev_cl[4](2,0);
              ...
              pixel_cl[4](0,0)=pixel[4][0] - camera_info.K[2];//今観測したマーカーの正規化画像座標
              pixel_cl[4](1,0)=pixel[4][1] - camera_info.K[5];
            ...
            //新しくマーカーが観測された時
            if(MC_point[4][3]==1&&xEst_prev_clP[4]==0){
              xEst_prev_cl[4](0,0)=MC_point[4][0];//今観測したマーカーのカメラ座標（カルマン用)
              xEst_prev_cl[4](1,0)=MC_point[4][1];
              xEst_prev_cl[4](2,0)=MC_point[4][2];
            }   
          }
          //最新要素が無い時
          else{xEst0_prev_clP[4]=0,xEst0_clP[4]=0;}//要素がない時は0(マッチング確認用)
        ...
        if(xEst0_clP[4]==1){
          std::cout << "xEst0_prev_cl[4]=\n"<<xEst0_prev_cl[4]<< std::endl;
          std::cout << "xEst0_cl[4]=\n"<<xEst0_cl[4]<< std::endl;
        }
      ...
      //リサイズ用行列定義
      cv::Mat_<float>At=cv::Mat_<float>(3, 6);
      cv::Mat_<float>Et=cv::Mat_<float>(3, 1);
      cv::Mat_<float>AWt=cv::Mat_<float>(3, 4);//誤差共分散を求めるときに使用する式AW
      cv::Mat_<float>delta=cv::Mat_<float>(3, 1);//共分散deltaの内部要素

      cv::Mat_<float>Tt=cv::Mat_<float>(6, 1);//教科書運動パラメータ

      // Pt0の第0行から第3行をPtの第0行から第3行に代入(リサイズ)
	    At0.rowRange(0, 3).copyTo(At.rowRange(0, 3));
	    Et0.rowRange(0, 3).copyTo(Et.rowRange(0, 3));
	    AWt0.rowRange(0, 3).copyTo(AWt.rowRange(0, 3));
	    delta0.rowRange(0, 3).copyTo(delta.rowRange(0, 3));

      std::cout <<"At=\n"<<At<< std::endl;
      std::cout <<"Et=\n"<<Et<< std::endl;
      std::cout <<"AWt=\n"<<AWt<< std::endl;

      ////特徴点1+マーカーを使用した外部パラメータ推定
      //if (swap_on==true) {
      //  std::cout <<"特徴点1を使用した外部パラメータ推定"<<std::endl;
      //  ALL_At=cv::Mat_<float>(kosuu+PT_kosuu, 6);
      //  ALL_Et=cv::Mat_<float>(kosuu+PT_kosuu, 1);
      ...
      //特徴点1検出動作時(マーカーのみで外バラ推定)
      //if (swap_on==false) {
        std::cout <<"マーカーを使用した外部パラメータ推定"<<std::endl;
        //マーカーを使用した外部パラメータ推定
        ALL_At=cv::Mat_<float>(3, 6);
        ALL_Et=cv::Mat_<float>(3, 1);
        At.rowRange(0, 3).copyTo(ALL_At.rowRange(0, 3));
	      Et.rowRange(0, 3).copyTo(ALL_Et.rowRange(0, 3));
      //}
      ...
      //状態モデルの予測式
          ...
          Gt[4](0,0)=-1,Gt[4](0,1)=0, Gt[4](0,2)=0, Gt[4](0,3)=-xEst0_prev_cl[4](2,0);
          Gt[4](1,0)=0, Gt[4](1,1)=-1,Gt[4](1,2)=0, Gt[4](1,3)=-0;
          Gt[4](2,0)=0, Gt[4](2,1)=0, Gt[4](2,2)=-1,Gt[4](2,3)=xEst0_prev_cl[4](0,0);
          std::cout <<"Gt[4]=\n"<<Gt[4]<< std::endl;
          std::cout <<"xEst0_prev_cl[4]=\n"<<xEst0_prev_cl[4]<< std::endl;
          std::cout <<"ALL_Vt=\n"<<ALL_Vt<< std::endl;
          std::cout <<"ALL_Ft=\n"<<ALL_Ft<< std::endl;
          ...
          xEst_cl[4]=ALL_Ft*xEst0_prev_cl[4]+Gt[4]*ALL_Vt;//カルマン初回動作はこっち
      ...
      //誤差共分散の予測式-----------------------------------
      cv::Mat eye = cv::Mat_<float>::eye(3, 3);// 単位行列3, 4
      cv::Mat_<float>eyetemp=cv::Mat_<float>(1, 3);
      cv::Mat_<float>deltaeye=cv::Mat_<float>(3, 3);
      cv::Mat_<float>AWt_1=cv::Mat_<float>(3, 4);
      cv::Mat_<float>Wt = cv::Mat_<float>::zeros(4, 4);

      int kosuuNO;
      for(int i=0;i<3;i++){//共分散の対角化
        ...
        if(i%3==0){eyetemp=eyetemp*delta(i,0)*0.0000065175;}
        else if(i%3==1){eyetemp=eyetemp*delta(i,0)*0.0000085768;}
        else if(i%3==2){eyetemp=eyetemp*delta(i,0)*0.00024422;}
      ...
      std::cout <<"Wt=\n"<<Wt<< std::endl;
        ...
        if(xEst0_clP[4]==1){//要素がある時のみ実行
          if(KAL==false){//カルマン初回実行時はこっち
            Mt_cl[4]= (cv::Mat_<float>(3, 3) <<
              10, 0, 0,
              0, 10, 0,
              0, 0, 10);//Mt_cl[i]の初期値
          }
          else{Mt_cl[4]=ALL_Ft*Pt_cl[4]*ALL_Ft.t()+Gt[4]*Wt*Gt[4].t();}
          //Mt_cl[i]=ALL_Ft*Pt_cl[i]*ALL_Ft.t()+Gt[i]*Wt*Gt[i].t();
          //Mt_cl[i]=Ft*Pt_cl[i]*Ft.t()+Gt[i]*Wt*Gt[i].t();
          std::cout <<"Pt_cl[4]=\n"<<Pt_cl[4]<< std::endl;
          std::cout <<"Mt_cl[4]=\n"<<Mt_cl[4]<< std::endl;//誤差共分散の予測式Mt
        }
      //更新ステップ----------------------------------------------------------------------
      //観測残差
        if(xEst0_clP[4]==1){//要素がある時のみ実行
          Ht_cl[4]= (cv::Mat_<float>(2, 3) <<
            camera_info.K[0]/xEst0_prev_cl[4](2,0), 0,                                      -camera_info.K[0]*(xEst0_prev_cl[4](0,0)/(xEst0_prev_cl[4](2,0)*xEst0_prev_cl[4](2,0))),
            0,                                      camera_info.K[0]/xEst0_prev_cl[4](2,0), -camera_info.K[0]*(xEst0_prev_cl[4](1,0)/(xEst0_prev_cl[4](2,0)*xEst0_prev_cl[4](2,0))));

          Ut_cl[4]= (cv::Mat_<float>(2, 1) <<
            camera_info.K[0]*(xEst0_prev_cl[4](0,0)/xEst0_prev_cl[4](2,0)),
            camera_info.K[0]*(xEst0_prev_cl[4](1,0)/xEst0_prev_cl[4](2,0)));

          std::cout <<"Ht_cl[4]=\n"<<Ht_cl[4]<< std::endl;
          std::cout <<"Ut_cl[4]=\n"<<Ut_cl[4]<< std::endl;

          Yt_cl[4]=pixel_cl[4]-(Ht_cl[4]*xEst_cl[4]+Ut_cl[4]);
          std::cout <<"pixel[4][0]="<<pixel[4][0]<< std::endl;
          std::cout <<"pixel[4][1]="<<pixel[4][1]<< std::endl;
          std::cout <<"pixel_cl[4]=\n"<<pixel_cl[4]<< std::endl;
          std::cout <<"xEst_cl[4]=\n"<<xEst_cl[4]<< std::endl;
          std::cout <<"Ht_cl[4]*xEst_cl[4]=\n"<<Ht_cl[4]*xEst_cl[4]<< std::endl;
          std::cout <<"Ht_cl[4]*xEst_cl[4]+Ut_cl[4]=\n"<<Ht_cl[4]*xEst_cl[4]+Ut_cl[4]<< std::endl;

          std::cout <<"観測残差:Yt_cl[4]=\n"<<Yt_cl[4]<< std::endl;//観測残差

          St[4]=Ht_cl[4]*Mt_cl[4]*Ht_cl[4].t()+u_;
          std::cout <<"観測残差の共分散:St[4]=\n"<<St[4]<< std::endl;

          Pt_cl[4]=Mt_cl[4].inv()+(Ht_cl[4].t()*u_.inv()*Ht_cl[4]);
          Pt_cl[4]=Pt_cl[4].inv();
          std::cout <<"誤差共分散の更新:Pt_cl[4]=\n"<<Pt_cl[4]<< std::endl;//誤差共分散の更新

          //Kt_cl[4]=Pt_cl[4]*Ht_cl[4].t()*u_.inv();
          Kt_cl[4]=Pt_cl[4]*Ht_cl[4].t()*St[4].inv();
          std::cout <<"u_.inv()=\n"<<u_.inv()<< std::endl;
          std::cout <<"カルマンゲイン:Kt_cl[4]=\n"<<Kt_cl[4]<< std::endl;//カルマンゲイン

          xEst_cl[4]=xEst_cl[4]+(Kt_cl[4]*Yt_cl[4]);
          std::cout <<"状態モデルの更新:xEst_cl[4]=\n"<<xEst_cl[4]<< std::endl;//状態モデルの更新

          xEst_prev_cl[4]=xEst_cl[4];//現在の推定を保存
          xEst_prev_clP[4]=1;
          std::cout <<"xEst_prev_clP[4]="<<xEst_prev_clP[4]<< std::endl;
          KAL=true;//次の動作の時カルマンの推定結果と観測値の追跡を確認する
        }
        //新しくマーカーが観測された時
        else if(MC_point[4][3]==1&&xEst_prev_clP[4]==0){
          xEst_prev_clP[4]=1;
          std::cout <<"新しくマーカーが観測された時 xEst_prev_clP[4]="<<xEst_prev_clP[4]<< std::endl;
        }
        //更新データが存在しない時
        else{xEst_prev_clP[4]=0;
          //std::cout <<"ELSE xEst_prev_clP[4]="<<xEst_prev_clP[4]<< std::endl;
        }
      
      //outputfileDpx<<xEst0_cl[32](0,0)<<"\n";
      //outputfileDpy<<xEst0_cl[32](1,0)<<"\n";
      //outputfileDpz<<xEst0_cl[32](2,0)<<"\n";
      //outputfileDpEstx<<xEst_cl[32](0,0)<<"\n";
      //outputfileDpEsty<<xEst_cl[32](1,0)<<"\n";
      //outputfileDpEstz<<xEst_cl[32](2,0)<<"\n";
      //outputfilePix<<pixel_cl[32](0,0)<<"\n";
      //outputfilePiy<<pixel_cl[32](1,0)<<"\n";

      outputfileDpx<<xEst0_cl[4](0,0)<<"\n";
      outputfileDpy<<xEst0_cl[4](1,0)<<"\n";
      outputfileDpz<<xEst0_cl[4](2,0)<<"\n";
      outputfileDpEstx<<xEst_cl[4](0,0)<<"\n";
      outputfileDpEsty<<xEst_cl[4](1,0)<<"\n";
      outputfileDpEstz<<xEst_cl[4](2,0)<<"\n";
      outputfilePix<<pixel_cl[4](0,0)<<"\n";
      outputfilePiy<<pixel_cl[4](1,0)<<"\n";


      outputfile11<<"　　推定結果　　 "<<"               "<<"　観測データ\n";
      outputfile11<<"xEst0_prev_cl[4]="<<xEst0_prev_cl[4](0,0)<<", xEst0_cl[4]="<<xEst0_cl[4](0,0)<<",外部パラメータ推定:Tt="<<Tt(0,0)<<",  "<<Tt(3,0)<<",外部パラメータ推定:ALL_Tt="<<ALL_Tt(0,0)<<",  "<<ALL_Tt(3,0)<<"\n";
      outputfile11<<"                 "<<xEst0_prev_cl[4](1,0)<<"              "<<xEst0_cl[4](1,0)<<"                       "<<Tt(1,0)<<",  "<<Tt(4,0)<<"                           "<<ALL_Tt(1,0)<<",  "<<ALL_Tt(4,0)<<"\n";
      outputfile11<<"                 "<<xEst0_prev_cl[4](2,0)<<"              "<<xEst0_cl[4](2,0)<<"                          "<<Tt(2,0)<<",  "<<Tt(5,0)<<"                           "<<ALL_Tt(2,0)<<",  "<<ALL_Tt(5,0)<<"\n";
    }//if(KAL_Wait>1)→end(マーカー再認識初回は動作しない)
    ...
    KAL=false;//マーカーが認識されなくなったらカルマンリセット
    KAL_Wait=0;//再びマーカー認識時一周目はカルマンを行わない
    }
