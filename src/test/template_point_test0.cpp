...
std::string win_dst2 = "dst2";//カメラ画像
std::string win_dst3 = "dst3";//カメラ画像
...
cv::Mat frame,image_curr, image_prev,img_dst,img_dst2,img_dst3,img_dst4,img_1;
...
cv::Mat temple1,temple2;//テンプレートを取得した画像
cv::Mat FeaturePoint0[100];//特徴点周囲の切り取った画像
int FP=0;
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
    ...
    int template_size=15;
      ...
      if(template_size<points_curr[i].y&&points_curr[i].y<480-template_size){
        if(template_size<points_curr[i].x&&points_curr[i].x<640-template_size){
          ...
          if(kaisu==0){
            FeaturePoint0[FP] = image(roi2); // 切り出し画像
            cv::rectangle(img_dst2, roi2, cv::Scalar(255, 255, 255), 2, cv::LINE_AA);//四角形を描写(白)
            FP=FP+1;//取得したテンプレの数
          }
          if(kaisu==20){
            FeaturePoint0[FP] = image(roi2); // 切り出し画像
            cv::rectangle(img_dst3, roi2, cv::Scalar(255, 255, 255), 2, cv::LINE_AA);//四角形を描写(白)
            FP=FP+1;//取得したテンプレの数
          }
          if(kaisu==40){
            FeaturePoint0[FP] = image(roi2); // 切り出し画像
            cv::rectangle(img_dst4, roi2, cv::Scalar(255, 255, 255), 2, cv::LINE_AA);//四角形を描写(白)
            FP=FP+1;//取得したテンプレの数
          }
  ...
  //二回目動作プログラム--------------------------------------------------------------------------------------------
    ...
    /*for (int i = 0; i < FP; i++) {
      ...
      FeaturePoint0[i].copyTo(img_template1); // 切り出し画像
    ...
    for (int i = 0; i < FP; i++) {
      ...
      std::cout << "min_val1(白)["<<i<<"]=" << min_val1[i] << std::endl;//一致度が上がると値が小さくなる
      std::cout << "max_val1(白)["<<i<<"]=" << max_val1[i] << std::endl;
      if(0.99<max_val1[i]){//最小値がしきい値以下なら表示
