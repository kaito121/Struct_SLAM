...
//  
	...
	std::vector<cv::Point2f> Harris_corners;
  	cv::goodFeaturesToTrack(img_gray, Harris_corners, 1000, 0.1, 5);
  	for (int i = 0; i < Harris_corners.size(); i++) {
  	  cv::circle(img_Harris, cv::Point(Harris_corners[i].x, Harris_corners[i].y), 4, cv::Scalar(0, 0, 255), 2);//赤色
  	}
