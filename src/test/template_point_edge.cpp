...
cv::Mat frame,image_curr, image_prev,img_dst,img_dst2,img_1,img_edge;
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
  ...
  //cv::Canny(image, img_edge, 200, 200);

  //エッジ抽出(二次微分)
  Mat img_tmp1;
  cv::Laplacian(image, img_edge, CV_32F, 3);
  cv::convertScaleAbs(img_edge, img_edge, 1, 0);
  ...
  if (reset == true) {
    ...
    if(template_size<points_curr[i].y&&points_curr[i].y<480-template_size){
      ...
      //FeaturePoint[i] = img_edge(roi2); // 切り出し画像
    ...
    Template_Points_Curr = points_curr_limited;//テンプレートの中心座標
    Template_Points_Curr.resize(limited);
		reset = false;//if文切り替え
  }//------------------------------------------------------------------------------------------------------------------------ 
  ...
  //二回目動作プログラム--------------------------------------------------------------------------------------------
    ...
    for (int i = 0; i < Template_Points_Curr.size(); i++) {
      ...
      if(min_val1[i]<max_val1[i]*0.05){//最小値がしきい値以下なら表示
        ...
        Template_Points_Curr[i].x=min_pt1[i].x+img_template1.cols/2;
        Template_Points_Curr[i].y=min_pt1[i].y+img_template1.rows/2;
        templP=templP+1;//マッチングの中心座標個数
        std::cout <<"マッチングの中心座標["<<i<<"]="<<Template_Points_Curr[i]<< std::endl;
      }
    }//テンプレートマッチング終了 ------------------------------------------------------------------------(差分二乗和)*/
    ...
    for (int i = 0; i < Template_Points_Curr.size(); i++) {
      ...
      cv::Mat img_minmax1;
      // テンプレートマッチング
      cv::matchTemplate(image, img_template1, img_minmax1, cv::TM_CCORR_NORMED);//正規化相互相関
      cv::minMaxLoc(img_minmax1, &min_val1[i], &max_val1[i], &min_pt1[i], &max_pt1[i]);
      std::cout << "min_val1(白)["<<i<<"]=" << min_val1[i] << std::endl;//一致度が上がると値が大きくなる
      std::cout << "max_val1(白)["<<i<<"]=" << max_val1[i] << std::endl;
      if(0.99<max_val1[i]){//最小値がしきい値以上なら表示
    ...
    for (int i = 0; i < Template_Points_Curr.size(); i++) {
      ...
      cv::Mat img_minmax1;
      // テンプレートマッチング
      cv::matchTemplate(image, img_template1, img_minmax1, cv::TM_CCOEFF_NORMED);//正規化相互相関
      //cv::matchTemplate(img_edge, img_template1, img_minmax1, cv::TM_CCOEFF_NORMED);//正規化相互相関
      cv::minMaxLoc(img_minmax1, &min_val1[i], &max_val1[i], &min_pt1[i], &max_pt1[i]);
      std::cout << "min_val1(白)["<<i<<"]=" << min_val1[i] << std::endl;//一致度が上がると値が大きくなる
      std::cout << "max_val1(白)["<<i<<"]=" << max_val1[i] << std::endl;
      if(0.8<max_val1[i]){//最小値がしきい値以上なら表示
        min_ptA[templP]=max_pt1[i];
        cv::rectangle(img_dst, cv::Rect(max_pt1[i].x, max_pt1[i].y, img_template1.cols, img_template1.rows), cv::Scalar(0, 255, 0), 3);//白枠
        cv::circle(img_dst, cv::Point(max_pt1[i].x+img_template1.cols/2, max_pt1[i].y+img_template1.rows/2), 5, cv::Scalar(0, 255, 0), -1);//テンプレートの中心座標
        std::cout << "マッチング座標max_pt1["<<i<<"]=" << max_pt1[i] << std::endl;
    ...
    Template_Points_Curr.resize(templP);
    ...
    //マッチングの個数が1以下になったらオプティカルフロースキップ(オプティカルフローの追跡エラー対策)
		if(Template_Points_Curr.size()<=1){reset = true;}
    else{
    cv::line(img_dst2,cv::Point(Template_Points_Curr[0].x,Template_Points_Curr[0].y),cv::Point(Template_Points_Prev[0].x,Template_Points_Prev[0].y),cv::Scalar(0,0,0), 2, cv::LINE_AA);
    cv::line(img_dst2,cv::Point(Template_Points_Curr[1].x,Template_Points_Curr[1].y),cv::Point(Template_Points_Prev[1].x,Template_Points_Prev[1].y),cv::Scalar(255,0,0), 2, cv::LINE_AA);
    cv::line(img_dst2,cv::Point(Template_Points_Curr[2].x,Template_Points_Curr[2].y),cv::Point(Template_Points_Prev[2].x,Template_Points_Prev[2].y),cv::Scalar(0,0,255), 2, cv::LINE_AA);
    }
