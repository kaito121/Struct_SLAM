...
//  
	...
	{	
		...
		samplePointA[0]=corners[rand1][0];//x1//ランダムに点の座標を取得
		samplePointA[1]=corners[rand1][1];//y1
		samplePointB[0]=corners[rand2][0];//x2
		samplePointB[1]=corners[rand2][1];//y2
		...
		for(int j=0;j<cornerCount;j++){
			double calcY;
			samplePoint[0]=corners[j][0];//x(観測データ)
			samplePoint[1]=corners[j][1];//y
			//３．求めたパラメータを、総データ点数から取り出したn個のデータを除いたものに式を当てはめ、
			//観測されたデータと２．で求めたパラメータの誤差を計算します。
	...
	//６．仮パラメータを使ってすべてのデータに再度式を適用し、誤差が許容範囲内のものを抽出します。
	//(この時点で雑音データがほぼ取り除かれます）
	for(int j=0;j<cornerCount;j++){
