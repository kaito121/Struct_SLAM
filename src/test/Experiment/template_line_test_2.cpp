      ...
      //3回目以降動作
        ...
        for(int i=0;i<DMT_prev_ok;i++){
          ...
          if(0.75<max_val1[i]){//最小値がしきい値以下なら表示
            ...
            std::cout << "マッチング座標max_pt1["<<i<<"]=" << max_pt1[i] << std::endl;
            //std::cout << "マッチング座標max_pt1["<<i<<"].y=" << max_pt1[i].y << std::endl;
            MTPP[i].x=max_pt1[i].x+img_template1.cols/2;
            MTPP[i].y=max_pt1[i].y+img_template1.rows/2;
            MTTP[i]=MT_prev_Templ[i];//マッチングしたテンプレート画像(Matching_Tate_Templ_curr=MTTC)
            std::cout <<"マッチングの中心座標["<<i<<"]="<<MTPP[i]<< std::endl;
            MT_prev_pixel[i]=MT_prev_pixel[i];
            MT_prev_camera[i]=MT_prev_camera[i];
            MT_prev_Templ[i]=MT_prev_Templ[i];
            cv::line(img_dst,cv::Point(MTPP[i]),cv::Point(MT_prev_pixel[i]),cv::Scalar(0,255,255), 3, cv::LINE_AA);
        ...
        MTPP.resize(DMT_prev_ok);//Depth取得可能数でリサイズ(運動復元画像座標)
