...
//ここに処理項目
 ...
 /* 並び替えした数値を出力 */
  ...
  //for (int i=0; i<lines1.size(); ++i){ printf("lines[%d][1]=%f\n", i,lines1[i][1]); }
    int c[20],t,j,p,aa,aab[20],aac[5];
    double A[20][20][2][4],AA[20][20][2][4],B;
    c[0]=1,t=0,j=1,p=0;
    ...
    for(int i = 0; i < lines.size(); i++){
        ...
        if( B==theta[i+1]){
            ...
            A[t][j][0][0]=lines[i+1][0];  A[t][j][1][0]=lines[i+1][2];//代入
            A[t][j][0][1]=lines[i+1][1];  A[t][j][1][1]=lines[i+1][3];//代入
            A[t][j][0][2]=dep1[i+1];      A[t][j][1][2]=dep2[i+1];//代入
            A[t][j][0][3]=theta[i+1];     A[t][j][1][3]=theta[i+1];//代入

            aab[aa]=j;//平行線と線グループの番号の対応付け(No.aaはjの何番目に相当するか)
            j=j+1;//配列カウント
            aa=aa+1;//配列並行線カウント
            c[t]=c[t]+1;//要素数（同じ数値は何個あるか）
            std::cout <<"平行線の総数["<<t<<"] ="<<aac[t]<< std::endl;
            std::cout <<"平行線の線番号 "<< std::endl;
        }
        //前の番号と異なる数値
        else{
          aac[t]=aa;//平行線の総数
          ...
          if(theta[i+1]-B>0.3){//前の角度との差が0.5より大きい(異なる角度)
