...
vector<cv::Point2f> points_prev, points_curr,points_curr2,points_prev2;
	...
	cv::cvtColor(image, image_curr, cv::COLOR_BGR2GRAY);//グレースケール化
		...
		std::cout <<"初回検出プログラム"<< std::endl;//特徴点の座標
			...
			for (i = k = 0; i < status.size(); i++)
			{
				...
				if (status[i] == 0) {continue;}//ここで検索出来なかったものを消し探索の範囲を狭めている
				points_prev[k]   = points_prev[i];
				points_curr[k++] = points_curr[i];//ここでK=K+1されている
				std::cout <<"points_prevk["<<k<<"]="<<points_prev[i]<< std::endl;
				std::cout <<"points_currk["<<k<<"]="<<points_curr[i]<< std::endl;
			}
			std::cout <<"リサイズの大きさk="<<k<< std::endl;
			points_curr.resize(k);//kの大きさでリサイズされている
			points_prev.resize(k);
			tuika=0;

			//特徴点が100個以下になったら再び特徴点を検出する
			if(status.size()<150){
			std::cout <<"二回目検出プログラム"<< std::endl;//特徴点の座標
			// 特徴点検出(グレースケール画像から特徴点検出)
			cv::goodFeaturesToTrack(image_curr, points_curr2, 500, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
			...
			points_prev2 = points_curr2;//データのコピー
		...
		for (int i = 0; i < points_curr2.size(); i++) {
			cv::Scalar c(0, 255, 0);//緑
			if (cv::norm(points_prev2[i] - points_curr2[i]) > 0.5) {
				c = cv::Scalar(0, 100, 255);//今の特徴点と一つ前の特徴点との差が0.5以上の時オレンジ
			}

			cv::circle(img_dst, points_curr2[i], 8, Scalar(255,0,0), -1, cv::LINE_AA);
			
			//std::cout <<"points_curr2["<<i<<"]="<<points_curr2[i]<< std::endl;//特徴点の座標
			//std::cout <<"points_prev2["<<i<<"]="<<points_prev2[i]<< std::endl;//特徴点の座標
		}
