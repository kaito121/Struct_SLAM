...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
  ...
  if (reset == true) {
    ...
    //cv::goodFeaturesToTrack(今画像, 前画像, 特徴点の個数, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
		cv::goodFeaturesToTrack(image_curr, points_curr, 15, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
    ...
    std::cout <<"制限した特徴点の個数(limited)="<<limited<< std::endl;
		reset = false;//if文切り替え
  }//------------------------------------------------------------------------------------------------------------------------ 
  ...
  //二回目オプティカルフロー--------------------------------------------------------------------------------------------
		...
		if(status.size()<6){reset = true;}//特徴点が100個以下になったら再び特徴点を検出する
	}//else文ここまで-------------------------------------------------------------------------------------------
  ...
  int NotZeroL=-1;
    ...
    std::cout <<"cv::norm(points_prev["<<i<<"] - points_curr["<<i<<"])="<<pointnorm[i]<< std::endl;//オプティカルフローの長さ
      ...
      if((points_curr[i].x-points_prev[i].x)>(points_curr[i].y-points_prev[i].y)){
        //cv::circle(img_dst2, points_curr[i], 1, cv::Scalar(255,0,0), -1, cv::LINE_AA);//今の座標情報
        cv::arrowedLine(img_dst2,points_curr[i],points_prev[i],cv::Scalar(255,255,0),1,8,0,1.0);//矢印の描写
      }
      else{
        //cv::circle(img_dst2, points_curr[i], 1, cv::Scalar(0,0,255), -1, cv::LINE_AA);//今の座標情報
    ...
    if(req == 1){
      ...
      cv::Mat img_template1 = FeaturePoint[i]; // 切り出し画像
      cv::Mat img_minmax1;
      // テンプレートマッチング
      ...
      cv::rectangle(img_dst, cv::Rect(max_pt1.x, max_pt1.y, img_template1.cols, img_template1.rows), cv::Scalar(255, 255, 255), 3);//白
      }
    }
   //----------------------------------------------------------------------------------------------------------------------
