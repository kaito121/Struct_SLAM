...
int main()
{
  ...
  KF2.statePre.at<float>(0) = 0;//予測の状態方程式(x'(k)): x(k)=A*x(k-1)+B*u(k)
  ...
  setIdentity(KF2.measurementMatrix);//観測データの配列(H)
  setIdentity(KF2.processNoiseCov, cv::Scalar::all(1e-1)); //誤差共分散の雑音(Q)
  setIdentity(KF2.measurementNoiseCov, cv::Scalar::all(1e-1));//観測誤差の標準偏差（Rt)(出力方程式の誤差共分散)
  setIdentity(KF2.errorCovPost, cv::Scalar::all(1e-1));//誤差共分散の更新式(P(k)): P(k)=(I-K(k)*H)*P'(k)
  ...
  cv::Mat img_src;
    ...
    cv::circle(img_src, cv::Point(kaisu2,300), 30, cv::Scalar(30, 90, 40), -1);//予測の点（黄色）
  ...
  /*while (1) {
    ...
    std::cout <<"観測座標_pos2.x="<<kaisu2<<std::endl;
    std::cout <<"観測座標_pos2.y="<<100<<std::endl;
    ...
    std::cout <<"測定関数KF2.correct(measurement2)="<<KF2.correct(measurement2)<<std::endl;
    std::cout <<"KF2.predict()="<<KF2.predict()<<std::endl;
    ...
    cv::circle(img_src, cv::Point(prediction2.at<float>(0), prediction2.at<float>(1)), 5, cv::Scalar(0, 255, 255), -1);//予測の点（黄色）
    cv::ellipse(img_src, cv::Point(prediction2.at<float>(0), prediction2.at<float>(1)),
    cv::Size(abs(prediction2.at<float>(2)), abs(prediction2.at<float>(3))),
    0.0, 0.0, 360.0, cv::Scalar(0, 255, 255), 3);//予測の円（黄色）
    std::cout <<"予測座標2x="<<prediction2.at<float>(0)<<std::endl;
    std::cout <<"予測座標2y="<<prediction2.at<float>(1)<<std::endl;
