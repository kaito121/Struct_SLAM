  ...
  //二回目動作プログラム--------------------------------------------------------------------------------------------
    ...
    int NotZeroL=-1;*/
      ...
      std::cout <<"OPT後マッチングの中心座標["<<i<<"].y="<<Template_Points_Curr[i].y<< std::endl;
		  ...
		  if (cv::norm(Template_Points_Prev[i] - points_curr_limited[i]) > 0.5) {
			  c = cv::Scalar(0, 100, 255);}//今の特徴点と一つ前の特徴点との差が0.5以上の時オレンジ
		  if(i==0){cv::circle(img_dst, Template_Points_Curr[i], 8, Scalar(0,255,255), -1, cv::LINE_AA);}
		  else{
			  cv::circle(img_dst, Template_Points_Curr[i], 4, c, -1, cv::LINE_AA);//今の座標情報
      ...
      if(req == 1){
        if(max_val1[i]>0.9){
          ...
          for(int karui=0;karui<karuman*2;karui++){
            if(karui<karuman){measurement.at<float>(karui) = Template_Points_Curr[karuix].x;
              std::cout <<"観測座標_pos["<<karui<<"].x="<<measurement.at<float>(karui)<<std::endl;
              karuix=karuix+1;}
            if(karui>=karuman){measurement.at<float>(karui) = Template_Points_Curr[karuiy].y;
