    ...
    if (reset == false) {//初回以降動作
      ...
      if(Tracking == true){
        ...
        for(int i=0;i<EST_MT_ok;i++){
          ...
          //cv::imshow("win_img_template1", MT_prev_Templ[i]);//黄色の特徴点を中心としたクロップ画像
          std::cout <<"img_template1.cols="<<img_template1.cols<<"img_template1.rows="<<img_template1.rows<< std::endl;//最初のフレーム
          std::cout <<"EST_MT_scope[i].cols="<<EST_MT_scope[i].cols<<"EST_MT_scope[i].rows="<<EST_MT_scope[i].rows<< std::endl;//最初のフレーム
