...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
    ...
    //FLD変換
    std::vector<cv::Vec4f> llines;
    cv::Ptr<cv::ximgproc::FastLineDetector> fld =  cv::ximgproc::createFastLineDetector();//特徴線クラスオブジェクトを作成
    fld->detect( img_gray, llines);//特徴線検索

    //FLDの線描写
    for(int i = 0; i < llines.size(); i++){
       //cv::line(img_dst,cv::Point(lines[i][0],lines[i][1]),cv::Point(lines[i][2],lines[i][3]),cv::Scalar(0,0,255), 4, cv::LINE_AA);  
       cv::line(img_line,cv::Point(llines[i][0],llines[i][1]),cv::Point(llines[i][2],llines[i][3]),cv::Scalar(0,0,255), 1.5, cv::LINE_AA); 
       cv::line(img_line2,cv::Point(llines[i][0],llines[i][1]),cv::Point(llines[i][2],llines[i][3]),cv::Scalar(0,0,255), 1.5, cv::LINE_AA);
    }
    ...
    double yokothetaCL[lines2.size()],tatethetaCL[lines2.size()],clusR;//クラスタリング用
    ...
    for (int j=0; j< ksk; ++j) {
        ...
        //縦線に分類
            ...
            if(lines2Y>lines2X){
                ...
                //if(tatetheta[tateyouso]>=90){
                //    tatethetaCL[tateyouso]=180-tatetheta[tateyouso];
                //}
                //else{tatethetaCL[tateyouso]=tatetheta[tateyouso];}
                //std::cout <<"tatethetaCL["<<tateyouso<<"](クラスタリング用)="<<tatethetaCL[tateyouso]<< std::endl;
    ...
    for (int i=0; i<tateyouso; ++i) {
       for (int j=i+1;j<tateyouso; ++j) {
            if (tatetheta[i] > tatetheta[j]) {
                tmp =  tatetheta[i];
                tmp1x =  tatelines2[i][0];
                tmp1y =  tatelines2[i][1];
                tmp2x =  tatelines2[i][2];
                tmp2y =  tatelines2[i][3];
                tmp3 =  tatelines2[i][4];

                tatetheta[i] = tatetheta[j];
                tatelines2[i][0] = tatelines2[j][0];
                tatelines2[i][1] = tatelines2[j][1];
                tatelines2[i][2] = tatelines2[j][2];
                tatelines2[i][3] = tatelines2[j][3];
                tatelines2[i][4] = tatelines2[j][4];

                tatetheta[j] = tmp;
                tatelines2[j][0] = tmp1x;
                tatelines2[j][1] = tmp1y;
                tatelines2[j][2] = tmp2x;
                tatelines2[j][3] = tmp2y;
                tatelines2[j][4] = tmp3;
            }
        }
    }
    ...
    for (int i=0; i<yokoyouso; ++i) {
       for (int j=i+1;j<yokoyouso; ++j) {
            if (yokotheta[i] > yokotheta[j]) {
                tmp =  yokotheta[i];
                tmp1x =  yokolines2[i][0];
                tmp1y =  yokolines2[i][1];
                tmp2x =  yokolines2[i][2];
                tmp2y =  yokolines2[i][3];
                tmp3 =  yokolines2[i][4];

                yokotheta[i] = yokotheta[j];
                yokolines2[i][0] = yokolines2[j][0];
                yokolines2[i][1] = yokolines2[j][1];
                yokolines2[i][2] = yokolines2[j][2];
                yokolines2[i][3] = yokolines2[j][3];
                yokolines2[i][4] = yokolines2[j][4];

                yokotheta[j] = tmp;
                yokolines2[j][0] = tmp1x;
                yokolines2[j][1] = tmp1y;
                yokolines2[j][2] = tmp2x;
                yokolines2[j][3] = tmp2y;
                yokolines2[j][4] = tmp3;
            }
        }
    }
