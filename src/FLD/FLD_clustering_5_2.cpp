...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
    ...
    double lines_NNM[lines_fld.size()][4],lines_NNM2[lines_fld.size()][4],lines_NNM_lc[lines_fld.size()],lines_NNM_thetal[lines_fld.size()],NNM_theta[lines_fld.size()];
	...
	double minlength = image.cols * image.cols * 0.002 ;// (線の最小長さの要件を定義する)

    //ナナメの線の抽出を行う-----------------------------------------------------------------------------------
    for(int i=0; i<lines_fld.size(); i++){
	    //(ほぼ垂直の場合は無視)
	    if ( abs(lines_fld[i][0]-lines_fld[i][2]) < 10 || abs(lines_fld[i][1]-lines_fld[i][3]) < 10){ //check if almost vertical
            lines3[lines3_count][0]=lines_fld[i][0];//ナナメの線以外を抽出する
            lines3[lines3_count][1]=lines_fld[i][1];
            lines3[lines3_count][2]=lines_fld[i][2];
            lines3[lines3_count][3]=lines_fld[i][3];
		...
		if( ((lines_fld[i][0]-lines_fld[i][2])*(lines_fld[i][0]-lines_fld[i][2]) +(lines_fld[i][1]-lines_fld[i][3])*(lines_fld[i][1]-lines_fld[i][3])) < minlength){
			continue;
        }   
        lines_NNM[lines_NNM_count][0]=lines_fld[i][0];//ナナメの線のみを抽出する
        lines_NNM[lines_NNM_count][1]=lines_fld[i][1];
        lines_NNM[lines_NNM_count][2]=lines_fld[i][2];
        lines_NNM[lines_NNM_count][3]=lines_fld[i][3];
        ...
        //座標から一次関数を引く関数
        //lines_NNM_thetal[lines_NNM_count]=M_PI-atan2((lines_NNM[lines_NNM_count][2]-lines_NNM[lines_NNM_count][0]),(lines_NNM[lines_NNM_count][3]-lines_NNM[lines_NNM_count][1]));
        ...
        NNM_theta[lines_NNM_count]=lines_NNM_thetal[lines_NNM_count]*180/M_PI;//deg表示化
        //θの範囲を0〜180にする
        if(NNM_theta[lines_NNM_count]>=180){
            NNM_theta[lines_NNM_count]=NNM_theta[lines_NNM_count]-180;
            lines_NNM_thetal[lines_NNM_count]=lines_NNM_thetal[lines_NNM_count]-M_PI;
        }

        cv::circle(img_graph, cv::Point(NNM_theta[lines_NNM_count]*6, 200), 3, Scalar(0,255,0), -1, cv::LINE_AA);//線の角度グラフ
    ...
    for (int j=0; j< lines3_count; ++j) {
        ...
        if(lines3X>lines3Y){
            ...
            cv::line(img_dst,cv::Point(yokolines3[yokoyouso][0],yokolines3[yokoyouso][1]),cv::Point(yokolines3[yokoyouso][2],yokolines3[yokoyouso][3]),cv::Scalar(255,0,0), 2, cv::LINE_AA);
          	//cv::circle(img_dst, cv::Point(yokolines3[yokoyouso][0],yokolines3[yokoyouso][1]), 4, cv::Scalar(255, 0, 50), 2);
          	//cv::circle(img_dst, cv::Point(yokolines3[yokoyouso][2],yokolines3[yokoyouso][3]), 4, cv::Scalar(255, 0, 50), 2);
    ...
    ////確率的ハフ変換
    //cv::cvtColor(img_FLD_Y, img_FLD_Y, cv::COLOR_RGB2GRAY);
    //cv::Canny(img_FLD_Y, img_FLD_Y, 125, 255);
    //std::vector<cv::Vec4i> lines_Y;
    //cv::HoughLinesP(img_FLD_Y, lines_Y, 1, CV_PI/180, 20,30,10);
    //for (int j=0; j< lines_Y.size(); ++j) {
    //cv::line(img_FLD_TY,cv::Point(lines_Y[j][0], lines_Y[j][1]),cv::Point(lines_Y[j][2],lines_Y[j][3]),cv::Scalar(255,0,0), 1, cv::LINE_AA);
    //}
    
    /*//確率的ハフ変換(タテ線はFLD単体よりFLD+確率ハフ変換でフィルタをかけたほうがノイズが少なくなる(ヨコ線はFLD単体のほうがノイズが少ない))
    tateyouso=0;
    cv::cvtColor(img_FLD_T, img_FLD_T, cv::COLOR_RGB2GRAY);
    cv::Canny(img_FLD_T, img_FLD_T, 125, 255);
    std::vector<cv::Vec4i> lines_T;
    cv::HoughLinesP(img_FLD_T, lines_T, 1, CV_PI/180, 20,30,10);
    for (int j=0; j< lines_T.size(); ++j) {
        cv::line(img_FLD_TY,cv::Point(lines_T[j][0], lines_T[j][1]),cv::Point(lines_T[j][2],lines_T[j][3]),cv::Scalar(0,0,255), 1, cv::LINE_AA);
        //FLD抽出線のy軸との角度を求める
        lines_T_theta[j]=M_PI-atan2((lines_T[j][2]-lines_T[j][0]),(lines_T[j][3]-lines_T[j][1]));
        std::cout <<"FLDの線の傾きlines_T_theta["<<j<<"]("<<lines_T_theta[j]<<")"<< std::endl;

        tatelines3[tateyouso][0]=lines_T[j][0];
        tatelines3[tateyouso][1]=lines_T[j][1];
        tatelines3[tateyouso][2]=lines_T[j][2];
        tatelines3[tateyouso][3]=lines_T[j][3];
        tatelines3[tateyouso][4]=lines_T_theta[j];

        //tatetheta[tateyouso]=lines_T_theta[j];
        tateZ1[tateyouso]=lines3_dep[j][0];
        tateZ2[tateyouso]=lines3_dep[j][1];

        tatetheta[tateyouso]=lines_T_theta[j]*180/M_PI;//deg表示化
    ...
    //タテ線とナナメ線の交点を検出する--------------------------------------------------------------------------------------------------
    double CrossPoint[5000][2];
    int CrossPointCount=0;
    //縦線の一次関数を求める
    for (int j=0; j<lines_NNM_count; ++j) {
        //std::cout <<"clust[MAXT="<<MAXT<<"]"<<clust[MAXT]<< std::endl;
        //std::cout <<"lines_NNM_count"<<lines_NNM_count<<"j="<<j<< std::endl;
        ...
        for (int i=0; i<clust[MAXT]; ++i) {
            //std::cout <<"NNM1111111"<< std::endl;
            //std::cout <<"clust[MAXT="<<MAXT<<"]="<<clust[MAXT]<< std::endl;
            //std::cout <<"tateclust[MAXT="<<MAXT<<"][i="<<i<<"][0]="<<tateclust[MAXT][i][0]<< std::endl;
            //std::cout <<"tateclust[MAXT="<<MAXT<<"][i="<<i<<"][2]="<<tateclust[MAXT][i][2]<< std::endl;
            ...
            CrossPoint[CrossPointCount][0]=NNM_TATE_X[j][i];
            CrossPoint[CrossPointCount][1]=NNM_TATE_Y[j][i];
            //std::cout <<"lines_fld.size()="<<lines_fld.size()<< std::endl;
            //std::cout <<"NNM_TATE_X[j="<<j<<"][i="<<i<<"]="<<NNM_TATE_X[j][i]<< std::endl;
            //std::cout <<"NNM_TATE_Y[j="<<j<<"][i="<<i<<"]="<<NNM_TATE_Y[j][i]<< std::endl;
            //std::cout <<"CrossPoint[CrossPointCount="<<CrossPointCount<<"][0]="<<CrossPoint[CrossPointCount][0]<< std::endl;
            //std::cout <<"CrossPoint[CrossPointCount="<<CrossPointCount<<"][1]="<<CrossPoint[CrossPointCount][1]<< std::endl;

            CrossPointCount=CrossPointCount+1;
    ...
    //縦線とナナメ線の交点と特徴点を比較し交点のクラスタ範囲内に特徴点があるかを検出する
    double long_point[CrossPointCount][points_curr.size()],ClusR2=5;//ClusR2の値は適当

    for (int i=0; i<CrossPointCount; ++i) {
        for (int j = 0; j < points_curr.size(); j++) {
            long_point[i][j]=sqrt((CrossPoint[i][0]-points_curr[j].x)*(CrossPoint[i][0]-points_curr[j].x)+(CrossPoint[i][1]-points_curr[j].y)*(CrossPoint[i][1]-points_curr[j].y));

            if(ClusR2>long_point[i][j]){//ここで交点と特徴点の範囲を見てる
  	            cv::circle(img_dst, cv::Point(points_curr[j].x, points_curr[j].y), 8, cv::Scalar(255, 0, 255), 2);
