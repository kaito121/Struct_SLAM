...
//20211201 FLD_marker2では線の識別を行なっておらず、線検出と追跡がくっついていない状態だった。
//そこでFLD_marker3では線検出とマッチング追跡結果を合わせることで線の追跡を行う。
//具体的には線検出の段階で、オドメトリーの値を利用して線のオドメトリー追跡（予測範囲)を求める。
//まず初回動作では線検出とテンプレート作成を行う
//2回目動作でもまた同様に線検出を行い、初回動作で検出した線の端点座標とオドメトリーの値を利用して線の予測範囲を求める。
//予測範囲内に線があれば、その線を同一線(仮)とする。(この時予測範囲外にある線は全て新規の線として初回と同様テンプレート作成を行う。)
//次にマッチングの結果を利用して、同一線(仮)が本当に同一線かどうかの確認を行う。
//もし同一線(仮)上にマッチングすれば、同一の線として保存(テンプレート更新を行う)
//同一線(仮)上にマッチングしなければ。同一線ではないので、新規線としてテンプレート作成を行う。
...
int main(int argc,char **argv){
  ...
  //LX=3.0,VX=0.25,omegaZ=2.15,THZ=0.25;
    ...
    MarkerW[4]= (cv::Mat_<float>(3, 1) <<-0.55, 0.28, 2.85);//実測値(X:
