...
//ここに処理項目
  ...
  for (int i=0; i<=lines2.size(); ++i) {
    ...
    //このグルーピングは縦横で分類したあと、水平か垂直かでさらに分類をしている
    ...
    for(int i = 0; i < yokoyouso; i++){
        ...
        if( yokoB==yokotheta[i+1]){
            ...
            //横成分の範囲を指定する
            //if((yokotheta[i+1]>=0.837981)&&(2.32537>=yokotheta[i+1])){
            //if((yokotheta[i+1]>=2.9670597)&&(3.14159265>=yokotheta[i+1])){
    ...
    for(int i = 0; i < tateyouso; i++){
        ...
        if( tateB==tatetheta[i+1]){
            ...
            //縦成分の範囲を指定する
            //if((tatetheta[i+1]>=0&&0.773351>=tatetheta[i+1])||(tatetheta[i+1]>=2.37158&&3.14159>=tatetheta[i+1])){
            //if((tatetheta[i+1]>=0&&0.1745329>=tatetheta[i+1])||(tatetheta[i+1]>=1.3962634&&1.745329>=tatetheta[i+1])){
