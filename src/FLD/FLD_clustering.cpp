...
//ここに処理項目
  ...
  for (int i=0; i<ksk; ++i) {
    ...
    int clusR,clusCY,clusCT,clusy[ksk],clust[ksk],MAXY=0,MAXT=0;//クラスタリング用
    ...
    if(yokoyouso==0){std::cout <<"実行しない--------------yokoyouso="<<yokoyouso<< std::endl;}
        ...
        for (int j=0; j< yokoyouso; ++j) {
            ...
            // クラスタリングされたヨコ線(クラスタ番号,クラスタ内部個数,成分番号)
                ...
                //クラスタリング半径内に属する角度を持つ線を一つのクラスタとしてまとめる
                if(clusR>=abs(yokotheta[j]*3-yokotheta[j+1]*3)){
                    std::cout <<"IF------------------クラスタ番号clusCY="<<clusCY<< std::endl;//クラスタ数
                    std::cout <<"abs(yokotheta["<<j<<"]*3-yokotheta["<<j+1<<"]*3)="<<abs(yokotheta[j]*3-yokotheta[j+1]*3)<< std::endl;
                    std::cout <<"abs(yokotheta[j="<<j<<"]*3)="<<abs(yokotheta[j]*3)<< std::endl;
                    std::cout <<"abs(yokotheta[j+1="<<j+1<<"]*3)="<<abs(yokotheta[j+1]*3)<< std::endl;
                    ...
                    std::cout <<"yokoclusy["<<clusCY<<"]["<<clusy[clusCY]<<"][0]="<<yokoclusy[clusCY][clusy[clusCY]][0]<< std::endl;//確認用
                    std::cout <<"yokoclusy["<<clusCY<<"]["<<clusy[clusCY]<<"][1]="<<yokoclusy[clusCY][clusy[clusCY]][1]<< std::endl;//確認用
                    std::cout <<"yokoclusy["<<clusCY<<"]["<<clusy[clusCY]<<"][2]="<<yokoclusy[clusCY][clusy[clusCY]][2]<< std::endl;//確認用
                    std::cout <<"yokoclusy["<<clusCY<<"]["<<clusy[clusCY]<<"][3]="<<yokoclusy[clusCY][clusy[clusCY]][3]<< std::endl;//確認用
                    cv::circle(img_line4,cv::Point(yokoclusy[clusCY][clusy[clusCY]][0],yokoclusy[clusCY][clusy[clusCY]][1]), 5, Scalar(30*clusCY,0,0), -1, cv::LINE_AA);//線の角度グラフ
                    cv::circle(img_line4,cv::Point(yokoclusy[clusCY][clusy[clusCY]][2],yokoclusy[clusCY][clusy[clusCY]][3]), 5, Scalar(30*clusCY,30*clusCY,0), -1, cv::LINE_AA);//線の角度グラフ
                    cv::circle(img_line3,cv::Point(yokoclusy[clusCY][clusy[clusCY]][0],yokoclusy[clusCY][clusy[clusCY]][1]), 5, Scalar(255,0,255), -1, cv::LINE_AA);//線の角度グラフ
                    cv::circle(img_line3,cv::Point(yokoclusy[clusCY][clusy[clusCY]][2],yokoclusy[clusCY][clusy[clusCY]][3]), 5, Scalar(0,255,255), -1, cv::LINE_AA);//線の角度グラフ

                    if(yokoclusy[clusCY][clusy[clusCY]][0]>640){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][1]>480){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][2]>640){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][3]>480){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][0]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][1]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][2]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(yokoclusy[clusCY][clusy[clusCY]][3]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}


                    cv::line(img_line4,cv::Point(yokoclusy[clusCY][clusy[clusCY]][0],yokoclusy[clusCY][clusy[clusCY]][1]),cv::Point(yokoclusy[clusCY][clusy[clusCY]][2],yokoclusy[clusCY][clusy[clusCY]][3]),cv::Scalar(30*clusCY,255,0), 5, cv::LINE_AA);
                    clusy[clusCY]=clusy[clusCY]+1;//クラスタの内部個数          
                }
                //線の角度の差がクラスタリング半径より大きい時（クラスタに属さない時→クラスタ更新）
                //更新時はクラスタの0番目を決める
                else{
                    std::cout <<"クラスタの内部個数clusy["<<clusCY<<"]="<<clusy[clusCY]<< std::endl;//クラスタの内部個数
                    std::cout <<"ELSE-----------------------------------------abs(yokotheta["<<j<<"]*3-yokotheta["<<j+1<<"]*3)="<<abs(yokotheta[j]*3-yokotheta[j+1]*3)<< std::endl;
                    std::cout <<"abs(yokotheta[j="<<j<<"]*3)="<<abs(yokotheta[j]*3)<< std::endl;
                    std::cout <<"abs(yokotheta[j+1="<<j+1<<"]*3)="<<abs(yokotheta[j+1]*3)<< std::endl;
                    //初回動作時でクラスタ半径以上の時(クラスタ代入＋クラスタ更新)
                    if(clusCY==0){
                        std::cout <<"初回動作時でクラスタ半径以上の時="<<clusCY<< std::endl;
                        MAXY=clusCY;
                        clusy[clusCY]=0;
                        clusy[MAXY]=0;
                        //// クラスタリングされたヨコ線(クラスタ番号,クラスタ内部個数,成分番号)
                        ...
                        //cv::line(img_line4,cv::Point(yokoclusy[clusCY][clusy[clusCY]][0],yokoclusy[clusCY][clusy[clusCY]][1]),cv::Point(yokoclusy[clusCY][clusy[clusCY]][2],yokoclusy[clusCY][clusy[clusCY]][3]),cv::Scalar(30*clusCY,255,0), 5, cv::LINE_AA);
                        //if(j+1!=yokoyouso){clusCY++;
                        //std::cout <<"クラスタ更新="<<clusCY<< std::endl;}//クラスタ更新
                    }//初回は強制MAXYに入れる

                    //else{
                        if(clusy[clusCY]>=clusy[MAXY]){ MAXY=clusCY; }//最大クラスタをキープする

                        std::cout <<"最大クラスタMAXY="<<MAXY<<",最大クラスタの内部個数clusy[MAXY]="<<clusy[MAXY]<< std::endl;
                        if(j+1!=yokoyouso&&clusCY!=0){
                            clusCY++;//クラスタ更新(最終動作時前は足さない)
                            std::cout <<"クラスタ更新="<<clusCY<< std::endl;}
                        clusy[clusCY]=0;//更新時初期化
                        ...
                        cv::line(img_line4,cv::Point(yokoclusy[clusCY][clusy[clusCY]][0],yokoclusy[clusCY][clusy[clusCY]][1]),cv::Point(yokoclusy[clusCY][clusy[clusCY]][2],yokoclusy[clusCY][clusy[clusCY]][3]),cv::Scalar(30*clusCY,255,0), 5, cv::LINE_AA);
                        //cv::line(img_line4,cv::Point(yokoclusy[clusCY][clusy[clusCY]][0],yokoclusy[clusCY][clusy[clusCY]][1]),cv::Point(yokoclusy[clusCY][clusy[clusCY]][2],yokoclusy[clusCY][clusy[clusCY]][3]),cv::Scalar(0,0,0), 5, cv::LINE_AA);
                        clusy[clusCY]=clusy[clusCY]+1;//クラスタの内部個数
                    //}
                }
                std::cout <<"yokotheta[j="<<j<<"]="<<yokotheta[j]<< std::endl;
            }
            //クラスタリング最終動作(for文の関係でこれ必要)
            if(j+1==yokoyouso){
                std::cout <<"最終Y-------------クラスタの内部個数clusy["<<clusCY<<"]="<<clusy[clusCY]<< std::endl;//クラスタの内部個数
                if(clusy[clusCY]>clusy[MAXY]){MAXY=clusCY;}
                std::cout <<"最終Y　最大クラスタMAXY="<<MAXY<<",最大クラスタの内部個数clusy[MAXY]="<<clusy[MAXY]<< std::endl;
                break;
            }
        }
        //一次関数を描写するプログラム
        /*for (int j=0; j<=clusy[MAXY]; ++j) {
         ...
         //座標から一次関数を引く関数
            ...
            //cv::line(img_line3,cv::Point(yokol[j][0],yokol[j][1]),cv::Point(yokol[j][2],yokol[j][3]),cv::Scalar(0,255,0), 1, cv::LINE_AA);
        }*/
    }
    ...
    clusR=2,clusCT=0;
    if(tateyouso!=0){
        std::cout <<"tateyouso="<<tateyouso<< std::endl;
        for (int j=0; j< tateyouso; ++j) {
            if(tateyouso==1){//要素がひとつしかないとき比較ができない
                std::cout <<"クラスタ番号clusCT="<<clusCT<< std::endl;//クラスタ数
                std::cout <<"IF------abs(tatetheta["<<j<<"]*3-tatetheta["<<j+1<<"]*3)="<<abs(tatetheta[j]*3-tatetheta[j+1]*3)<< std::endl;
                ...
                if(clusR>=abs(tatetheta[j]*3-tatetheta[j+1]*3)){
                    std::cout <<"clusCT="<<clusCT<< std::endl;
                    std::cout <<"abs(tatetheta["<<j<<"]*3-tatetheta["<<j+1<<"]*3)="<<abs(tatetheta[j]*3-tatetheta[j+1]*3)<< std::endl;
                    std::cout <<"abs(tatetheta[j="<<j<<"]*3)="<<abs(tatetheta[j]*3)<< std::endl;
                    std::cout <<"abs(tatetheta[j+1="<<j+1<<"]*3)="<<abs(tatetheta[j+1]*3)<< std::endl;

                    cv::circle(img_graph, cv::Point(tatetheta[j]*3, 240), 3, Scalar(0,30*clusCT,255), -1, cv::LINE_AA);

                    // クラスタリングされたタテ線(クラスタ番号,クラスタ内部個数,成分番号)
                    ...
                    std::cout <<"tateclust["<<clusCT<<"]["<<clust[clusCT]<<"][0]="<<tateclust[clusCT][clust[clusCT]][0]<< std::endl;//確認用
                    std::cout <<"tateclust["<<clusCT<<"]["<<clust[clusCT]<<"][1]="<<tateclust[clusCT][clust[clusCT]][1]<< std::endl;//確認用
                    std::cout <<"tateclust["<<clusCT<<"]["<<clust[clusCT]<<"][2]="<<tateclust[clusCT][clust[clusCT]][2]<< std::endl;//確認用
                    std::cout <<"tateclust["<<clusCT<<"]["<<clust[clusCT]<<"][3]="<<tateclust[clusCT][clust[clusCT]][3]<< std::endl;//確認用

                    cv::circle(img_line5,cv::Point(tateclust[clusCT][clust[clusCT]][0],tateclust[clusCT][clust[clusCT]][1]), 5, Scalar(30*clusCT,0,0), -1, cv::LINE_AA);//線の角度グラフ
                    cv::circle(img_line5,cv::Point(tateclust[clusCT][clust[clusCT]][2],tateclust[clusCT][clust[clusCT]][3]), 5, Scalar(30*clusCT,30*clusCT,0), -1, cv::LINE_AA);//線の角度グラフ
                    cv::circle(img_line3,cv::Point(tateclust[clusCT][clust[clusCT]][0],tateclust[clusCT][clust[clusCT]][1]), 5, Scalar(30*clusCT,0,0), -1, cv::LINE_AA);//線の角度グラフ
                    cv::circle(img_line3,cv::Point(tateclust[clusCT][clust[clusCT]][2],tateclust[clusCT][clust[clusCT]][3]), 5, Scalar(30*clusCT,30*clusCT,0), -1, cv::LINE_AA);//線の角度グラフ

                    if(tateclust[clusCT][clust[clusCT]][0]>640){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][1]>480){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][2]>640){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][3]>480){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][0]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][1]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][2]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}
                    if(tateclust[clusCT][clust[clusCT]][3]<0){std::cout <<"eraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;}



                    cv::line(img_line5,cv::Point(tateclust[clusCT][clust[clusCT]][0],tateclust[clusCT][clust[clusCT]][1]),cv::Point(tateclust[clusCT][clust[clusCT]][2],tateclust[clusCT][clust[clusCT]][3]),cv::Scalar(0,0,255), 2, cv::LINE_AA);
                    //cv::line(img_line5,cv::Point(tateclust[clusCT][clust[clusCT]][0],tateclust[clusCT][clust[clusCT]][1]),cv::Point(tateclust[clusCT][clust[clusCT]][2],tateclust[clusCT][clust[clusCT]][3]),cv::Scalar(0,30*clusCT,255), 2, cv::LINE_AA);

                    clust[clusCT]=clust[clusCT]+1;//クラスタの内部個数
                }
                else{
                    std::cout <<"クラスタの内部個数clust["<<clusCT<<"]="<<clust[clusCT]<< std::endl;//クラスタの内部個数
                    std::cout <<"ELSE---------abs(tatetheta["<<j<<"]*3-tatetheta["<<j+1<<"]*3)="<<abs(tatetheta[j]*3-tatetheta[j+1]*3)<< std::endl;

                    if(clusCT==0){MAXT=clusCT;}//初回は強制MAXYに入れる

                    if(clust[clusCT]>=clust[MAXT]){MAXT=clusCT;}//最大クラスタをキープする

                    std::cout <<"最大クラスタMAXT="<<MAXT<<",最大クラスタの内部個数clust[MAXT]="<<clust[MAXT]<< std::endl;
                    cv::line(img_line5,cv::Point(tateclust[clusCT][clust[clusCT]][0],tateclust[clusCT][clust[clusCT]][1]),cv::Point(tateclust[clusCT][clust[clusCT]][2],tateclust[clusCT][clust[clusCT]][3]),cv::Scalar(0,0,0), 2, cv::LINE_AA);

                    if(j+1!=tateyouso){clusCT++;}
                }
                std::cout <<"tatetheta["<<j<<"]="<<tatetheta[j]<< std::endl;
            }
            //クラスタリング最終動作(for文の関係でこれ必要)
            if(j+1==tateyouso){
                std::cout <<"クラスタの内部個数clust["<<clusCT<<"]="<<clust[clusCT]<< std::endl;//クラスタの内部個数
                if(clust[clusCT]>clust[MAXT]){ MAXT=clusCT;}//動作終了時に再度比較
                std::cout <<"最終T 最大クラスタMAXT="<<MAXT<<",最大クラスタの内部個数clust[MAXT]="<<clust[MAXT]<< std::endl;
                 break;
            }  
        }
        ...
        for (int j=0; j<=clust[MAXT]; ++j) {
            ...
            //cv::line(img_line3,cv::Point(tatel[j][0],tatel[j][1]),cv::Point(tatel[j][2],tatel[j][3]),cv::Scalar(0,0,255), 1, cv::LINE_AA);
        }
