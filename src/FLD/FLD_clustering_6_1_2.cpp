...
std::string win_CTL = "CTL";//線のクロップ画像
...
cv::Mat Crop_Tate_Line[100];//線のクロップ画像
cv::Point min_pt1[100], max_pt1[100],min_pt2[100], max_pt2[100],ALLmin_pt[100];//テンプレートマッチング用変数
double min_val1[100], max_val1[100],min_val2[100], max_val2[100],ALLmin_val[100];
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
    ...
    if(tateyouso==0){
       ...
       for (int j=0; j<clust[MAXT]; ++j) {
        ...
        //テンプレートマッチング(t,t-1秒間でテンプレマッチング→t-1秒での画像をテンプレートに使用)
        //ここでは常にtとt-1間しか見てないので追跡を行なっている訳ではない
        //一つ前のテンプレート数分回す(tで線が取得できずt-1のほうが線が多い場合マッチング結果が余る)
        //20211011 検出線でクロップした場合tとt-1で検出線とテンプレートの数が異なるためクロップした全ての線画像で全てのテンプレマッチングをする必要がある
        //つまり高速化にならないかも？？？
        //今検出した線分のクロップ画像を作成
        for(int i=0;i<tateno;i++){
            ALLmin_val[i]=1e+10;
            //クロップの不具合を考慮
            if(TATE_XN[i]<template_size*2){
                cv::Rect roiCTL(cv::Point(0,0), cv::Size(TATE_XN[i]+(template_size*2), 480));//線の中点を中心とした線の画像を切り取る(クロップ不具合考慮)
                Crop_Tate_Line[i] = img_src(roiCTL); // 切り出し画像
            }
            else if(template_size*2<=TATE_XN[i]&&TATE_XN[i]<=640-(template_size*2)){
                cv::Rect roiCTL(cv::Point(TATE_XN[i]-(template_size*2),0), cv::Size(template_size*4, 480));//線の中点を中心とした線の画像を切り取る
                Crop_Tate_Line[i] = img_src(roiCTL); // 切り出し画像
            }
            else if(640-(template_size*2)<TATE_XN[i]){
                cv::Rect roiCTL(cv::Point(TATE_XN[i]-(template_size*2),0), cv::Size((template_size*2)+640-TATE_XN[i], 480));//線の中点を中心とした線の画像を切り取る
                Crop_Tate_Line[i] = img_src(roiCTL); // 切り出し画像
            }
            //クロップした画像に対しテンプレートマッチングを行う
            for(int j=0;j<tateno_prev;j++){
                cv::rectangle(img_dst2, cv::Point(TATE_XN_prev[j]-template_size,TATE_YN_prev[j]+template_size), 
                 cv::Point(TATE_XN_prev[j]+template_size,TATE_YN_prev[j]-template_size), cv::Scalar(0, 0, 255), 1, cv::LINE_AA);//四角形を描写(t-1でのテンプレート作成地点)
                cv::Mat img_minmax2;
                //クロップした画像に対しテンプレートマッチング
                cv::matchTemplate(Crop_Tate_Line[i], FTL_template[j], img_minmax2, cv::TM_SQDIFF);//差分二乗和
                cv::minMaxLoc(img_minmax2, &min_val2[j], &max_val2[j], &min_pt2[j], &max_pt2[j]);
                std::cout << "min_val2(白)["<<j<<"]=" << min_val2[j] << std::endl;//一致度が上がると値が小さくなる
                std::cout << "max_val2(白)["<<j<<"]=" << max_val2[j] << std::endl;
                if(min_val2[j]<max_val2[j]*0.01){//最小値がしきい値以下なら表示
                    if(ALLmin_val[i]>min_val2[j]){//全てのマッチングの最小値を求める
                        ALLmin_val[i]=min_val2[j];
                        ALLmin_pt[i].x=min_pt2[j].x;
                        ALLmin_pt[i].y=min_pt2[j].y;
                    }
                }
            }
            int R,G,B;
            ...
            if(i==3){R=255,G=255,B=0;}
            if(i==4){R=255,G=0,B=255;}
            if(i==5){R=0,G=255,B=255;}
            if(i==6){R=255,G=255,B=255;}
            if(i==7){R=0,G=0,B=0;}
            if(i==8){R=255,G=100,B=0;}
            if(i==9){R=255,G=0,B=100;}
            if(i==10){R=0,G=255,B=100;}
            if(i==11){R=100,G=255,B=0;}
            if(i==12){R=100,G=0,B=255;}
            if(i==13){R=0,G=100,B=255;}
            if(i==14){R=0,G=50,B=100;}
            if(i==15){R=50,G=00,B=100;}

            if(TATE_XN[i]<30){
                cv::rectangle(img_dst2, cv::Rect(ALLmin_pt[i].x, ALLmin_pt[i].y, FTL_template[i].cols, FTL_template[i].rows), cv::Scalar(R, G, B), 3);//白枠
                std::cout << "マッチング成功min_val2["<<i<<"]=" << ALLmin_val[i] <<",(R,G,B)=("<<R<<","<<G<<","<<B<<")"<< std::endl;
                cv::line(img_dst2,cv::Point(ALLmin_pt[i].x, ALLmin_pt[i].y),cv::Point(TATE_XN_prev[i]-template_size,TATE_YN_prev[i]-template_size),cv::Scalar(0,255,255), 2, cv::LINE_AA);
            }
            else{
                cv::rectangle(img_dst2, cv::Rect(TATE_XN[i]-30+ALLmin_pt[i].x, ALLmin_pt[i].y, FTL_template[i].cols, FTL_template[i].rows), cv::Scalar(R, G, B), 3);//白枠
                std::cout << "マッチング成功min_val2["<<i<<"]=" << ALLmin_val[i] <<",(R,G,B)=("<<R<<","<<G<<","<<B<<")"<< std::endl;
                cv::line(img_dst2,cv::Point(TATE_XN[i]-30+ALLmin_pt[i].x, ALLmin_pt[i].y),cv::Point(TATE_XN_prev[i]-template_size,TATE_YN_prev[i]-template_size),cv::Scalar(0,255,255), 2, cv::LINE_AA);
            }

            cv::namedWindow(win_CTL, cv::WINDOW_AUTOSIZE);
            cv::imshow(win_CTL, Crop_Tate_Line[i]);
        ...
        /*for(int j=0;j<tateno_prev;j++){
            ...
            if(min_val1[j]<max_val1[j]*0.01){//最小値がしきい値以下なら表示
                int R,G,B;
                if(j==0){R=255,G=0,B=0;}
                if(j==1){R=0,G=255,B=0;}
                if(j==2){R=0,G=0,B=255;}
                if(j==3){R=255,G=255,B=0;}
                if(j==4){R=255,G=0,B=255;}
                if(j==5){R=0,G=255,B=255;}
                if(j==6){R=255,G=255,B=255;}
                if(j==7){R=0,G=0,B=0;}
                if(j==8){R=255,G=100,B=0;}
                if(j==9){R=255,G=0,B=100;}
                if(j==10){R=0,G=255,B=100;}
                if(j==11){R=100,G=255,B=0;}
                if(j==12){R=100,G=0,B=255;}
                if(j==13){R=0,G=100,B=255;}
                if(j==14){R=0,G=50,B=100;}
                if(j==15){R=50,G=00,B=100;}

                cv::rectangle(img_dst2, cv::Rect(min_pt1[j].x, min_pt1[j].y, FTL_template[j].cols, FTL_template[j].rows), cv::Scalar(R, G, B), 3);//白枠
                std::cout << "マッチング成功min_val1["<<j<<"]=" << min_val1[j] <<",(R,G,B)=("<<R<<","<<G<<","<<B<<")"<< std::endl;
