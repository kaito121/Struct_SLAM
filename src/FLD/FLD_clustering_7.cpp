...
std::string win_line_curr = "line_curr";
std::string win_line_prev = "line_prev";
std::string win_line_dst = "line_dst";
...
int linesu=400;
int kosuu=0,PT_kosuu=0,PT2_kosuu=0,points_kosuu=0;

int depth_line_points_tate_prev_ok=0; //depth取得可能な特徴点の数
int depth_line_points_tate_curr_ok=0; //depth取得可能な特徴点の数
int depth_line_points_tate_currA_ok=0; //depth取得可能な特徴点の数
...
cv::Mat img_line_prev,img_line_curr,img_line_dst;//線の追跡用画像
...
vector<cv::Point2f> line_points_tate_prev, line_points_tate_curr;//線のトラッキング用ポイント
vector<cv::Point2f> line_points_tate_prevA, line_points_tate_currA;//線のトラッキング用ポイント
vector<cv::Point2f> ALL_line_points_tate_prev, ALL_line_points_tate_curr;//線のトラッキング用ポイント

float depth_line_points_tate_prev[600],depth_line_points_tate_curr[600];
float depth_line_points_tate_prevA[600],depth_line_points_tate_currA[600];
...
int kaisu=0,optkaisu;//行列初期設定用変数
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
    ...
    cv::Mat depthimage = bridgedepthImage->image.clone();//image変数に変換した画像データを代入
    ...
    img_line_curr = img_src.clone();
    img_line_curr = cv::Scalar(255,255,255);
    ...
    int ksk=0;
    for(int i =  0; i < lines_fld.size(); i++){
        dep1[i]= depthimage.at<float>(cv::Point(lines_fld[i][0],lines_fld[i][1]));//点の三次元距離データ取得//20210908ここでDepth情報を取得する必要はない気がする
        dep2[i]= depthimage.at<float>(cv::Point(lines_fld[i][2],lines_fld[i][3]));
    ...
    /*for (int i=0; i<lines3_count; ++i) {
       for (int j=i+1;j<lines3_count; ++j) {
            if (lines3_theta[i] > lines3_theta[j]) {
                tmp =  lines3_theta[i];
                tmp1x =  lines3[i][0];
                tmp1y =  lines3[i][1];
                tmp2x =  lines3[i][2];
                tmp2y =  lines3[i][3];
                tmpdep1 = lines3_dep[i][0];
                tmpdep2 = lines3_dep[i][1];

                lines3_theta[i] = lines3_theta[j];
                lines3[i][0] = lines3[j][0];
                lines3[i][1] = lines3[j][1];
                lines3[i][2] = lines3[j][2];
                lines3[i][3] = lines3[j][3];
                lines3_dep[i][0] = lines3_dep[j][0];
                lines3_dep[i][1] = lines3_dep[j][1];
                ...
                lines3[j][2] = tmp2x;
                lines3[j][3] = tmp2y;
                lines3_dep[j][0] = tmpdep1;
                lines3_dep[j][1] = tmpdep2;
            }
        }
    }*/
    ...
    if(tateyouso==0){
       ...
       for (int j=0; j<tateok; ++j) {
        ...
        for (int j=0; j<tateno; ++j) {
            ...
            //トラッキング用
            cv::line(img_line_curr,cv::Point(tatel[j][0],tatel[j][1]),cv::Point(tatel[j][2],tatel[j][3]),cv::Scalar(0,0,255), 1, cv::LINE_AA);
            //cv::line(img_line_curr,cv::Point(TATE_line[j][0],TATE_line[j][1]),cv::Point(TATE_line[j][2],TATE_line[j][3]),cv::Scalar(0,0,255), 4, cv::LINE_AA);
            cv::circle(img_line_curr, cv::Point((TATE_line[j][0]+TATE_line[j][2])/2,240),  3, Scalar(0,255,0), -1, cv::LINE_AA);
            cv::circle(img_line_curr, cv::Point((TATE_line[j][0]+TATE_line[j][2])/2,240+20), 3, Scalar(0,255,0), -1, cv::LINE_AA);
            cv::circle(img_line_curr, cv::Point((TATE_line[j][0]+TATE_line[j][2])/2,240-20), 3, Scalar(0,255,0), -1, cv::LINE_AA);

            //cv::line(img_line_curr,cv::Point(((TATE_line[j][0]+TATE_line[j][2])/2)+10,240),cv::Point(((TATE_line[j][0]+TATE_line[j][2])/2)-10,240),cv::Scalar(0,0,255), 1, cv::LINE_AA);
            //cv::line(img_line_curr,cv::Point(((TATE_line[j][0]+TATE_line[j][2])/2)+10,240+20),cv::Point(((TATE_line[j][0]+TATE_line[j][2])/2)-10,240+20),cv::Scalar(0,0,255), 1, cv::LINE_AA);
            //cv::line(img_line_curr,cv::Point(((TATE_line[j][0]+TATE_line[j][2])/2)+10,240-20),cv::Point(((TATE_line[j][0]+TATE_line[j][2])/2)-10,240-20),cv::Scalar(0,0,255), 1, cv::LINE_AA);
            ...
            if(j==0){
            cv::line(img_dst2,cv::Point(TATE_line[j][0],TATE_line[j][1]),cv::Point(TATE_line[j][2],TATE_line[j][3]),cv::Scalar(255,255,255), 4, cv::LINE_AA);//白
            }
            else if(j==1){
            cv::line(img_dst2,cv::Point(TATE_line[j][0],TATE_line[j][1]),cv::Point(TATE_line[j][2],TATE_line[j][3]),cv::Scalar(255,0,0), 4, cv::LINE_AA);//青
            }
            else if(j==2){
            cv::line(img_dst2,cv::Point(TATE_line[j][0],TATE_line[j][1]),cv::Point(TATE_line[j][2],TATE_line[j][3]),cv::Scalar(0,255,0), 4, cv::LINE_AA);//緑
            }
            else if(j==3){
            cv::line(img_dst2,cv::Point(TATE_line[j][0],TATE_line[j][1]),cv::Point(TATE_line[j][2],TATE_line[j][3]),cv::Scalar(0,255,255), 4, cv::LINE_AA);//黄色
            }
            else{
    ...
    //線の追跡------------------------------
    //コーナー検出とオプティカルフローを使って無理やり追跡する
    //線だけの画像を作り、その画像に対しコーナー検出を行う。ただ普通の線ではうまくできないと思うから線に特徴をつけて無理やりトラッキングさせる
    //この時線が上下に動くので中点だとぶれてしまう。そこで画像の中心位置に線の特徴を固定させてトラッキングする。
    //トラッキングした線の３次元データは検出画像座標を使用してDepth画像から取り出す。
    //img_line_curr.copyTo(img_line_dst);
    img_line_dst = img_line_curr.clone();


	cv::cvtColor(img_line_curr, img_line_curr, cv::COLOR_BGR2GRAY);//グレースケール化
  if (reset == true) {
	  std::cout <<"初回検出プログラム"<< std::endl;
    depth_line_points_tate_prev_ok=0; //depth取得可能な特徴点の数(初期化)
    swap_on=false;
    //cv::goodFeaturesToTrack(今画像, 今の特徴点, 特徴点の個数, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
	cv::goodFeaturesToTrack(img_line_curr, line_points_tate_curr, 50, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);// 特徴点検出(グレースケール画像から特徴点検出)
    if(line_points_tate_curr.size()>0){
	    cv::cornerSubPix(img_line_curr, line_points_tate_curr, cv::Size(10, 10), cv::Size(-1, -1), cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 20, 0.03));
    }
	line_points_tate_prev = line_points_tate_curr;//データのコピ
    //camera_point_p.resize(line_points_tate_prev.size());//要素数初期設定(座標変換用)
	  std::cout <<"test2_line_points_tate_prev.size()="<<line_points_tate_prev.size()<< std::endl;
    
	for (int i = 0; i < line_points_tate_prev.size(); i++) {
		cv::circle(img_line_dst, line_points_tate_prev[i], 6, Scalar(255,0,0), -1, cv::LINE_AA);
		cv::circle(img_line_curr, line_points_tate_prev[i], 6, Scalar(255,0,0), -1, cv::LINE_AA);
        //画像→カメラ座標変換----------------------------------------------------------------------
          depth_line_points_tate_prev[i] = depthimage.at<float>(cv::Point(line_points_tate_prev[i].x,line_points_tate_prev[i].y));
        //Depthが取得できない特徴点を削除する+Depthの外れ値を除く
        if(depth_line_points_tate_prev[i]>0.001&&depth_line_points_tate_prev[i]<10000){
          //camera_point_p[depth_line_points_tate_prev_ok].x = depth_line_points_tate_prev[i] * ((line_points_tate_prev[i].x - camera_info.K[2]) / camera_info.K[0])/1000;//メートル表示変換
          //camera_point_p[depth_line_points_tate_prev_ok].y = depth_line_points_tate_prev[i] * ((line_points_tate_prev[i].y - camera_info.K[5]) / camera_info.K[4])/1000;
          //camera_point_p[depth_line_points_tate_prev_ok].z = depth_line_points_tate_prev[i]/1000;
          //std::cout << "特徴点のカメラ座標:camera_point_p[depth_line_points_tate_prev_ok="<<depth_line_points_tate_prev_ok<<"]={"<< camera_point_p[depth_line_points_tate_prev_ok].x <<","<<camera_point_p[depth_line_points_tate_prev_ok].y<<","<<camera_point_p[depth_line_points_tate_prev_ok].z<<"}"<< std::endl;
          line_points_tate_prev[depth_line_points_tate_prev_ok] = line_points_tate_prev[i];
          depth_line_points_tate_prev_ok=depth_line_points_tate_prev_ok+1;//Depth取得可能の個数をカウント
          }
	}
    line_points_tate_prev.resize(depth_line_points_tate_prev_ok);//Depth取得可能数でリサイズ(二次元)
    //camera_point_p.resize(depth_line_points_tate_prev_ok);//Depth取得可能数でリサイズ(三次元カメラ座標)
    reset = false;//if文切り替え
  ...
  //else if(tateno>0){
	  std::cout <<"tateno="<<tateno<< std::endl;// 特徴点追跡(二回目のフレーム)
	  std::cout <<"line_points_tate_prev.size()="<<line_points_tate_prev.size()<< std::endl;// 特徴点追跡(二回目のフレーム)

    if(line_points_tate_prev.size()>0){
	  std::cout <<"オプティカルフロー"<< std::endl;// 特徴点追跡(二回目のフレーム)
	  vector<uchar> status;//特徴点の数
	  vector<float> err;
      swap_on = true;
      depth_line_points_tate_curr_ok=0; //depth取得可能な特徴点の数(初期化)

        cv::calcOpticalFlowPyrLK(img_line_prev, img_line_curr, line_points_tate_prev, line_points_tate_curr, status, err);//オプティカルフロー
        //一つ前のOPTで追跡可能だったline_points_tate_prevと
        //cv::calcOpticalFlowPyrLK(image_prev, image_curr, line_points_tate_curr, line_points_tate_prev, status, err);//オプティカルフロー(line_points_tate_currは追跡可能points)
	      ...
	      for (i = k = n =0; i < status.size(); i++){
	      	...
	      	if (status[i] != 0) {	
	      	  line_points_tate_prev[k] = line_points_tate_prev[i];
            //camera_point_p[k] = camera_point_p[i];
	      	  line_points_tate_curr[k++] = line_points_tate_curr[i];
	      	}
        }
	    line_points_tate_curr.resize(k);//ここでkの個数でリサイズ
	    line_points_tate_prev.resize(k);
	    //camera_point_p.resize(k);
	    //camera_point_c.resize(k);

        for (j= n = 0; j < line_points_tate_curr.size(); j++){
          //追跡距離が極端に長い追跡を削除(追跡距離が100以下の時のみ使用)
          if(abs(line_points_tate_prev[j].x-line_points_tate_curr[j].x)<50||abs(line_points_tate_prev[j].y-line_points_tate_curr[j].y)<50){
            	line_points_tate_prev[n] = line_points_tate_prev[j];
              //camera_point_p[n] = camera_point_p[j];
	        	  line_points_tate_curr[n++] = line_points_tate_curr[j];
          }
        }
          line_points_tate_curr.resize(n);//ここでkの個数でリサイズ
	      line_points_tate_prev.resize(n);
	      //camera_point_p.resize(n);
	      //camera_point_c.resize(n);
		

	      // 特徴点を丸で描く-------------------------------------------------------------------------------------------
	    for (int i = 0; i < line_points_tate_curr.size(); i++) {
            //std::cout <<"OPT後マッチングの中心座標["<<i<<"]="<<line_points_tate_curr[i]<< std::endl;
            cv::circle(img_line_dst, line_points_tate_prev[i], 3, Scalar(255,255,255), -1, cv::LINE_AA);//一つ前の画像の座標
	        cv::circle(img_line_dst, line_points_tate_curr[i], 4, Scalar(0, 255, 0), -1, cv::LINE_AA);//今の座標情報
	          //cv::line(img_line_dst,cv::Point(line_points_tate_curr[i]),cv::Point(line_points_tate_prev[i]),cv::Scalar(0,255,255), 1, cv::LINE_AA);//線を描写する
            cv::line(img_line_dst,cv::Point(line_points_tate_curr[i].x,line_points_tate_prev[i].y),cv::Point(line_points_tate_prev[i]),cv::Scalar(255,0,0), 1, cv::LINE_AA);//線を描写する
            cv::line(img_line_dst,cv::Point(line_points_tate_prev[i].x,line_points_tate_curr[i].y),cv::Point(line_points_tate_prev[i]),cv::Scalar(0,255,0), 1, cv::LINE_AA);//線を描写する

            //画像→カメラ座標変換----------------------------------------------------------------------
                depth_line_points_tate_curr[i] = depthimage.at<float>(cv::Point(line_points_tate_curr[i].x,line_points_tate_curr[i].y));
                //Depthが取得できない特徴点を削除する+Depthの外れ値を除く
                if(depth_line_points_tate_curr[i]>0.001&&depth_line_points_tate_curr[i]<10000){
                  //camera_point_c[depth_line_points_tate_curr_ok].x = depth_line_points_tate_curr[i] * ((line_points_tate_curr[i].x - camera_info.K[2]) / camera_info.K[0])/1000;//メートル表示変換
                  //camera_point_c[depth_line_points_tate_curr_ok].y = depth_line_points_tate_curr[i] * ((line_points_tate_curr[i].y - camera_info.K[5]) / camera_info.K[4])/1000;
                  //camera_point_c[depth_line_points_tate_curr_ok].z = depth_line_points_tate_curr[i]/1000;
                  //std::cout << "特徴点のカメラ座標:camera_point_c[depth_line_points_tate_curr_ok="<<depth_line_points_tate_curr_ok<<"]={"<< camera_point_c[depth_line_points_tate_curr_ok].x <<","<<camera_point_c[depth_line_points_tate_curr_ok].y<<","<<camera_point_c[depth_line_points_tate_curr_ok].z<<"}"<< std::endl;
                  //camera_point_p[depth_line_points_tate_curr_ok]=camera_point_p[i];
                  line_points_tate_prev[depth_line_points_tate_curr_ok] = line_points_tate_prev[i];
                  line_points_tate_curr[depth_line_points_tate_curr_ok] = line_points_tate_curr[i];
                  depth_line_points_tate_curr_ok=depth_line_points_tate_curr_ok+1;//Depth取得可能の個数をカウント
                }
        }
        cv::circle(img_line_dst, line_points_tate_prev[0], 4, Scalar(0,0,255), -1, cv::LINE_AA);//一つ前の画像の座標
    }
    depth_line_points_tate_currA_ok=0;//depth取得可能な特徴点の数(初期化)
    if(line_points_tate_prev.size()<3){//特徴点が100個以下になったら再び特徴点を検出する
      //特徴点の追加（今現在の画像から検出してオプティカルフロー追跡点に足して、次回のオプティカルフローに使用する)
		cv::goodFeaturesToTrack(img_line_curr, line_points_tate_currA, 50, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);// 特徴点検出(グレースケール画像から特徴点検出)
        if(line_points_tate_currA.size()>0){
		    cv::cornerSubPix(img_line_curr, line_points_tate_currA, cv::Size(10, 10), cv::Size(-1, -1), cv::TermCriteria(cv::TermCriteria::COUNT | cv::TermCriteria::EPS, 20, 0.03));
        }
		//line_points_tate_prev = line_points_tate_curr;//データのコピ
        //camera_point_cA.resize(line_points_tate_currA.size());//要素数初期設定(座標変換用)
	    std::cout <<"test2_line_points_tate_prev.size()="<<line_points_tate_prev.size()<< std::endl;
      
		for (int i = 0; i < line_points_tate_currA.size(); i++) {
		    cv::circle(img_line_dst, line_points_tate_currA[i], 3, Scalar(255,255,0), -1, cv::LINE_AA);

            ////画像→カメラ座標変換----------------------------------------------------------------------
            depth_line_points_tate_currA[i] = depthimage.at<float>(cv::Point(line_points_tate_currA[i].x,line_points_tate_currA[i].y));
            //Depthが取得できない特徴点を削除する+Depthの外れ値を除く
            if(depth_line_points_tate_currA[i]>0.001&&depth_line_points_tate_currA[i]<10000){
              //camera_point_cA[depth_line_points_tate_currA_ok].x = depth_line_points_tate_currA[i] * ((line_points_tate_currA[i].x - camera_info.K[2]) / camera_info.K[0])/1000;//メートル表示変換
              //camera_point_cA[depth_line_points_tate_currA_ok].y = depth_line_points_tate_currA[i] * ((line_points_tate_currA[i].y - camera_info.K[5]) / camera_info.K[4])/1000;
              //camera_point_cA[depth_line_points_tate_currA_ok].z = depth_line_points_tate_currA[i]/1000;
              //std::cout << "特徴点のカメラ座標:camera_point_p[depth_line_points_tate_currA_ok="<<depth_line_points_tate_currA_ok<<"]={"<< camera_point_p[depth_line_points_tate_currA_ok].x <<","<<camera_point_p[depth_line_points_tate_currA_ok].y<<","<<camera_point_p[depth_line_points_tate_currA_ok].z<<"}"<< std::endl;
              line_points_tate_currA[depth_line_points_tate_currA_ok] = line_points_tate_currA[i];
              depth_line_points_tate_currA_ok=depth_line_points_tate_currA_ok+1;//Depth取得可能の個数をカウント
		      cv::circle(img_line_dst, line_points_tate_currA[i], 3, Scalar(255,255,0), -1, cv::LINE_AA);
            }
		}
      line_points_tate_currA.resize(depth_line_points_tate_currA_ok);//Depth取得可能数でリサイズ(二次元)
      //camera_point_cA.resize(depth_line_points_tate_currA_ok);//Depth取得可能数でリサイズ(三次元カメラ座標)
    }
  }
  //else{swap_on =false;
  //       reset = true;
  //}
    ...
    cv::namedWindow(win_line_curr, cv::WINDOW_AUTOSIZE);
    cv::namedWindow(win_line_prev, cv::WINDOW_AUTOSIZE);
    cv::namedWindow(win_line_dst, cv::WINDOW_AUTOSIZE);
    ...
    cv::imshow(win_line_curr, img_line_curr);
    cv::imshow(win_line_dst, img_line_dst);

    if(kaisu!=0){
        cv::imshow(win_line_prev, img_line_prev);
    }
    cv::swap(img_line_curr, img_line_prev);//トラッキング用画像の保存

    if(swap_on ==true){//初回プログラム実行時はswapしない(データの浅いコピーの影響を考慮)
        points_kosuu=0;
        std::cout <<"points_kosuu="<<points_kosuu<<std::endl;
        std::cout <<"depth_line_points_tate_curr_ok="<<depth_line_points_tate_curr_ok<<std::endl;
        std::cout <<"depth_line_points_tate_currA_ok="<<depth_line_points_tate_currA_ok<<std::endl;

        ALL_line_points_tate_curr.resize(depth_line_points_tate_curr_ok+depth_line_points_tate_currA_ok);
        //ALL_camera_point_c.resize(depth_line_points_tate_curr_ok+depth_line_points_tate_currA_ok);
        for(int i=0;i<depth_line_points_tate_curr_ok;i++){
          ALL_line_points_tate_curr[points_kosuu]=line_points_tate_curr[i];//追跡可能点に検出点をプラスする
          //ALL_camera_point_c[points_kosuu]=camera_point_c[i];
          points_kosuu=points_kosuu+1;
        }
        //同じ場所のpointを消したい
        for(int i=0;i<depth_line_points_tate_currA_ok;i++){
          PLAS_POINT=true;
          for(int j=0;j<depth_line_points_tate_curr_ok;j++){
            //検出1点に対してすべての追跡点で検査→１つでもかぶる点があるならその検出点を消去
            if(abs(line_points_tate_curr[j].x-line_points_tate_currA[i].x)<1.5&&abs(line_points_tate_curr[j].y-line_points_tate_currA[i].y)<1.5){//差が0.1以下なら同じ点とみなして消去
              PLAS_POINT=false;//更新オフ
              continue;//かぶる点が検出された時for分終了 
            }
          }
          if(PLAS_POINT==true){//かぶる点が無いとき追加動作実行
            ALL_line_points_tate_curr[points_kosuu]=line_points_tate_currA[i];//追跡可能点に検出点をプラスする
            //ALL_camera_point_c[points_kosuu]=camera_point_cA[i];
            points_kosuu=points_kosuu+1;
          }
        }
        cv::swap(ALL_line_points_tate_curr, line_points_tate_prev);//二次元画像座標を保存(line_points_tate_curr→line_points_tate_prev)(追跡可能点をline_points_tate_prevに保存)
        //cv::swap(ALL_camera_point_c, camera_point_p);//三次元カメラ座標を保存(camera_point_c→camera_point_p)
    }

    //cv::swap(line_points_tate_curr, line_points_tate_prev);//トラッキング用ポイントの保存(仮)



    //cv::swap(line_line_points_tate_curr_tate, line_line_points_tate_prev_tate);//トラッキング用ポイントの保存
