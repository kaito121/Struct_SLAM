...
// Mutex for shared variables over threads(スレッド上の共有変数のためのミューテックス)
	boost::mutex mMutexMCurrentInvalidatingDistance;
#endif

// Costructor
JLinkage::JLinkage(
	float (*nDistanceFunction)(const std::vector<float>  &nModel, const std::vector<float>  &nDataPt), // The Distance function(距離関数)
	float nInliersThreshold, // Scale of the algorithm(アルゴリズムの規模)
	unsigned int nModelBufferSize, // Model buffer size, set to 0 to allow growable size(モデル・バッファ・サイズ、0に設定すると成長可能なサイズになる)
	bool nCopyPtsCoords, // Copy the coords value of the new points or use directly the pointer? (The second choice is faster but you need to deallocate memory outside the class)(新しい点の座標値をコピーするか、ポインタを直接使うか（2番目の選択は速いが、クラスの外でメモリを解放する必要がある）。)
	unsigned int nPtDimension, //Used by Kd-Tree(Kd-Treeで使用)
	int nKNeighboards,  //K-neighboards for each points to be used for the clusterization (neg value means use all the dataset)(クラスタ化に使用する各点のK-neighboards (負の値はデータセットをすべて使用することを意味します))
	bool (*nClusterClusterDiscardTest)(const sClLnk *, const sClLnk *), // Function handler to perform a cluster-cluster test. Should return true if the cluster can be merged by some additional condition, false otherwise.(クラスタ-クラスタテストを行うための関数ハンドラ．何らかの追加条件でクラスタを統合できる場合はtrueを、そうでない場合はfalseを返さなければなりません。)
	void (*nInitializeAdditionalData)(sClLnk *), // Function handler -  initialization routine when additional data are created(関数ハンドラ - 追加データが作成されたときの初期化ルーチン)
	void (*nClusterMergeAdditionalOperation)(const sClLnk *), // Function handler - called when two clusters are merged(関数ハンドラ - 2つのクラスタが結合されるときに呼び出される。)
	void (*nDestroyAdditionalData)(sClLnk *) // Function handler -  called when additional data are destroyed(関数ハンドラ - 追加データが破棄されるときに呼び出される)
):
