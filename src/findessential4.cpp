...
#include<struct_slam/plm.h>//自作メッセージ用ヘッダ
#include <opencv/highgui.h>


ros::Subscriber sub;//データをsubcribeする奴
ros::Publisher  pub_plm;
...
//行列の要素抜き出し（数値)
  ...
  struct_slam::plm pclP;
  struct_slam::plm0 plm0;
  plm0.R=R;
  plm0.t=t;
  plm0.Rt=Rt;

  plmP.plmP.emplace_back(plm0);//配列に送るデータを追加
  plmP.header.stamp = ros::Time::now();//トピックにパブリッシュした（データを送った
  pub_plm.publish(plmP);
	...
	cv::imwrite(file_src, RGBimage);//画像書き込み
	cv::waitKey(1);
...
int main(int argc,char **argv){
    ...
    pub_pla=nhPub.advertise<struct_slam::plm>("plm", 1000);//パブリッシュ設定
