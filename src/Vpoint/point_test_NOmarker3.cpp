...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg, const sensor_msgs::CameraInfo::ConstPtr& cam_info)
{
  ...
  //オプティカルフロー-------------------------------------------------------------------------------------
    ...
    if(camera_point_p.size()<500||k<500){//特徴点が100個以下になったら再び特徴点を検出する
      //特徴点の追加（今現在の画像から検出してオプティカルフロー追跡点に足して、次回のオプティカルフローに使用する)
		  cv::goodFeaturesToTrack(image_curr, points_currA, 300, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);// 特徴点検出(グレースケール画像から特徴点検出)
...
*/
  ...
  if(swap_on ==true){//初回プログラム実行時はswapしない(データの浅いコピーの影響を考慮)
    ...
    //for(int i=0;i<depth_point_curr_ok;i++){
    //  for(int j=0;j<depth_points_currA_ok;j++){
    //    ALL_points_curr[points_kosuu]=points_curr[i]+points_currA[j];//追跡可能点に検出点をプラスする
    //    ALL_camera_point_c[points_kosuu]=camera_point_c[i]+camera_point_cA[j];
    ...
    //std::cout <<"ALL_points_curr.size()="<<ALL_points_curr.size()<<std::endl;
    //std::cout <<"ALL_camera_point_c.size()="<<ALL_camera_point_c.size()<<std::endl;
    //std::cout <<"points_currA.size()="<<points_currA.size()<<std::endl;
    //std::cout <<"camera_point_cA.size()="<<camera_point_cA.size()<<std::endl;
    //std::cout <<"points_currA[i]="<<points_currA[0].x<<","<<points_currA[0].y<<std::endl;
    //std::cout <<"camera_point_cA[i]="<<camera_point_cA[0].x<<","<<camera_point_cA[0].y<<std::endl;
    ...
    for(int i=0;i<depth_points_currA_ok;i++){
      ...
      for(int j=0;j<depth_point_curr_ok;j++){
        ...
        if(abs(points_curr[j].x-points_currA[i].x)<0.5&&abs(points_curr[j].y-points_currA[i].y)<0.5){//差が0.1以下なら同じ点とみなして消去
