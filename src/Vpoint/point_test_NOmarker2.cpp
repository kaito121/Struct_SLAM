...
bool swap_on2 = false;//初回プログラム実行時はswapしない(データの浅いコピーの影響を考慮)
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg, const sensor_msgs::CameraInfo::ConstPtr& cam_info)
{
  ...
  if (reset2 == true) {
    ...
    depth_point_prev2_ok=0; //depth取得可能な特徴点の数(初期化)
    swap_on2=false;
    //cv::goodFeaturesToTrack(今画像, 前画像, 特徴点の個数, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);
		cv::goodFeaturesToTrack(image_curr, points_curr2, 100, 0.01, 10, cv::Mat(), 3, 3, 0, 0.04);// 特徴点検出(グレースケール画像から特徴点検出)
  ...
  //オプティカルフロー2-------------------------------------------------------------------------------------
	  ...
	  vector<float> err2;
    swap_on2 = true;
    depth_point_curr2_ok=0; //depth取得可能な特徴点の数(初期化)

    cv::calcOpticalFlowPyrLK(image_prev, image_curr, points_prev2, points_curr2,  status2, err2);//オプティカルフロー
...
*/
    ...
    //camera_base_pose.orientation.w = 1;
    ...
    //tf(camera_base camera_link間のlink)-----------------------------------------------------------------------------------------
      ...
      //Camera_BasetoCamera_Link_pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(YawZ,RollX,PitchY);
