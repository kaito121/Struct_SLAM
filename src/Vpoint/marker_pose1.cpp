...
ros::Publisher pub_pcl;//クラスタリング用PCL
...
//クラスタリング用定義(ここは変更してもいいかも)
double X_wariai=749.85;
double Y_wariai=749.85;
double X_pcl=500;
double Y_pcl=500;
double Z_pcl=1.0;
  ...
  //オプティカルフロー-------------------------------------------------------------------------------------
    ...
    points_curr.resize(n);//ここでkの個数でリサイズ
      ...
      if(depth_point_curr[i]>0.001&&depth_point_curr[i]<10000){
        ...
        //jk.x= camera_point_c[depth_point_curr_ok].x;
	      //jk.y= camera_point_c[depth_point_curr_ok].y;
	      //jk.z= camera_point_c[depth_point_curr_ok].z;

	      jk.x=(float)points_curr[i].x/X_wariai;//ピクセル距離をクラスタリング距離に変換
	      jk.y=(float)points_curr[i].y/Y_wariai;
	      //jk.z=depth_point_curr[i]/20000;//Depthの値でクラスタリングにちらつきが起こる
	      jk.z=0;//Depthを無視するとチラ付きは消えるが２次元のクラスタリングになる
	      pointCloud->points.emplace_back(jk);//ポイントクラウドに座標データを移動

	      std::cout <<"(float)points_curr["<<i<<"].x/X_wariai"<<(float)points_curr[i].x/X_wariai<< std::endl;//特徴点の個数
	      std::cout <<"(float)points_curr["<<i<<"].y/Y_wariai"<<(float)points_curr[i].y/Y_wariai<< std::endl;//特徴点の個数
	      std::cout <<"depth_point_prev["<<i<<"]/10000       "<<depth_point_curr[i]/10000<< std::endl;//特徴点の個数
    ...
    ec.setClusterTolerance (0.05);//同じクラスタとみなす距離
    ec.setMinClusterSize (5);//クラスタを構成する最小の点数
    ec.setMaxClusterSize (100);//クラスタを構成する最大の点数
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = indices.begin (); it != indices.end (); ++it){ 
        pclj=pclj+1; MAXPX=-10000,MINPX=10000,MAXPY=-10000,MINPY=10000;
        for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit){ // グループ中の点にアクセスするループ(pit=グループ内の点番号)
            // 点へのアクセス
      
            pointCloud -> points[*pit].r = R[pclj%12];//ポイントクラウドのマスの色付け
            pointCloud -> points[*pit].g = G[pclj%12];
            pointCloud -> points[*pit].b = B[pclj%12];
            ...
            //rectangle(img_dst3, Rect((int)(pointCloud -> points[*pit].x*X_wariai), (int)(pointCloud -> points[*pit].y*Y_wariai),W,H), Scalar(B[j%12],G[j%12],R[j%12]),1);//四角作成
			      cv::circle(imageCopy, cv::Point((int)(pointCloud -> points[*pit].x*X_wariai), (int)(pointCloud -> points[*pit].y*Y_wariai)), 6, Scalar(B[pclj%12],G[pclj%12],R[pclj%12]), -1, cv::LINE_AA);//一つ前の画像の座標
      ...
      rectangle(imageCopy,Rect(MINPX,MINPY,MAXPX-MINPX,MAXPY-MINPY),Scalar(255,0,0),2);//四角作成
      rectangle(imageCopy,Rect(MINPX,MINPY,MAXPX-MINPX,MAXPY-MINPY),Scalar(255,0,0),2);//四角作成
    }
...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
      ...
      if(KAL==false){//カルマン推定前はこっち
        for(int i=0;i<ALLMarker;i++){//全マーカー分回す
          if(point[i][3]!=0){//最新要素が無いものを除外
            ...
            xEst0_prev_cl[i](0,0)=MarkerW[i](0,0)/1000;//マーカーの世界座標を使用
            xEst0_prev_cl[i](1,0)=MarkerW[i](1,0)/1000;
            xEst0_prev_cl[i](2,0)=MarkerW[i](2,0)/1000;
            std::cout << "マーカーの世界座標["<<i<<"]=\n"<< xEst0_prev_cl[i]<< std::endl;
  ...
  if(kaisu!=0){
    ...
    if(markerIds.size() > 0){
    std::cout <<"マーカーを使用した外部パラメータ推定"<<std::endl;
    std::cout <<"kosuu="<<kosuu<< std::endl;//推定外部パラメータ
    std::cout <<"At=\n"<<At<< std::endl;
    std::cout <<"Et=\n"<<Et<< std::endl;

    //マーカーの世界座標を使用した外部パラメータ推定
    ALL_At=cv::Mat_<float>(kosuu, 6);
    ALL_Et=cv::Mat_<float>(kosuu, 1);
    At.rowRange(0, kosuu).copyTo(ALL_At.rowRange(0, kosuu));
	  Et.rowRange(0, kosuu).copyTo(ALL_Et.rowRange(0, kosuu));
    std::cout <<"test"<< std::endl;//推定外部パラメータ
    std::cout <<"ALL_At=\n"<<ALL_At<< std::endl;
    std::cout <<"ALL_Et=\n"<<ALL_Et<< std::endl;
    ...
    if(kaisu>1){
      ...
      //マーカーの世界座標を使用する場合
      camera_prmX = ALL_Tt(2,0);
      camera_prmY = ALL_Tt(0,0);
      camera_prmZ = ALL_Tt(1,0);
...
int main(int argc,char **argv){
  ...
  pub_pcl=nhPub.advertise<sensor_msgs::PointCloud2>("depth_pcl", 1000);
