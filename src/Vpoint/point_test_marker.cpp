...
cv::Mat_<float> ALL_prve_Tt=cv::Mat_<float>(6, 1);//一つ前の教科書運動パラメータ
...
void callback(const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg, const sensor_msgs::CameraInfo::ConstPtr& cam_info)
{
  ...
  //オプティカルフロー-------------------------------------------------------------------------------------
    ...
    if(camera_point_p.size()<10||k<10){//特徴点が100個以下になったら再び特徴点を検出する
      reset = true;
  ...
  //オプティカルフロー2-------------------------------------------------------------------------------------
    ...
    if(camera_point_p2.size()<5||k<5){//特徴点が100個以下になったら再び特徴点を検出する
      reset2 = true;
      std::cout <<" 特徴点再検出リセット"<<std::endl;}

    //cv::Mat_<float>F_Pt=cv::Mat_<float>(depth_point_curr2_ok*2, 1);
    //cv::Mat_<float>F_Xt=cv::Mat_<float>(depth_point_curr2_ok*2, 1);
    //cv::Mat_<float>F_Ot_1=cv::Mat_<float>(depth_point_curr2_ok*2, 4);
    ...
    for(int i=0;i<depth_point_curr2_ok;i++){
      ...
      //PT2_At0(PT2_kosuu,0)=-1,  PT2_At0(PT2_kosuu,1)=0,   PT2_At0(PT2_kosuu,2)=0,   PT2_At0(PT2_kosuu,3)=0,                       PT2_At0(PT2_kosuu,4)=-camera_point_p2[i].z,  PT2_At0(PT2_kosuu,5)=-camera_point_p2[i].y;
      //PT2_At0(PT2_kosuu+1,0)=0, PT2_At0(PT2_kosuu+1,1)=-1,PT2_At0(PT2_kosuu+1,2)=0, PT2_At0(PT2_kosuu+1,3)=camera_point_p2[i].z, PT2_At0(PT2_kosuu+1,4)=0,                     PT2_At0(PT2_kosuu+1,5)=-camera_point_p2[i].x;
      //PT2_At0(PT2_kosuu+2,0)=0, PT2_At0(PT2_kosuu+2,1)=0, PT2_At0(PT2_kosuu+2,2)=-1,PT2_At0(PT2_kosuu+2,3)=-camera_point_p2[i].y,PT2_At0(PT2_kosuu+2,4)=camera_point_p2[i].x, PT2_At0(PT2_kosuu+2,5)=0;
    ...
    PT2_Ft = cv::Mat_<float>(3, 3);//回転行列(日高手法)
      ...
      //PT2_Ft(0,0)=1,        PT2_Ft(0,1)=0,        PT2_Ft(0,2)=-PT2_Tt(4,0);
      //PT2_Ft(1,0)=0,        PT2_Ft(1,1)=1,        PT2_Ft(1,2)=0,
      //PT2_Ft(2,0)=PT2_Tt(4,0),  PT2_Ft(2,1)=0,        PT2_Ft(2,2)=1;
      //std::cout <<"回転行列(日高手法)PT2_Ft=\n"<<PT2_Ft<< std::endl;//回転行列
...
//マーカー検出+外部パラメータ推定-------------------------------------------------------------------------------------------  
  ...
  if (markerIds.size() > 0) {
    ...
    if(kaisu!=0){
      ...
      //リサイズ用行列定義
      At=cv::Mat_<float>(kosuu, 6);
      Et=cv::Mat_<float>(kosuu, 1);
      AWt=cv::Mat_<float>(kosuu, 4);//誤差共分散を求めるときに使用する式AW
  ...
  if(kaisu!=0){
    ...
    if(markerIds.size() > 0){
      ...
      //特徴点1検出時(特徴点2+マーカーを使用した外部パラメータ推定)
      if (swap_on==false&&swap_on2==true) {
        std::cout <<"特徴点2を使用した外部パラメータ推定"<<std::endl;
        ALL_At=cv::Mat_<float>(kosuu+PT2_kosuu, 6);
        ALL_Et=cv::Mat_<float>(kosuu+PT2_kosuu, 1);
        At.rowRange(0, kosuu).copyTo(ALL_At.rowRange(0, kosuu));
	      Et.rowRange(0, kosuu).copyTo(ALL_Et.rowRange(0, kosuu));
        PT2_At0.rowRange(0, PT2_kosuu).copyTo(ALL_At.rowRange(kosuu, kosuu+PT2_kosuu));
	      PT2_Et0.rowRange(0, PT2_kosuu).copyTo(ALL_Et.rowRange(kosuu, kosuu+PT2_kosuu));
      }
      //特徴点2検出時(特徴点1+マーカーを使用した外部パラメータ推定)
      ...
      //特徴点+特徴点2+マーカーを使用した外部パラメータ推定
      if (swap_on==true&&swap_on2==true) {
        std::cout <<"特徴点1+特徴点2+マーカーを使用した外部パラメータ推定"<<std::endl;
        ALL_At=cv::Mat_<float>(kosuu+PT_kosuu+PT2_kosuu, 6);
        ALL_Et=cv::Mat_<float>(kosuu+PT_kosuu+PT2_kosuu, 1);
        At.rowRange(0, kosuu).copyTo(ALL_At.rowRange(0, kosuu));
	      Et.rowRange(0, kosuu).copyTo(ALL_Et.rowRange(0, kosuu));
        PT_At0.rowRange(0, PT_kosuu).copyTo(ALL_At.rowRange(kosuu, kosuu+PT_kosuu));
	      PT_Et0.rowRange(0, PT_kosuu).copyTo(ALL_Et.rowRange(kosuu, kosuu+PT_kosuu));
        PT2_At0.rowRange(0, PT2_kosuu).copyTo(ALL_At.rowRange(kosuu+PT_kosuu, kosuu+PT_kosuu+PT2_kosuu));
	      PT2_Et0.rowRange(0, PT2_kosuu).copyTo(ALL_Et.rowRange(kosuu+PT_kosuu, kosuu+PT_kosuu+PT2_kosuu));
      }
      //特徴点1,2ともに検出動作時(マーカーのみで外バラ推定)
      if (swap_on==false&&swap_on2==false) {
    ...
    //マーカーが観測されなかった時（特徴点のみで外パラ推定)
    else{
      std::cout <<"swap_on="<<swap_on<<std::endl;
      std::cout <<"swap_on2="<<swap_on2<<std::endl;
      //特徴点1検出時(特徴点2を使用した外部パラメータ推定)
      if (swap_on==false&&swap_on2==true) {
        std::cout <<"特徴点2を使用した外部パラメータ推定"<<std::endl;
    ...
    if(kaisu>1){
     ...
     //tf(map camera_base間のlink)-----------------------------------------------------------------------------------------
      ...
      camera_base_pose.orientation.z = sin(kaisu*0.1/ 2);
      camera_base_pose.orientation.w = cos(kaisu*0.1/ 2);
  ...
  if(kaisu!=0){
    ALL_prve_Tt = ALL_Tt.clone();//一つ前の外部パラメータを保存(深いコピー)
  }
